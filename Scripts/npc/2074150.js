...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (size < 1) {
            cm.sendOkS("#fs11#"+검정+"이봐, #h #! 더 이상 #r조합이 불가능한 펫#k"+검정+"을 데리고 있나?\r\n닉스 페스티벌#k"+검정+"을 더 재밌게 즐기기 위해 내가 "+펫+" #b루나드림 펫#k "+검정+"1마리 당 #b#z5068300##k "+검정+"1개"+검정+"로 교환해주고 있다네.\r\n\r\n하지만 자네는 교환에 필요한 루나드림 펫이 부족한거 같군. ", 0x04, 9401232);
            cm.dispose();
        } else {
        ...
        count = selection;
        cm.sendYesNoS("#fs11#"+검정+"정말 #b루나드림 펫 " + (count) + "마리#k"+검정+"로 #r#z5068300# " + count + " 개#k"+검정+"를 교환할텐가?", 0x04, 9401232);
    } else if (status == 2) {
        inz = cm.getInventory(5)
        for (w = 0; w < inz.getSlotLimit(); w++) {
            if (!inz.getItem(w) || inz.getItem(w).getPet() == null) {
