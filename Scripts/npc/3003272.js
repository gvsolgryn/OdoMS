...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        chat += "안녕하세요! 저는 닉스월드에서 큐브를 돌려주고있습니다.!" + enter;
        chat += "큐브를 돌리는데에는 아래와같은 규칙이 있습니다." + enter + enter;

        chat += "1. 모든 큐브는 레벨제한이 100이상인 레전더리 옵션의 아이템만 이용하실 수 있습니다." + enter;
        chat += "2. <유지하기> 옵션을 선택하시면 현재 옵션을 유지한 채 큐브를 다시 돌릴수있고," + enter;
        chat += "3. <선택하기> 옵션을 선택하시면 선택하신 옵션으로 아이템의 옵션이 변경됩니다," + enter;
        chat += "4. <대화 그만하기> 를 선택하시면 옵션이 변경되지 않습니다.";

        chat += enter + enter;
        chat += "#L" + CubeStatus.BLACK_CUBE + "# 닉스큐브" + enter;
