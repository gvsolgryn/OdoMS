...
function action(mode, type, sel) {
    ...
    if (status == 0) {
        p = cm.getPlayer().getKeyValue(501661, "point");
        if (p == -1) {
            cm.getPlayer().setKeyValue(501661, "point", "0");
        }
        var msg = "자네 사냥포인트로 HEINZ 코인을 구매하겠는가 ?" + enter;
        msg += "현재 #b#h ##k님의 사냥포인트는 #b" + p + "P#k입니다.#fs11##b" + enter;
        ...
        if (sel == 9999 && cm.getPlayer().isGM()) {
            cm.getPlayer().setKeyValue(501661, "point", "999");
            cm.dispose();
        }
        ...
        if (isEquip) {
            ...
            cm.getPlayer().setKeyValue(501661, "point", "" + (p - selditem['price']));
        ...
        if (!isEquip) {
            ...
            cm.getPlayer().setKeyValue(501661, "point", "" + (p - (selditem['price'] * seldqty)));
            cm.sendOk("거래가 완료되었습니다.");
            cm.dispose();
        }
    }
