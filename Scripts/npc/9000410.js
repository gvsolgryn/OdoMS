...
var banitem = [1148257, 2434146, 2431693, 4001431, 2046992, 2046991, 2047814, 2430143, 4001890, 4001891, 4001893, 4001894, 4001895, 4001896, 4001897, 4001898, 4001899, 4001900, 4001902, 4001904, 4001906, 4001905];
var sda = false;
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        var ask = " #fs11#안녕! 나 맹맹이! 나 배달 잘해. 어느 타입의 아이템을 선물할래?\r\n";
        //ask += "#L1##b[장비]#k아이템을 선물할래!\r\n";
        ask += "#L5##b[캐시]#k아이템을 선물할래!\r\n";
        //ask += "#L6##b[치장]#k아이템을 선물할래!\r\n";
        ask += "#L7##b설명이 듣고 싶어.#k"
        cm.sendSimple(ask);
        ...
        if (selection >= 1 && selection <= 6) {
            cm.sendGetText("#fs11#선물 받을 닉네임을 알려줘!#r\r\n\r\n선물을 하기 위해선 천만메소가 필요하고, 같은 채널에 접속중이여야 해. 잘못된 사용으로 인한 문제는 운영진 측에서 책임지지 않아. 고유 아이템 선물 시 상대방이 이미 아이템을 보유 중일 경우 증발되니까 꼭 확인해줘.");
        } else if (selection == 7) {
            cm.sendOk("#fs11# 선물시스템은 아이템의 종류나 옵션에 관계없이 누구에게나 선물할수 있는 시스템이야. 선물을 하기 위해선 천만메소가 필요하고, 같은 채널에 접속중이여야 해. 잘못된 사용으로 인한 문제는 운영진 측에서 책임지지 않아.");
            cm.dispose();
        }
        ...
        if (ch < 0) {
            cm.sendOk("#fs11# 현재 접속중이 아니거나 채널이 다른가봐. 혹은 존재하지 않는 아이디일 수도 있어.");
            cm.dispose();
        } else {
            ...
            if (!ok) {
                cm.sendOk("#fs11#선물할 아이템이 없는것 같은데?");
                cm.dispose();
                return;
            }
                ...
                if (GameConstants.isPet(item.getItemId()) == false) {
                    if (cm.getPlayer().getName() != text) {
                        if (!canHold(conn, item.getItemId())) {
                            cm.sendOk("#fs11# 선물을 받을 상대방 인벤토리에 빈 공간이 없나봐.");
                            cm.dispose();
                            return;
                        }
                        if (isban) {
                        ...
                        MapleInventoryManipulator.addFromDrop(conn.getClient(), item, true, false, false);
                        conn.getClient().getSession().writeAndFlush(SLFCGPacket.OnYellowDlg(9000086, 3000, "#b" + cm.getPlayer().getName() + "#k 님에게 #b(#t" + item.getItemId() + "#)#k을(를) 선물받으셨습니다. 인벤토리를 확인해보세요.", ""));
                        cm.sendOk("#b" + text + "#k 님에게 #i" + item.getItemId() + "##b(#t" + item.getItemId() + "#)#k을(를) 배달했어!");
                        cm.dispose();
                        sda = true;
                    } else {
                        cm.sendOk("#fs11#응? 장난이지? 자기 자신에게는 선물할수 없어.");
                        cm.dispose();
                    }
                } else {
                    cm.sendOk("#fs11# 펫은 선물할수 없어.");
                    cm.dispose();
                }
            } else {
