	...
	if (getk("day") == 6) {
        ...
        if (selection == 0) {
            ...
            cm.sendSimple(말);
       		...
       		limit = cm.getPlayer().getInventory(GameConstants.getInventoryType(4310261)).countById(4310261);
		    cm.sendGetNumber("#fs11#몇 회나 충전할 거니?\r\n\r\n#r1회당 몬스터 1마리를 충전합니다.\r\n한 번에 2500마리 이상 충전해 발생한 문제는 책임지지 않습니다.\r\n갑자기 남은 마리수가 모두 사라질 경우 캐릭터를 재접속 해주세요.\r\n\r\n#b( 보유 중인 #i"+ ccoin + "# #z" + limit + "개", 1, 1, limit);
     } else {
            cm.sendOk(selection)
        }
    } else if (status == 2) {
		...
		setk("day", (getk("day") + selection));
                cm.addFrozenMobCount(1 * selection);
                cm.gainItem(4310261, -selection);
                cm.getPlayer().dropMessage(1, selection * 1 + "마리 충전이 완료되었습니다.");
