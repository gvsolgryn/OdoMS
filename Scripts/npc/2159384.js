...
function action(mode, type, selection) {
	...
	names = map.names.split(",");
		if(cm.getPlayer().isDead) {
			...
			if(cm.getPlayer().isPoliceVote) {
					...
					}catch(e){					}}				var text = "마피아 일것같은 사람을 지목해 주세요.\r\n";
	...
	if(cm.getPlayer().isPoliceVote) {
		...
		map.broadcastMessage(CWvsContext.serverNotice(11,cm.getPlayer().getName()+"님이 "+names[sel]+"님을 마피아로 선택셨습니다. 마피아 인지 아닌지는 아침에 발표됩니다."));
