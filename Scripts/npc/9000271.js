...
var status = -1;
var getsafe = DatabaseConnection.getConnection().prepareStatement("SELECT * FROM safe").executeQuery();
var item = 0;
var amount = 0;
var id = 0;
var code = 0;
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (selection == 0) {
            ...
            var 리스트 = DatabaseConnection.getConnection().prepareStatement("SELECT * FROM safe WHERE unlocker IS NOT null ORDER BY id").executeQuery();
            while (리스트.next()) {
                list += "[#d" + [리스트.getTimestamp("date").getMonth() + 1] + "월 " + 리스트.getTimestamp("date").getDate() + "일#k] #b" + 리스트.getString("unlocker") + "#k님이 #b#z" + 리스트.getInt("item") + "##k " + 리스트.getInt("amount") + "개 획득\r\n";
            }
            cm.sendOk(list);
            리스트.close();
            cm.dispose();
        ...
        if (selection == code) {
            ...
            DatabaseConnection.getConnection().prepareStatement("UPDATE safe SET `unlocker`='" + unlocker + "' WHERE `id`=" + id + "").executeUpdate();
            ...
            unlocker = cm.getPlayer().getName();
            World.Broadcast.broadcastSmega(CField.getGameMessage(12, "[금고] " + unlocker + "님이 금고를 개봉하려고 했으니 실패 하였습니다."));
