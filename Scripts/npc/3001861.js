...
function action(mode, type, selection) {
    ...
    if (mode == 0) {
        if (status == 19) {
            ...
            cm.sendOkS("마음이 바뀌면 언제든 다시 찾아오게나. 크크.", 4, 9401232)
            cm.dispose();
            return;
        } else if (status = 3) {
            cm.sendOkS("음... 뭐 고민할 시간이 필요한가 보구만, 알겠네. 크크.", 4, 9401232)
            cm.dispose();
            return;
        }
        status--;
    ...
    if (status == 0) {
        말 = "#fs11#" + 검정 + "#h #! 자네는 " + 보라 + "루나 쁘띠 펫#k" + 검정 + "을 가지고 있나? 성능을 극대화 시킬수 있는 방법을 알려주겠네! 크크\r\n";
        말 += "#fc0xFFD5D5D5#────────────────────────────#k#fs11#\r\n";
        말 += "#L1##b루나 쁘띠 펫을 성장시키고 싶어요.\r\n";
        ...
        if (selection == 0) {
            설명 = 1
            말 = "#fs11#" + 검정 + "우선 자네는 " + 펫 + " " + 보라 + "루나 쁘띠 펫#k" + 검정 + "이 있다는 가정하에 설명하겠네!\r\n\r\n";
            말 += "우선, 루나 쁘띠는 " + 레드 + "1기, 2기, 3기#k" + 검정 + "로 나뉘어져 있다네.\r\n";
            말 += "1기와 2기펫" + 검정 + "은 #b#z5069100##k" + 검정 + "을 이용해 뽑기가 가능하지만,\r\n3기펫은 " + 남색 + "성장 시스템#k" + 검정 + "으로만 구할수 있다네.\r\n";
            말 += "성장 성공 확률은 #r50%#k" + 검정 + " 이며, #i4310248# #b#z4310248##k" + 검정 + " 아이템을 소지하면 추가로 20%가 올라간다네.";
            cm.sendNextS(말, 0x04, 9401232);
        } else if (selection == 1) {
            구매 = 2;
            if (!cm.haveItem(5069101, 1)) {
                cm.sendOkS("#fs11#" + 검정 + "자네는 #i5069101# #b#z5069101##k" + 검정 + "이 없는거 같네.", 0x04, 9401232);
                cm.dispose();
                return;
            }
            ...
            if (!c) {
                cm.sendOkS("#fs11#" + 검정 + "이봐, #h # 성장 시킬수 있는 펫이 없는것 같은데?", 0x04, 9401232);
                cm.dispose();
            } else {
            ...
            말 += "　　　　　　└#b#z4310291# #b300 개#k" + 검정 + ""
            cm.sendSimpleS(말, 0x04, 9401232);
        }
    } else if (status == 2) {
        if (구매 >= 0) {
            if (구매 == 1) {
                if (!cm.haveItem(4310291, 300) && cm.getPlayer().getMeso() < 200000000) {
                    cm.sendOkS("자네는 #z5069101#을 구매하기 위한 비용이 부족한거 같네.", 4, 9401232);
                } else {
                ...
                if (!c) {
                    cm.sendOkS("#fs11#이봐, #h # 성장 시킬수 있는 #r2기#k 펫이 없는것 같은데?", 0x04, 9401232);
                    cm.dispose();
                } else {
                    cm.sendSimpleS(txt, 0x04, 9401232);
                }
        ...
        cm.getPlayer().dropMessage(-1, "숲 속 사이 요정들의 축복이 담긴 푸른 보름달 루나 드림이 드러납니다.");
        statusplus(6000);
    } else if (status == 7) {
        ...
        cm.sendNextS("#r#h #!#k 이곳을 소개하지! 이곳은 바로 요정들의 #b푸른 달빛의 축복#k을 받을 수 있는 루나 호수라네!", 0x25, 9062342);
    } else if (status == 16) {
        ...
        if (Randomizer.isSuccess(suc - 15)) {
            ...
            World.Broadcast.broadcastMessage(CWvsContext.serverMessage(11, cm.getPlayer().getClient().getChannel(), "", cm.getPlayer().getName() + "님이 루나 쁘띠 펫 성장에 성공하여 {}를 획득하셨습니다!", true, item));
