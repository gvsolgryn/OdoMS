...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        talk = "스우를 쓰러뜨리기 위해 블랙헤븐 코어로 이동할까?\r\n\r\n"
        //        if (cm.getPlayer().isGM()) {
        //talk += "#L4##b블랙헤븐 코어(익스트림 모드)#k로 이동 한다.(레벨 190이상)#l\r\n"
        //      }
        talk += "#L1#블랙헤븐 코어(#b노멀 모드#k)로 이동 한다.(레벨 190이상)#l\r\n"
        talk += "#L0#블랙헤븐 코어(#b하드 모드#k)로 이동 한다.(레벨 190이상)#l\r\n"
        ...
        if (cm.getParty() == null) {
            ...
            cm.sendOkS("이미 누군가가 스우에 도전하고 있습니다.\r\n다른채널을 이용 해 주세요.", 0x24);
            cm.dispose();
            return;
        } else if (!cm.isLeader()) {
        ...
        if (!cm.partyhaveItem(4001877, 1)) {
            talk = "파티원 중 #i4001877# #b#z4001877##k를 소지하고 있지 않은 파티원이 있습니다.";
        ...
        /*if (!cm.isBossAvailable(setting[st][0], setting[st][1]) && st != 2 && st != 3) {
            ...
            talk += "#k#n님의 레벨이 부족합니다. 스우 " + name[st] + "모드는 " + setting[st][3] + " 레벨 이상만 입장 가능합니다.";
