...
var coin = 4036458; // 재화
var price = 1; // 가격
var allstat = 1000, atk = 1000; // 1회당 올스텟, 공마 증가치
...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		if (item == null) {
			cm.sendOkS("오류가 발생했습니다. 재시도 부탁드립니다.", 4, 9401232);
			cm.dispose();
			return;
		}
		cm.sendNextS("#fs11#정말 #r#i"+item.getItemId()+":# #z"+item.getItemId()+":##k을 강화 하시겠나요? 선택은 되돌릴 수 없으니 신중하게 선택하여 주세요.", 4, 9401232);
	} else if (status == 3) {
		if (sssss == 1) {
			cm.sendScreenText("#fs27##fn나눔고딕 Extrabold##fc0xFF6B66FF#치장아이템 강화#k에 대해 설명 드리겠습니다.", false);
		} else {
			if (!cm.haveItem(coin, price)) {
				cm.sendOkS("#fs11#"+검정+"강화 재료가 부족해요! #i4310248##r#t4310248# 1000개#d가 필요합니다.", 0x04, 9401232);
				cm.dispose();
				return;
			}
			...
			if (item.getItemId() >= 1112000 && item.getItemId() <= 1112099 || item.getItemId() >= 1112800 && item.getItemId() <= 1112899) {
				cm.sendOkS("#fs11#"+검정+"커플링과 우정링은 강화가 불가능해요.", 0x04, 9401232);
				cm.dispose();
				return;
			}
			if (item.getStr() > 249 || item.getDex() > 249 || item.getInt() > 249 || item.getLuk() > 249 || item.getWatk() > 149 || item.getMatk() > 149 || item.getHp() > 2490) {
				cm.sendOkS("#fs11#"+검정+"더 이상 강화는 불가능해요! 한 아이템에 최대 #r30회#d 까지만 강화할 수 있습니다.", 0x04, 9401232);
				cm.dispose();
				return;
			}
			if (item.getItemId() >= 1802000 && item.getItemId() <= 1802999) {
				cm.sendOkS("#fs11#"+검정+"펫 장비는 강화가 불가능해요.", 0x04, 9401232);
				cm.dispose();
				return;
			}
			if (!cm.isCash(item.getItemId())) {
				cm.sendOkS("#fs11#"+검정+"비정상적인 아이템인것 같습니다.", 0x04, 9401232);
				cm.dispose();
				return;
			}
			var suc = false;
