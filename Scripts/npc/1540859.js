...
var price1 = 150000; //가격
var price2 = 1000000; //가격
...
function action(mode, type, selection) {
	...
	sel = selection;
		...
		msg += "#L100##fs11# P마일리지 (올스텟 300/ 공마 300, "+price1+"포인트 지불)\r\n";
		msg += "#L101##fs11# 마일리지 (올스텟 300/ 공마 300, "+price2+"포인트 지불)\r\n";
		cm.sendSimpleS(msg, 0x04, 2192030);
    } else if (status == 2) {
	seld = selection;
	if (seld == 100 && cm.getPlayer().getDonationPoint() < price1) { //가지고 있는 포인트 확인
		cm.sendOk("#fs11#P마일리지가 부족합니다.");
		cm.dispose();
		return;
	} else if (seld == 101 && cm.getPlayer().getHPoint() < price2) { //가지고 있는확인
		cm.sendOk("#fs11#마일리지가 부족합니다.");
		cm.dispose();
		return;
	}
