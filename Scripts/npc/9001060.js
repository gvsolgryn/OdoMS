	...
	if (status == 0) {
		...
		switch (seld) {
			...
			case 3:
				...
				break;
			case 4:
				if (cm.getClient().getQuestStatus(100825) == 2 && cm.getPlayer().getKeyValue(100832, "count") != 1) {
					cm.getPlayer().setKeyValue(100832, "count", "1");
					cm.getPlayer().setKeyValue(100832, "state", "1");
				}
				if (cm.getPlayer().getKeyValue(51351, "startquestid") == 49017) {
					//돌발미션 폴로프리토 1회 클리어
					cm.getPlayer().setKeyValue(51351, "queststat", "3");
					cm.getPlayer().getClient().send(CWvsContext.updateSuddenQuest(cm.getPlayer().getKeyValue(51351, "midquestid"), false, PacketHelper.getKoreanTimestamp(new Date().getTime()) + 600000000, "count=1;Quest=" + cm.getPlayer().getKeyValue(51351, "startquestid") + ";state=3;"));
				}
				cm.getPlayer().setFrittoEgg(null);
				cm.getPlayer().setFrittoEagle(null);
		...
		switch (seld) {
			...
			case 3:
				...
				if (cm.getPlayer().getKeyValue(51351, "startquestid") == 49017) {
					//돌발미션 폴로프리토 1회 클리어
					cm.getPlayer().setKeyValue(51351, "queststat", "3");
					cm.getPlayer().getClient().send(CWvsContext.updateSuddenQuest(cm.getPlayer().getKeyValue(51351, "midquestid"), false, PacketHelper.getKoreanTimestamp(System.currentTimeMillis()) + 600000000, "count=1;Quest=" + cm.getPlayer().getKeyValue(51351, "startquestid") + ";state=3;"));
				}
				cm.dispose();
				cm.gainItem(item, 1);
				cm.warp(parseInt(cm.getPlayer().getV("poloFritto")));
