...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        var chat = "#fs11#안녕하세요? 크루엘라에서 후원을 담당하고 있는 발발이입니다.\r\n\r\n";
	    ...
	    var chat = "#fn나눔고딕 ExtraBold##fs11#크루엘라의 후원 스킬을 구매하고 싶으신가요? 구매를 원하시는 항목을 선택해주시면 구매를 도와드리겠습니다.\r\n\r\n";
	...
	if (selection == 0) {
		...
		if (cm.getPlayer().getDonationPoint() >= 100000) {
		       ...
		       cm.getPlayer().setKeyValue(190830, "symbol", "1");
    		       cm.sendOk("#fn나눔고딕 Extrabold##fs11##s400001020##b <홀리 심볼>#k을 구매하셨습니다.");
    		       cm.dispose();
		} else {
    		       cm.sendOk("#fn나눔고딕 ExtraBold##fs11##r후원포인트#k가 부족하신거 같은데요?");
    		       cm.dispose();
		}
        } else if (selection == 1) {
		...
		if (cm.getPlayer().getDonationPoint() >= 100000) {
		       cm.getPlayer().gainDonationPoint(-100000);
		       cm.getPlayer().setKeyValue(190830, "sharp", "1");
    		       cm.sendOk("#fn나눔고딕 Extrabold##fs11##s100008002##b <샤프 아이즈>#k을 구매하셨습니다.");
    		       cm.dispose();
		} else {
    		       cm.sendOk("#fn나눔고딕 ExtraBold##fs11##r후원포인트#k가 부족하신거 같은데요?");
    		       cm.dispose();
		...
		if (cm.getPlayer().getDonationPoint() >= 100000) {
		       cm.getPlayer().gainDonationPoint(-100000);
		       cm.getPlayer().setKeyValue(190830, "wind", "1");
    		       cm.sendOk("#fn나눔고딕 Extrabold##fs11##s400001006##b <윈드 부스터>#k을 구매하셨습니다.");
    		       cm.dispose();
		} else {
    		       cm.sendOk("#fn나눔고딕 ExtraBold##fs11##r후원포인트#k가 부족하신거 같은데요?");
    		       cm.dispose();
		...
		if (cm.getPlayer().getDonationPoint() >= 300000) {
		       ...
		       cm.getPlayer().setKeyValue(190830, "buck", "1");
    		       cm.sendOk("#fn나눔고딕 Extrabold##fs11##s5321054##b <벅 샷>#k을 구매하셨습니다.");
    		       cm.dispose();
		} else {
    		       cm.sendOk("#fn나눔고딕 ExtraBold##fs11##r후원포인트#k가 부족하신거 같은데요?");
    		       cm.dispose();
		...
		if (cm.getPlayer().getDonationPoint() >= 100000) {
		       cm.getPlayer().gainDonationPoint(-100000);
		       cm.getPlayer().setKeyValue(190830, "heyst", "1");
    		       cm.sendOk("#fn나눔고딕 Extrabold##fs11##s4001005##b <헤이스트>#k을 구매하셨습니다.");
    		       cm.dispose();
		} else {
    		       cm.sendOk("#fn나눔고딕 ExtraBold##fs11##r후원포인트#k가 부족하신거 같은데요?");
    		       cm.dispose();
		...
		if (cm.getPlayer().getDonationPoint() >= 150000) {
		       ...
		       cm.getPlayer().setKeyValue(190830, "magicguard", "1");
    		       cm.sendOk("#fn나눔고딕 Extrabold##fs11##s2001002##b <매직가드>#k을 구매하셨습니다.");
    		       cm.dispose();
		} else {
    		       cm.sendOk("#fn나눔고딕 ExtraBold##fs11##r후원포인트#k가 부족하신거 같은데요?");
    		       cm.dispose();
