...
var haircolor_;
var st;
var skin = Array(0, 1, 2, 3, 4, 9, 10, 11, 12, 13, 15, 16, 18, 19, 24, 25, 26, 27);
	...
	chat = "#L7# 믹스염색 #l#l"+ enter
        ...
        if (selection == 0) { // 스킨
            ...
            if (cm.getPlayer().getJob() == 10112) {
                cm.askAvatar("특별히 #h #님을 위해 무료로 외형을 바꿔드릴게요…. 마음에 드시는 것으로 선택하세요.", skin);
            } else {
                cm.sendStyle("특별히 #h #님을 위해 무료로 외형을 바꿔드릴게요…. 마음에 드시는 것으로 선택하세요.", skin, skin);
            }
        } else if (selection == 1) {
            cm.dispose();
            cm.openNpc(1540493);
        ...
        if (st == 7) {
			...
			cm.getPlayer().updateSingleStat(haircolor[selection]);
