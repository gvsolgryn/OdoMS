...
function action(mode, type, selection) {
    itemcode = [5062005]
    list = [[40057,"크리데미지 + 8%"],[40041,"힘 + 12%"],[40042,"덱 + 12%"],[40043,"인트 + 12%"],[40044,"럭 + 12%"],[40650,"메소 획득량 + 20%"],[30601,"보스 공격력 + 20%"],[30602,"보스 공격력 + 30%"],
           [40656,"드랍률 + 20%"]]
    attlist = [[40051,"공격력 + 12%"],[40052,"마력 + 12%"],[40070,"데미지 + 12%"],[40603,"보스 공격력 + 40%"],[40602,"보스 공격력 + 35%"],[40051, "공격력 + 12%"],[40292,"방어력 무시 + 40%"],[40291,"방어력 무시 + 35%"],[40052,"마력 + 12%"]]     
          if (mode == 1) {
            status++;
    } else {
	cm.dispose();
    ...
    if (status == 0) {
       ...
       if (cm.itemQuantity(itemcode[sT]) >= 1) {
           ...
           cm.sendOk("#fs11#어메이징 미라클 큐브가 부족합니다. 획득처는 후원,홍보상점입니다.");
           cm.dispose();
       }
    } else if (status == 2) {
       if (selection < 9999) {
