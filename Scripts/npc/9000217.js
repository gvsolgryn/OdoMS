...
function action(mode, type, selection, selection2) {
    ...
    if (status == 0) {
            ...
            while (it.hasNext()) {
                ...
                if (itemPair.getRight() != "" || itemPair.getRight() != null) {
                    if (itemPair.getLeft() % 10 == (cm.getPlayer().getHair() % 10) /*&& cm.isExistFH(itemPair.getLeft())*/) {
                        if (getgen == 30 || getgen == 32 || getgen == 33 || getgen == 35 || getgen == 36 || getgen == 40 || getgen == 43 || getgen == 45 || getgen == 46 || getgen == 57 || getgen == 60) {
                            if (showhair == 0) {
                    ...
                    if (itemPair.getLeft() % 10 == (gh % 10) /*&& cm.isExistFH(itemPair.getLeft())*/) {
                        if (getgen == 31 || getgen == 34 || getgen == 37 || getgen == 38 || getgen == 39 || getgen == 41 || getgen == 42 || getgen == 44 || getgen == 47 || getgen == 48 || getgen == 57 || getgen == 61) {
                            if (showhair == 0) {
        ...
        타입 = "";
        if (st == 0) {
            말 += "" + 파랑 + "#e#fs12##fc0xFF6B66FF##e원하는 리스트#n#k#fs11#의 헤어를 선택해보세요!\r\n\r\n"
            if (st2 == 0) {
            ...
            말 += "#fs11#용사님은 어떤 얼굴이 취향이신가요?\r\n";
            if (st2 == 0) {
                말 += "#r(총 " + mface.length + "페이지로 구성되어 있습니다.)#k#n\r\n\r\n"
                for (i = 0; i < mface.length; i++) {
                    말 += "#L" + i + "# " + (parseInt(i) + 1) + "페이지 성형스타일을 선택하겠습니다.#l\r\n";
                }
            } else if (st2 == 1) {
                말 += "#r(총 " + fface.length + "페이지로 구성되어 있습니다.)#k#n\r\n\r\n"
                for (i = 0; i < fface.length; i++) {
                    말 += "#L" + i + "# " + (parseInt(i) + 1) + "페이지 성형스타일을 선택하겠습니다.#l\r\n";
                }
            } else {
                if (cm.getPlayer().getGender() == 0) {
                    말 += "#r(알파 성형은 총 " + mface.length + "페이지로 구성되어 있습니다.)#k#n\r\n\r\n"
                    for (i = 0; i < mface.length; i++) {
                        말 += "#L" + i + "# " + (parseInt(i) + 1) + "페이지 성형스타일을 선택하겠습니다.#l\r\n";
                    }
                } else {
                    말 += "#r(베타 성형은 총 " + fface.length + "페이지로 구성되어 있습니다.)#k#n\r\n\r\n"
                    for (i = 0; i < fface.length; i++) {
                        말 += "#L" + i + "# " + (parseInt(i) + 1) + "페이지 성형스타일을 선택하겠습니다.#l\r\n";
                    }
