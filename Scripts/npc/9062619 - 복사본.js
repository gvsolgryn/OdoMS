...
function action(mode, type, sel) {
	...
	if (status == 0) {
		...
		switch (sel) {
			...
			break;
			case 5:
				var msg = "현재 #b#h ##k님의 갈매기 등급은 #b"+cm.getPlayer().getHgrades()+"#k입니다."+enter;
			...
			case 6:
				if (cm.getPlayer().getHgrade() < 1) {
					cm.sendOk("갈매기 C 이상만 사용할 수 있는 기능입니다.");
					cm.dispose();
					return;
				}
  				time = new Date();
			...
			case 11:
				cm.dispose();
                                                          cm.openNpcCustom(cm.getClient(), 1540102, "1540102");
			break;
			case 100:
				cm.dispose();
