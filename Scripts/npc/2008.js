...
function action(M, T, S) {
    ...
    if (St == 0) {
        ...
        cm.sendSimple("#fs11#닉스월드에 오신것을 환영합니다!\r\n#L1##b장비옵션변경#l\r\n#L2##b잠재옵션코드#l");
    } else if (St == 1) {
        S1 = S;
        switch (S1) {
            case 1:
                inz = cm.getInventory(1)
                txt = "#r옵션을 변경하고 싶은 아이템#k을 선택해주세요.\r\n#b";
...
function addItemInfo() {
    if (rotation == 0) {
        ...
        txt = "#r#e[아이템 기본 옵션]#n\r\n#b#fs11#";
        sel = ["힘", "덱스", "인트", "럭", "최대 체력(HP)", "최대 마나(MP)", "공격력", "마력", "물리방어력", "마법방어력", "명중률", "회피치", "스피드", "점프력", "주문서 성공 횟수", "업그레이드 가능 횟수", "스타포스 성공 횟수", "놀라운 장비강화 주문서 성공 횟수", "보스 공격 시 데미지", "몬스터 방어력 무시", "총 데미지", "올스탯", "착용 레벨 감소", "잠재능력 등급", "1 번째 잠재능력", "2 번째 잠재능력", "3 번째 잠재능력", "4 번째 잠재능력", "5 번째 잠재능력", "6 번째 잠재능력", "모루"];
        ...
        if (S3 != 23) {
            if (max != 99999) {
                cm.sendGetNumber("변경을 원하는 #b" + sel[S3] + "#k 수치의 값을 입력해주세요.", 0, 0, max);
            } else {
                cm.sendGetNumber("변경을 원하는 #b" + sel[S3] + "#k 수치의 값을 입력해주세요.", 0, 0, max);
            }
        } else {
