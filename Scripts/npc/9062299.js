...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
            var leaf = cm.itemQuantity(4310320);
            var text = ""
            text = "#fs11#"+검정+"저는 #fc0xFF000087#[갈매기]#k "+검정+"컨텐츠 부서에서 #b칭호 뽑기#k"+검정+"를 담당하고 있는 니콜이라고 합니다."
            text += "\r\n칭호 뽑기 한번 당 #r#z4310320# 1000개#k"+검정+"로 가 가능합니다.\r\n\r\n";
            text += "현재 보유중인 #d#z4310320##k 갯수 : #e#r"+leaf+"#k개#n\r\n"
            text += "#fc0xFFD5D5D5#───────────────────────────#k\r\n";
            ...
            if (selection == 12) {
                ...
                item = selection;
                if (!cm.haveItem(4310320, 1000)) {
		            cm.sendOk("#fs11##i4310320# #b#z4310320##k 아이템이 부족하거나 없는거 같네요.");
                    cm.dispose();
                } else {
                    cm.sendYesNo("#fs11#정말로 #b칭호#k를 뽑으시겠습니까?\r\n뽑기 진행 시 #r#z4310320# 1000개#k가 소모됩니다.");
                }
            }
        } else if (status == 2) {
            switch (item) {
