...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (selection == 1) {
            if (cm.getPlayer().엔젤릭버스터임시  >= 0) {
                setAvatar(cm.getPlayer().엔젤릭버스터임시);
            }
    } else {
            if (cm.getPlayer().엔젤릭버스터임시  >= 0) {
    
                setevAvatar(cm.getPlayer().엔젤릭버스터임시);
            }
    }
    } else if (status == 2) {
        ...
        if (searchTarget.length >= MIN_SEARCHNAME_LENGTH) {
            ...
            if (searchedMolding.length > 0) {
                if (isHairId) { 
                    cm.askCoupon(5150190, searchedMolding);
                } else {
                cm.askCoupon(5152259, searchedMolding);
            }
              //cm.askAvatar(chat, searchedMolding);
