...
function action(mode, type, selection) {
    setting = [
        ["Normal_BloodyQueen", 1, 105200300, 125],
        ["Chaos_BloodyQueen", 1, 105200700, 180],
        ["Chaos_BloodyQueen", 1, 105200700, 180]
    ]
    name = ["노멀", "하드"]
    if (mode == -1 || mode == 0) {
    ...
    if (status == 0) {
        ...
        talk += "루타비스 남쪽 봉인의 수호자인 #r블러디 퀸#k이 지키고 있는 정원으로 가는 문이다. #r클리어 기록은 노멀의 경우 당일 자정, 카오스의 경우 매주 목요일 자정을 기준으로 초기화 됩니다.#k#n\r\n\r\n"
        talk += "#L0##i4033611##b#z4033611#를 사용하여 노멀 모드로 이동한다.(125레벨 이상)#l\r\n"
        ...
        if (cm.getPlayer().getParty() == null) {
            ...
            cm.sendNext("이미 다른 파티가 안으로 들어가 블러디 퀸에게 도전하고 있는 중입니다.");
            cm.dispose();
            return;
