...
function start() {
	var aa = cm.itemQuantity(4310229); // 자쿰의 돌조각
	    ...
	    //chat += "당신도..토벌 대원으로서 길을 걸어보시겠어요..?\r\n";
            chat += "\r\n--------------------------------------------------------------------------------\r\n";
	    chat += "#L100##fs13##r[보상]#k #b재작가능 입장 재료 확인하기#k#l#fs12#\r\n";
	    chat += "\r\n--------------------------------------------------------------------------------\r\n";
            chat += "                   #d[보스 입장 재료 목록]#k\r\n";
            chat += "--------------------------------------------------------------------------------\r\n";
	    chat += "\r\n#i4310229# #z4310229# (전체필드 몬스터 드랍)             #d("+aa+"/35000)#k\r\n\r\n";
	    chat += "--------------------------------------------------------------------------------\r\n";
	    chat += ""+sel+"";
	    chat += "--------------------------------------------------------------------------------";
	    cm.sendSimple(chat);
...
function action(mode, type, selection) {
	...
	if (selection == 0) {
	    ...
	    if (cm.haveItem(4310229, 35000)) {
		 if (cm.canHold(4036454) && cm.canHold(4036454)) {
			 cm.sendOk("#fn나눔고딕 Extrabold#성공적으로 재료를 구하 셨군요!\r\n\r\n#fUI/UIWindow2.img/QuestIcon/4/0#\r\n#i4036454# #b#z4036454##k #r#k");
			 cm.gainItem(4310229, -35000);
			 cm.gainItem(4036454, 1);
	item = Packages.server.MapleItemInformationProvider.getInstance().getEquipById(4036454);
	item.setStr(30);
	item.setDex(30);
	item.setInt(30);
	item.setLuk(300);
	...
	Packages.server.MapleInventoryManipulator.addbyItem(cm.getClient(), item, false);
	    ...
	    if (cm.haveItem(4310229, 35000)) {
		 if (cm.canHold(4033450) && cm.canHold(4033450)) {
			 cm.sendOk("#fn나눔고딕 Extrabold#성공적으로 재료를 구하 셨군요!\r\n\r\n#fUI/UIWindow2.img/QuestIcon/4/0#\r\n#i4033450# #b#z4033450##k #r#k");
			 cm.gainItem(4310229, -35000);
			 cm.gainItem(4033450, 1);
	item = Packages.server.MapleItemInformationProvider.getInstance().getEquipById(4033450);
	item.setStr(30);
	item.setDex(30);
	item.setInt(30);
	item.setLuk(30);
