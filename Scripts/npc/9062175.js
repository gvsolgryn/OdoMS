...
var questt = "jump_1822"; // jump_고유번호
...
function action(mode, type, sel) {
    ...
    if (status == 0) {
        ...
        var msg = " 인게임 인증을 감사합니다 . 새해복 많이 받으시길바랍니다 .~~~." + enter;
        msg += "클리어 보상을 받으시겠습니까?";
        cm.sendYesNo(msg);
    } else if (status == 1) {
        if (Integer.parseInt(cm.getClient().getKeyValue(questt)) > 0) {
        ...
        cm.gainItem(2630782, 0);
        cm.gainItem(2435796, 1);
        cm.gainItem(2435797, 2);
        cm.gainItem(4310320, 10000);
        cm.gainItem(4310010, 0);
        cm.gainItem(4001833, 0);
