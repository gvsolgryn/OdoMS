...
function action(mode, type, sel) {
	...
	if (status == 0) {
		...
		msg += "\r\n(1) 신규 유저#k\r\n";
		msg += "#L11# #b추천인을 등록하고 싶어요.#n #k\r\n";
		//msg += "#L12# #b지원을 받고 싶어요.#n #k\r\n";
		msg += "#l\r\n(2) 계정 관리 #k\r\n";
		msg += "#L1##b 유니온을 확인하고 싶어요.#n #k\r\n";
		//msg += "#L10# #b버프를 구매하고 싶어요.#n #k\r\n";
		msg += "#l\r\n(3) 캐릭터 #k\r\n";
		msg += "#L20##b 유니온 기어를 확인하고 싶어요.#n #k\r\n";
		msg += "#L5# #b길드를 생성하고 싶어요.#n #k\r\n";
		msg += "#L4# #b현재 랭킹을 확인하고 싶어요.#n #k\r\n";
		msg += "#L50# #b비연을 학습하고 싶어요.#n #k\r\n";
		msg += "#L55# #b커플링 & 우정링을 만들고 싶어요.#n #k\r\n";
		if(GameConstants.isPhantom(cm.getPlayer().getJob())) msg += "#L15# #b스킬을 훔치고 싶어요.#n #k\r\n";
		//msg += "#L11# #b추천인을 등록하고 싶어요.#n #k\r\n";
		//msg += "#L11# #b지원을 받고 싶어요.#n #k\r\n";
		...
		switch (sel) {
			...
			case 11:
				cm.dispose();
				cm.openNpcCustom(cm.getClient(), 3001604, "추천인");
				break;
				case 12:
					cm.dispose();
				...
				switch (sel) { // 레벨업보상
					case 200: // 200레벨 달성시 아래아이템 지급(보관함으로 지급되며 계정당 1회지급됨)
						if (cm.getPlayer().getClient().getKeyValue("level" + this.level) == null) {
							...
							cm.getPlayer().gainCabinetItem(2644000, 1); // 17성
					...
					case 210: // 210레벨
						if (cm.getPlayer().getClient().getKeyValue("level" + this.level) == null) {
							...
							cm.getPlayer().gainCabinetItem(2644000, 2); // 17성
							cm.getPlayer().gainCabinetItem(5121060, 4); // 경뿌
							cm.getPlayer().getClient().setKeyValue("level" + this.level, "true");
							cm.sendOk("보상을 정상적으로 수령하셨습니다.");
							cm.dispose();
					...
					case 220:
						if (cm.getPlayer().getClient().getKeyValue("level" + this.level) == null) {
							...
							cm.getPlayer().gainCabinetItem(2644000, 3); // 17성
							cm.getPlayer().gainCabinetItem(5121060, 4); // 경뿌
							cm.getPlayer().getClient().setKeyValue("level" + this.level, "true");
							cm.sendOk("보상을 정상적으로 수령하셨습니다.");
							cm.dispose();
					...
					case 230:
						if (cm.getPlayer().getClient().getKeyValue("level" + this.level) == null) {
							...
							cm.getPlayer().gainCabinetItem(4001716, 4); // 10억
							cm.getPlayer().gainCabinetItem(2644000, 4); // 17성
					...
					case 240:
						if (cm.getPlayer().getClient().getKeyValue("level" + this.level) == null) {
							...
							cm.getPlayer().gainCabinetItem(4001716, 5); // 10억
							cm.getPlayer().gainCabinetItem(2644000, 4); // 17성
							cm.getPlayer().gainCabinetItem(5121060, 8); // 경뿌
							cm.getPlayer().getClient().setKeyValue("level" + this.level, "true");
							cm.sendOk("보상을 정상적으로 수령하셨습니다.");
					...
					case 250:
						if (cm.getPlayer().getClient().getKeyValue("level" + this.level) == null) {
							...
							cm.getPlayer().gainCabinetItem(5062005, 60); // 어미큐
							cm.getPlayer().gainCabinetItem(5062503, 60); // 화에큐
					...
					case 260:
						if (cm.getPlayer().getClient().getKeyValue("level" + this.level) == null) {
							cm.getPlayer().gainCabinetItem(5060048, 8); // 애플
							cm.getPlayer().gainCabinetItem(2631527, 80); // 경코젬
							cm.getPlayer().gainCabinetItem(5062005, 80); // 어미큐
							cm.getPlayer().gainCabinetItem(5062503, 80); // 화에큐
					...
					case 270:
						if (cm.getPlayer().getClient().getKeyValue("level" + this.level) == null) {
							cm.getPlayer().gainCabinetItem(5060048, 10); // 애플
							cm.getPlayer().gainCabinetItem(2631527, 100); // 경코젬
							cm.getPlayer().gainCabinetItem(5062005, 100); // 어미큐
							cm.getPlayer().gainCabinetItem(5062503, 100); // 화에큐
					...
					case 280:
						if (cm.getPlayer().getClient().getKeyValue("level" + this.level) == null) {
							cm.getPlayer().gainCabinetItem(5060048, 12); // 애플
							cm.getPlayer().gainCabinetItem(2631527, 120); // 경코젬
							cm.getPlayer().gainCabinetItem(5062005, 120); // 어미큐
							cm.getPlayer().gainCabinetItem(5062503, 120); // 화에큐
							cm.getPlayer().gainCabinetItem(2450163, 33); // 3배
					...
					case 290:
						if (cm.getPlayer().getClient().getKeyValue("level" + this.level) == null) {
							cm.getPlayer().gainCabinetItem(5060048, 14); // 애플
							cm.getPlayer().gainCabinetItem(2631527, 140); // 경코젬
							cm.getPlayer().gainCabinetItem(5062005, 140); // 어미큐
							cm.getPlayer().gainCabinetItem(5062503, 140); // 화에큐
							cm.getPlayer().gainCabinetItem(2450163, 38); // 3배
					...
					case 300:
						if (cm.getPlayer().getClient().getKeyValue("level" + this.level) == null) {
							cm.getPlayer().gainCabinetItem(5060048, 16); // 애플
							cm.getPlayer().gainCabinetItem(2631527, 160); // 경코젬
							cm.getPlayer().gainCabinetItem(5062005, 160); // 어미큐
							cm.getPlayer().gainCabinetItem(5062503, 160); // 화에큐
							cm.getPlayer().gainCabinetItem(2450163, 43); // 3배
							cm.getPlayer().gainCabinetItem(4001716, 20); // 10억
							cm.getPlayer().gainCabinetItem(2049376, 3); // 17성
							cm.getPlayer().gainCabinetItem(5121060, 20); // 경뿌
							cm.getPlayer().getClient().setKeyValue("level" + this.level, "true");
							cm.sendOk("보상을 정상적으로 수령하셨습니다.");
