...
function action(mode, type, selection) {
    if (mode == -1) {
		...
		if (status == 0) {
			...
			chat += "[ #b혁신의 룰렛#k 가동 조건 : #b#i4310248##k #r3000 개#k ]\r\n";
			...
			if (cm.haveItem(4310248,3000)) {
				cm.sendOk("#fn나눔고딕 Extrabold#당신의 #b내면의 힘#k 을 믿어보세요.\r\n눈을 크게 뜨고 #r룰렛 점수#k 를 #d확인#k 해주세요.");
		   	} else {
				cm.sendOk("#fn나눔고딕 Extrabold##rNERO 코인이 부족하면 당신의 내면을 바라볼 수 없어요.#k");
				cm.dispose();
			}
		}
		else if (status == 2) {
			cm.gainItem(4310248,-3000);
			cm.getPlayer().혁신의룰렛();
			cm.dispose();
		}
		else {
