...
function action(mode, type, selection) {
	...
	if (status == 0) {
		말 = "#fs11#" + 검정 + "이봐 #b#h ##k" + 검정 + "! 자네가 입고 있는 옷도 강화가 가능한 걸 알고 있나? 내가 도와줄테니 한번 해 보게!\r\n"
		말 += "#fc0xFFD5D5D5#─────────────────────────────#k\r\n";
		...
		sssss = selection;
		if (selection == 0) {
			var txt = "#fs11#" + 검정 + "어떤 아이템을 강화할지 선택해보게!\r\n";
			txt += "#fc0xFFD5D5D5#─────────────────────────────#k\r\n";
		...
		if (item == null) {
			cm.sendOkS("오류가 발생했네. 다시 시도해주게나.", 4, 9401232);
			cm.dispose();
			return;
		}
		cm.sendNextS("#fs11#정말 #r#i"+item.getItemId()+":# #z"+item.getItemId()+":##k을 강화 하겠나? 선택은 되돌릴 수 없으니 신중하게 선택하게나.", 4, 9401232);
	} else if (status == 3) {
		if (sssss == 1) {
			cm.sendScreenText("#fs27##fn나눔고딕 Extrabold##fc0xFF6B66FF#치장아이템 강화#k에 대해 설명 드리겠습니다.", false);
		} else {
			if (!cm.haveItem(coin, price)) {
				cm.sendOkS("#fs11#"+검정+"도와주긴 하겠지만 필요한 재료는 자네가 가져와야 한다네.", 0x04, 9401232);
				cm.dispose();
				return;
			}
			...
			if (item.getStr() > 49 || item.getDex() > 49 || item.getInt() > 49 || item.getLuk() > 49 || item.getWatk() > 49 || item.getMatk() > 49 || item.getHp() > 490) {
				cm.sendOkS("#fs11#"+검정+"더 이상 강화는 불가능하다네.", 0x04, 9401232);
				cm.dispose();
				return;
			}
			if (item.getItemId() >= 1802000 && item.getItemId() <= 1802999) {
				cm.sendOkS("#fs11#"+검정+"펫 장비는 강화가 불가능 하다네.", 0x04, 9401232);
				cm.dispose();
				return;
			}
			if (item.getItemId() >= 1112000 && item.getItemId() <= 1112099 || item.getItemId() >= 1112800 && item.getItemId() <= 1112899) {
				cm.sendOkS("#fs11#"+검정+"커플링과 우정링은 강화가 불가능하다네.", 0x04, 9401232);
				cm.dispose();
				return;
			}
			if (!cm.isCash(item.getItemId())) {
				cm.sendOkS("#fs11#"+검정+"비정상적인 아이템인거 같네.", 0x04, 9401232);
				cm.dispose();
				return;
			}
			var suc = false;
			...
			if (!suc) {
				...
				말 = "#fs11#" + 검정 + "강화에 실패했네.. 현재 자네 아이템의 옵션이라네.\r\n"
				말 += "#fc0xFFD5D5D5#───────────────────────────#k#l\r\n";
				...
				cm.gainItem(coin, -price);
				cm.getPlayer().forceReAddItem(item, Packages.client.inventory.MapleInventoryType.getByType(6));
				말 = "#fs11#" + 검정 + "축하하네! 강화에 성공했다네! 현재 자네 아이템의 옵션이라네.\r\n"
				말 += "#fc0xFFD5D5D5#───────────────────────────#k#l\r\n";
				말 += "#i" + item.getItemId() + "# #fc0xFF4641D9##z" + item.getItemId() + "##k\r\n"
				말 += "#fc0xFF6799FF#STR : + " + (item.getStr() + item.getEnchantStr()) + "\r\n"
				말 += "DEX : + " + (item.getDex() + item.getEnchantDex()) + "\r\n"
		...
		cm.sendScreenText("#fs18#강화마다 #i4310291# #fc0xFFFFBB00##z4310291##k #r10개#k가 필요하며,", false);
	} else if (status == 6) {
