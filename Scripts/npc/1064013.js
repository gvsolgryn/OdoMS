...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        talk = "#r#e<루타비스 동쪽 정원 입구>#k#n\r\n"
        talk += "루타비스 동쪽 봉인의 수호자인 #r반반#k이 지키고 있는 정원으로 가는 문이다. #r클리어 기록은 노멀의 경우 당일 자정, 카오스의 경우 매주 목요일 자정을 기준으로 초기화 됩니다.#k#n\r\n\r\n"
        ...
        if (cm.getPlayer().getParty() == null) {
            ...
            cm.sendNext("이미 다른 파티가 안으로 들어가 반반에게 도전하고 있는 중입니다.");
            cm.dispose();
            return;
        ...
        if (!cm.isBossAvailable(setting[st][0], setting[st][1]) && st != 2) {
            ...
            talk += "#k#n님의 레벨이 부족합니다. 반반은 레벨 " + setting[st][3] + "이상만 도전하실 수 있습니다.";
        } else {
