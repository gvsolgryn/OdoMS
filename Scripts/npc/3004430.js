...
var coin = 4310248; // 재화
var price = 2000; // 가격
var allstat = 5, atk = 3; // 1회당 올스텟, 공마 증가치
...
function action(mode, type, selection) {
	if (mode == 1) {
		...
		if (status == 2) {
			cm.sendOkS("그래, 알겠네 마음이 바뀌면 다시 찾아오게나.", 4, 9401232);
		}
		cm.dispose();
		return;
	}
	if (status == 0) {
		말 = "#fs11#" + 검정 + "안녕하세요 #b#h ##k님! \r\n";
                        말 = "#fs11#" + 검정 + "혹시 캐시 아이템도 강화가 된다는 사실 알고 계신가요?  \r\n";
                        말 = "#fs11#" + 검정 + "치장 강화 시스템은 #i4310248##b#t4310248##d을 통해 치장 아이템을 \r\n";
                        말 = "#fs11#" + 검정 + "강화할 수 있는 시스템입니다. \r\n";
                        말 = "#fs11#" + 검정 + "한번에 #r올스탯 5와 공격력,마력이 3#d씩 증가합니다. \r\n";
                        말 = "#fs11#" + 검정 + "HEINZ 코인 2000개로 50회까지 강화가 가능합니다.                             1회당 올스텟+5 공마+3                                                                #r펫 장비를 비롯한 일부 아이템#d에는 사용할 수 없습니다.\r\n"
		말 += "#fc0xFFD5D5D5#─────────────────────────────#k\r\n";
		...
		if (item == null) {
			cm.sendOk("오류가 발생했습니다. 재시도 부탁드립니다.");
			cm.dispose();
			return;
		}
		cm.sendNext("#fs11#정말 #r#i"+item.getItemId()+":# #z"+item.getItemId()+":##k을 강화 하시겠나요? 선택은 되돌릴 수 없으니 신중하게 선택하여 주세요.");
	} else if (status == 3) {
		if (sssss == 1) {
			cm.sendScreenText("#fs27##fn나눔고딕 Extrabold##fc0xFF6B66FF#치장아이템 강화#k에 대해 설명 드리겠습니다.", false);
		} else {
			if (!cm.haveItem(coin, price)) {
				cm.sendOk("#fs11#"+검정+"강화 재료가 부족해요! #i4310248##r#t4310248# 2000개#d가 필요합니다.");
				cm.dispose();
				return;
			}
			...
			if (item.getItemId() >= 1112000 && item.getItemId() <= 1112099 || item.getItemId() >= 1112800 && item.getItemId() <= 1112899) {
				cm.sendOk("#fs11#"+검정+"커플링과 우정링은 강화가 불가능해요.");
				cm.dispose();
				return;
			}
			if (item.getStr() > 251 || item.getDex() > 251 || item.getInt() > 251 || item.getLuk() > 251 || item.getWatk() > 151 || item.getMatk() > 151 || item.getHp() > 5001) {
				cm.sendOk("#fs11#"+검정+"더 이상 강화는 불가능해요! 한 아이템에 최대 #r50회#d 까지만 강화할 수 있습니다.");
				cm.dispose();
				return;
			}
			if (item.getItemId() >= 1802000 && item.getItemId() <= 1802999) {
				cm.sendOk("#fs11#"+검정+"펫 장비는 강화가 불가능해요.");
				cm.dispose();
				return;
			}
			if (!cm.isCash(item.getItemId())) {
				cm.sendOk("#fs11#"+검정+"비정상적인 아이템인것 같습니다.");
				cm.dispose();
				return;
			}
			var suc = false;
