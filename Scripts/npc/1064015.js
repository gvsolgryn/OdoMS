...
function action(mode, type, selection) {
    setting = [
        ["Normal_Vellum", 1, 105200400, 100],
        ["Chaos_Vellum", 1, 105200800, 180],
        ["Chaos_Vellum", 1, 105200800, 180]
    ]
    name = ["노멀", "하드"]
    if (mode == -1 || mode == 0) {
    ...
    if (status == 0) {
        talk = "#r#e<루타비스 북쪽 정원 입구>#k#n\r\n"
        talk += "루타비스 북쪽 봉인의 수호자인 #r벨룸#k이 지키고 있는 정원으로 가는 문이다. #r클리어 기록은 자정을 기준으로 초기화 됩니다.#k#n\r\n\r\n"
        talk += "#L0##i4033611##b#z4033611#를 사용하여 노말 모드로 이동한다.(100레벨 이상)#l\r\n"
        ...
        if (cm.getPlayer().getParty() == null) {
            cm.sendNextS("파티를 맺어야만 입장할 수 있는 것 같군. 파티를 찾아보자.", 2)
            cm.dispose();
            return;
        } else if (!cm.isLeader()) {
            ...
            cm.sendNext("이미 다른 파티가 안으로 들어가 벨룸에게 도전하고 있는 중입니다.");
            cm.dispose();
            return;
        ...
        if (!cm.isBossAvailable(setting[st][0], setting[st][1]) && st != 2) {
            ...
            talk += "#k#n님의 레벨이 부족합니다. 벨룸은 레벨 " + setting[st][3] + "이상만 도전하실 수 있습니다.";
        } else {
