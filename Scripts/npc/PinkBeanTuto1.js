...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		cm.getClient().send(SLFCGPacket.InGameDirectionEvent("", 7, 1000, 1500, 1000, 3380, 734));
		statusplus(1000);
	} else if (status == 76) {
		cm.getClient().send(SLFCGPacket.ShowEffectChatNpc(1, "크크... 내가 이기면 친구들처럼 분홍 분홍 하게 만들어줘야지.", 0, 1, 3000, 1, 0, 300, 0, 4, 9072307, 0, 1719));
		statusplus(3000);
	} else if (status == 77) {
		cm.getClient().send(SLFCGPacket.ShowEffectChatNpc(1, "이곳엔 신기한 것들이 많아 보이니까, 재밌게 놀면서 모험을 해볼까?", 0, 1, 3000, 1, 0, 300, 0, 4, 9072307, 0, 1719));
		statusplus(4000);
	} else if (status == 78) {
		cm.removeNpc(9072306);
		cm.removeNpc(9072307);
