...
function action(mode, type, selection) {
        var count = 0;
    setting = [
        ["Chaos_Mitsuhide", count, 874000011, 290]
    ]
    name = ["노멀", "하드"]
    if (mode == -1 || mode == 0) {
        cm.dispose();
    ...
    if (status == 0) {
         talk = "#e<보스: 아케치 미츠히데 ( 하드 )>#n\r\n"
         talk+= "私の中の悪の光秀を防ぎに来ました。!.\r\n\r\n"
        talk+= "#L0##b<보스: 아케치 미츠히데 ( 하드 )> 입장을 신청한다.#l\r\n"
        ...
        if (!cm.isBossAvailable(setting[st][0], setting[st][1])) {
            ...
            talk = "파티원 중 미츠히데에 도전하기 위한 레벨이 부족한 사람이 있어."
