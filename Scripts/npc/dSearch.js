...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		if(cm.getPlayer().getDonationPoint() >= 가격) {
			cm.sendGetText("\r\n#fs11##fc0xFF000000#원하는 치장 아이템을 검색해!\r\n\r\n아이템의 정확한 명칭을 모른다면\r\n#b아이템의 이름 일부분만 입력해도 검색이 가능하니 알아두라구\r\n#r단, 안드로이드엔 착용 불가합니다.\r\n#fc0xFF000000#\r\n#fs11#\r\n#b후원 검색캐시는 올스텟 공마 + 50과 크뎀 +6%를 가지고 있어!#fc0xFF000000#\r\n\r\n");
		} else {
        ...
        cm.sendSimple("#fs11#입력한 [#b" + cm.getText() + "#k]의 검색 결과야\r\n\r\n" + t);
    } else if (status == 2) {
        	sel = selection;
	...
	if (banitem.indexOf(sel) != -1) {
		cm.sendOk("#fs11#해당 아이템은 구매할 수 없어");
		cm.dispose();
		return;
	}
...
try {
	...
	cm.getPlayer().setDonationPoint(cm.getPlayer().getDonationPoint() - 가격);
        cm.sendYesNo("#fs11#선택한 아이템을 지급했어!\r\n추가로 더 원하는 게 있다면 '예'를 눌러줘");
}catch(e){
cm.sendOk(e);
}
   } else if (status == 4) {
