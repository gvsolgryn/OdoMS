var s1;
var mutos = [[4034959, 0, 1, 130000, 45000], [4034960, 1, 1, 130000, 45000], [4034961, 2, 1, 130000, 45000], [4034962, 3, 1, 130000, 45000], [4034963, 4, 1, 130000, 45000], [4034964, 5, 1, 130000, 45000], [4034965, 6, 1, 130000, 45000], [4034966, 7, 1, 130000, 45000], [4034967, 8, 1, 130000, 45000], [4034968, 9, 1, 130000, 45000], [4034969, 10, 1, 130000, 45000], [4034970, 11, 1, 130000, 45000], [4034971, 12, 1, 130000, 45000], [4034972, 13, 1, 130000, 45000], [4034973, 14, 1, 130000, 45000], [4034974, 15, 1, 130000, 45000]];
...
function action(mode, type, selection) {
	...
	if (cm.getPlayer().getMapId() == 450002024) {
		...
		cm.sendSimple("#b#e<배고픈 무토>#n#k\r\n #b무토#k를 도와 #r굴라#k를 물리치는 걸 도와 주시겠어요?\r\n#r※10분간 플레이가 진행되며 중간에 이탈할 수 없습니다.#k\r\n#b#L0# <배고픈 무토>에 입장한다.#l\r\n#L1# 시미아 에게서 설명을 듣는다.#l\r\n#L2#오늘의 남은 도전횟수를 확인한다.#l");  
		        ...
		        cm.sendOk("#r레벨 #b210 이상#k부터 입장 가능합니다.#k");
		        cm.dispose();
		    }
    } else if (status == 1) {
	s1 = selection;
	...
	if (s1 == 0) {
		...
		} else if (cm.isMemberCleared("muto", "3") != null) {
			cm.sendOk("파티원 중 #b#e" + cm.isMemberCleared("muto") + "#k#n 님이 오늘 클리어 횟수를 전부 사용했습니다.");
		} else if (cm.getPlayerCount(maps[selection]) > 0) {
			cm.sendOk("이미 누군가가 입장했습니다.");
		} else {
			cm.resetMap(maps[selection]);
			cm.warpParty(maps[selection]);
			muto = mutos[Packages.server.Randomizer.nextInt(8)];
			cm.getPlayer().getMap().setMuto(new Packages.server.maps.HungryMuto(muto[0], muto[1], muto[2], muto[3], muto[4]), cm.getPlayer().getParty());
	    		mobid = setting[selection];

	                for (i = 0; i < xy.length; i++) {
				cm.spawnMonster(mobid[parseInt(i / 5)], xy[i][0], xy[i][1]);
	                }
			
			rand = Packages.server.Randomizer.nextInt(xy.length);

			cm.spawnMonster(mobid[mobid.length - 1], xy[rand][0], xy[rand][1]);
