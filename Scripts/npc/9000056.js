...
function action(mode, type, selection) {
	...
	if (status == 0) {
			...
			말 = 아이콘+"#fs15##fn나눔고딕 Extrabold# #r"+ServerConstants.serverName+" #bSend Item#k#fs12##fn굴림체#\r\n───────────────────────────\r\n\r\n";
			말 +="#fs11#* 선물에는 1 천만 메소가 차감됩니다.\r\n";
			...
			if (selection >= 1 && selection <=6) {
				cm.sendGetText("#fs11##b1. 선물받으실 분의 닉네임을 입력해주세요.");
			}
	} else if (status == 2) {
			if (operation == 1) {
				type = MapleInventoryType.EQUIP;
		...
		if (conn == null) {
			cm.sendOk("#fs11##r현재 접속중이 아니거나 채널이 다릅니다.\r\n또는 존재하지 않는 닉네임일 수도 있습니다.");
			cm.dispose();
		} else {
		ok = false;
		selStr = "#fs11##b2. 어떤 아이템을 선물 하시겠습니까.?\r\n";
		for (x = 1; x < 2; x++) {
		...
		if (!ok) {
			cm.sendOk("#fs11##r인벤토리에 아이템이 존재하지 않습니다.");
			cm.dispose();
		}
		...
		if (statsSel == null) {
			cm.sendOk("#fs11##r오류입니다. 운영자에게 보고해주세요.");
			cm.dispose();
		}
		text = cm.getText();
		...
		if (leftslot < 1) {
			cm.sendOk("#fs11##r상대방의 "+타입+" 인벤토리 공간이 최소한 1 칸은 필요합니다.");
			cm.dispose();
			return;
		}
		if (item.getQuantity() == 1) {
			...
			if (isban) {
				cm.sendOk("#fs11##r해당 아이템은 선물이 제한된 품목입니다.");
				cm.dispose();
				return;
			}
			if (cm.getMeso()>=메소) {
				/*if (item.getItemId() != 1115012 && item.getItemId() != 1115013 && item.getItemId() != 1115014 && item.getItemId() != 1115100 && item.getItemId() != 1115101 && item.getItemId() != 1115102) {
					if (cm.isCash(item.getItemId()) && (item.getStr() >= 1 || item.getDex() >= 1 || item.getInt() >= 1 || item.getLuk() >= 1 || item.getMatk() >= 1 || item.getWatk() >= 1 || Item.getHp() >= 1)) {
						cm.sendOk("#fs11##r스탯이 부여된 캐시 장비는 선물하실 수 없습니다.");
						cm.dispose();
						return;
					}
				}*/
			//	if (GameConstants.isPet(item.getItemId()) == false) {
					if (cm.getPlayer().getName() != text) {
						...
						cm.sendOk("#fs11##b"+text + "#k 님에게 #i"+item.getItemId()+"# #d(#t"+item.getItemId()+"#)#k 을(를) 보냈습니다.");
						cm.gainMeso(-메소);
						cm.dispose();
					} else {
						cm.sendOk("#fs11##r자기 자신에게는 선물할 수 없습니다.");
						cm.dispose();
					}
			//	} else {
			//		cm.sendOk("#fs11##r펫은 선물할 수 없습니다.");
			//		cm.dispose();
			//	}
			} else {
				cm.sendOk("#fs11##r선물을 위한 메소가 부족합니다.");
				cm.dispose();
			}
		} else {
			cm.sendGetNumber("#fs11##b3. 총 몇 개를 선물 하시겠습니까.?#k\r\n현재 소지중인 #i"+item.getItemId()+"# #d(#t"+item.getItemId()+"#)#k 개수 : #b"+item.getQuantity()+"",1,1,item.getQuantity());
		}
		name = text;
	} else if (status==4) {
		sele = selection%1000;
		...
		if (text != 0xFFFF && text <= 32767 && text <= 0x7FFF && item.getQuantity() >= text && text > 0 && text > -1 && selection != 0xFFFF && selection <= 32767 && selection <= 0x7FFF && item.getQuantity() >= selection && selection > 0 && selection > -1) {
			...
			if (cm.getMeso()>=메소){
				...
				if (cm.getPlayer().getName() != name) {
					...
					cm.sendOk("#fs11##b"+name + "#k 님에게 #i"+item.getItemId()+"# #d(#t"+item.getItemId()+"#) #b"+item.getQuantity()+" 개#k 를 보냈습니다.");
					cm.gainMeso(-메소);
					cm.dispose();
				} else {
					cm.sendOk("#fs11##r자기 자신에게는 선물할 수 없습니다.");
			...
			cm.sendOk("#fs11##r수량이 부족하거나, 입력된 값이 잘못되었습니다.");
			cm.dispose();
		}
