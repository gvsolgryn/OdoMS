...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        chat += "반가워요! #h #님 저는 원클릭으로 에디큐브를 돌려주는 NPC입니다." + enter+ enter;

        chat += "#e1#n. #r에디 큐브는 레전드리미만의 아이템에 사용하면 등급업이 되지않습니다.#k" + enter;
        chat += "#e2#n. #fc0xFFb45f04#<Before> 옵션을 선택하시면 현재 옵션을 유지한 채 큐브를 다시 돌릴수있습니다#k" + enter;
        chat += "#e3#n. #fc0xFF0404b4# <After> 옵션을 선택하시면 선택하신 옵션으로 아이템의 옵션이 변경됩니다.#k         "; + enter;


        chat += enter + enter;
        //chat += "#L" + CubeStatus.BLACK_CUBE + "# #b블랙큐브 돌리기#k" + enter;
        chat += "#L" + CubeStatus.ADDITIONAL_CUBE + "# #b에디셔널 큐브#k" + enter;
        //chat += "#L" + CubeStatus.RED_CUBE + "# #b레드큐브#k" + enter;
        //chat += "#L" + CubeStatus.GOODCRAFT_CUBE + "# #b장인큐브#k" + enter;
        ...
        if (!checkCost(cubeState)) {
            ...
            switch (selection) {
                ...
                case CubeStatus.GOODCRAFT_CUBE:
                ...
                case CubeStatus.ADDITIONAL_CUBE:
                    composer = function (_item) {
                        ...
                        if (_item.getPotential4() > 0 &&requireLevel >= 100) {
                            return true;
                        } else {
                            return false;
                        }
...
function checkCost(_cubestate) {
    if (cm.haveItem(getPotentialItemId(_cubestate), 2)) {
        return true;
    }
...
function decreaseCost(_cubestate) {
    if (checkCost(_cubestate)) {
        cm.gainItem(getPotentialItemId(_cubestate), -2);
        return true;
    }
