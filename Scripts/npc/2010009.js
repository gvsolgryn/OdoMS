	...
	if (status == 0) {
	    ...
	    if (selection == 0) {
			...
			if (cm.getPlayer().getParty() == null || partymembers == null || partymembers.size() != 2 || !cm.isLeader()) {
				...
				gs = cm.getGuild(cm.getPlayer().getGuildId());
				gs2 = cm.getGuild(partymembers.get(1).getGuildId());
				
				alliance = cm.getAlliance(gs.getAllianceId());				
				alliance2 = cm.getAlliance(gs2.getAllianceId());
				if (cm.partyMembersInMap() < 2) {
					cm.sendOk("파티원 전부 같은 맵에 있어야 합니다.");
					cm.dispose();
				} else if (alliance != null || alliance2 != null) {
					cm.sendOk("이미 연합을 맺고 있습니다.");
					cm.dispose();
				} else
...
/*		    if (alliance != null && alliance2 == null) {
			cm.sendYesNo(alliance.getName() + " 연합에 새로운 길드를 등록합니다. 계속하시겠습니까?");
		    } else if (alliance2 != null && alliance == null) {
			cm.sendYesNo(alliance2.getName() + " 연합에 새로운 길드를 등록합니다. 계속하시겠습니까?");
		    } else {*/
...
/*		if (alliance != null && alliance2 == null) {
		    cm.setNewAlliance(gs2.getId(), alliance.getId());
		    cm.sendOk(alliance.getName() + " 연합에 " + gs2.getName() + " 길드가 새로 등록되었습니다.");
		    cm.dispose();
		} else if (alliance2 != null && alliance == null) {
		    cm.setNewAlliance(gs.getId(), alliance2.getId());
		    cm.sendOk(alliance2.getName() + " 연합에 " + gs.getName() + " 길드가 새로 등록되었습니다.");
		    cm.dispose();
		} else {*/
