...
function action(mode, type, selection) {
    setting = [
        ["Normal_FireWolf", 1, 993000500, 295]
    ]
    names = ["노멀"];
    if (mode == -1 || mode == 0) {
        cm.dispose();
    ...
    if (status == 0) {
        talk = "#L1#불꽃늑대 입장하기 (레벨 295이상)#e#k#l";
        ...
        if (cm.getParty() == null) {
            ...
            cm.sendOkS("이미 누군가가 불꽃늑대에 도전하고 있습니다.\r\n다른채널을 이용 해 주세요.", 0x26);
            cm.dispose();
            return;
        } else if (!cm.isLeader()) {
        ...
        if (!cm.isBossAvailable(setting[st][0], setting[st][1])) {
            ...
            talk += "#k#n님이 오늘 입장했습니다. 불꽃늑대는 하루에 " + setting[st][1] + "번만 도전하실 수 있습니다.";
            ...
            talk += "#k#n님의 레벨이 부족합니다. 불꽃늑대는 " + setting[st][3] + " 레벨 이상만 입장 가능합니다.";
            cm.sendOkS(talk, 0x26);
            cm.dispose();
            return;
        } else {
