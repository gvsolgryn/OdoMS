...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (selection <= 3) {
            ...
            말 = "#fs11##z4310026# 아이템을 구매하기 위해 아래의 아이템이 필요합니다.\r\n"
            말 += "#fc0xFFD5D5D5#───────────────────────────#k\r\n";
        ...
        if (sel == 0) {
            ...
            if (!cm.haveItem(4310022, 500) || !cm.haveItem(4310024, 500)) {
                cm.sendOk("#fs11##i4310026# #b#z4310026##k 아이템을 구매하기 위한 재료가 부족합니다.");
                cm.dispose();
                return;
            }
        ...
        말 = "#fs11##i" + item + "# #b#z" + item + "##k 아이템을 제작하겠나?"
        ...
        if (!cm.haveItem(need1, 1) || !cm.haveItem(need2, 1) || !cm.haveItem(need3, 1) || !cm.haveItem(4310022, needcoin)) {   //재료체크
            cm.sendOkS("#fs11##b#z" + item + "##k을 제작하기 위한 재료 아이템이 부족한거 같군.", 0x04, 9401232);
            cm.dispose();
            return;
        }
        if (needcoin2 > 0) {
        ...
        if (sucss) {// 성공했을때
            ...
            말 = "#fs11##b#z" + item + "##k 제작에 성공했다네!\r\n\r\n"
            말 += "#fUI/UIWindow2.img/QuestIcon/4/0#\r\n"
            말 += "#i" + item + "##b#z" + item + "##k"
            cm.sendOkS(말, 0x04, 9401232);
            cm.dispose();
        } else { // 실패했을때
            cm.sendOkS("#fs11##r제작이 실패했네.. 다음에는 꼭 성공해보게!", 0x04, 9401232);
            cm.dispose();
        }
    }
}