...
var map = new Array(953020000, 953030000, 953040000, 953050000, 953060000, 953070000, 953080000, 953090000, 954000000, 954010000, 954020000, 954030000, 954040000, 954050000, 954060000, 954070000, 954080000, 954090000);
var mapname = new Array("자동 경비 구역(Lv.105~114)", "이끼나무 숲(Lv.115~124)", "하늘 숲 수련장(Lv.120~129)", "해적단의 비밀 기지(Lv.125~134)", "이계의 전장(Lv.135~144)", "외딴 숲 위험 지역(Lv.140~149)", "금지된 시간(Lv.145~154)", "숨겨진 유적(Lv.150~159)", "폐허가 된 도시(Lv.160~169)", "죽은 나무의 숲(Lv.170~179)", "감시의 탑(Lv.175~184)", "용의 둥지(Lv.180~189)", "망각의 신전(Lv.185~194)", "기사단의 요새(Lv.190~199)", "원혼의 협곡(Lv.200~209)", "소멸의 여로","츄츄 아일랜드", "꿈의 도시 레헬른");
...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		var text = "#e<오늘은 #b" + String(Packages.tools.CurrentTime.getDay()) + "#k입니다.>\r\n<오늘 클리어 횟수 #b" + cm.getClient().getKeyValue("mPark") + " / 7회#k (월드 통합당 기준)>\r\n#e오늘의 무료 클리어 가능 횟수가 #b" + (7 - cm.getClient().getKeyValue("mPark")) + "회#k 남았습니다.#n#b\r\n";
		...
		cm.dispose();
		if (cm.getClient().getKeyValue("mPark") >= 7) {
