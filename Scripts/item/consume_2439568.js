...
/* 아이템 세팅 */
var 소울종류 = new Array(2591668, 2591669, 2591670, 2591671, 2591672, 2591673, 2591674, 2591675, 2591676);
아이템 = [2591668, 2591669, 2591670, 2591671, 2591672, 2591673, 2591674, 2591675];
소울조각코드 = 2439568
위대한소울뽑힐확률 = 5
위대한소울코드 = 2591676
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		if (cm.haveItem(소울조각코드,10)) {
			cm.sendYesNo("#b#z"+소울조각코드+"##k 10개를 소비하여 소울 아이템으로 교환 할거야?", 9000174);
		} else {
			말 = "#b#z"+소울조각코드+"##k\r\n"
		...
		if (leftslot < 2) {
			cm.sendOk("#fs11##r소비창 2칸 이상을 확보해주세요.");
			cm.dispose();
			return;
		}
		...
		cm.gainItem(아이템코드, 1);
		if (debeck == 1) {
			cm.sendOk(위대한대사, 9000174);
		} else {
			cm.sendOk("오호~! #b#z"+아이템코드+"##k이 나왔구나! 요간하게 쓰도록 하렴. 크크크...", 9000174);
		}
		cm.dispose();
   	}
}
