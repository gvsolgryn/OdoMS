...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        var text = "#fs11#이봐 #e#h ##n! #b닉스#k을 즐기기 전에 선물 하나를 주겠네.\r\n";
        ...
        switch (cm.getPlayer().getJob()) {
            ...
            case 10112:
                ...
                text += "\r\n#fs11##r제로 직업군은 무기대신 반지를 지급합니다.#k\r\n"
                break;
            case 200:
            case 210:
            case 211:
            ...
            case 3612:
                ...
                break;
            default:
                cm.sendOkS("#fs11#자네는 전직을 하지 않았구만, 전직을 하고 다시 찾아오게.", 0x04, 9090006)
                break;
        }
        cm.sendSimpleS(text, 0x04, 9090006);
    } else if (status == 1) {
        item = selection;
        cm.sendYesNoS("#fs11##i" + item + "# #r#z" + item + "# 아이템#k을 정말로 받을텐가?\r\n\r\n\r\n#Cgray#잘못 선택시엔 다시 지급받기 어렵습니다.", 0x04, 9090006);
    } else if (status == 2) {
        var a = "";
        a += "#fs11##b[블랙]#k 캐릭터당 #r1회#k만 지급한다네. 아참! 이 아이템은 #r2주간#k 사용이 가능하니 주의하도록 하게나! \r\n\r\n";
        a += "#fUI/UIWindow2.img/QuestIcon/4/0#\r\n";
        for (i = 0; i < 기본지급.length; i++) {
            a += "#fs11##b#i" + 기본지급[i] + "# #z" + 기본지급[i] + "# #k\r\n";
        ...
        if (안줌 == 0) {
            a += "#fs11##b#i" + item + "# #z" + item + "##k #r(선택한 무기)#k #n\r\n";
