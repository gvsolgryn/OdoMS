...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getPlayer().getLevel() > 200) {
            cm.sendOkS("#fs11#현재 캐릭터는 200레벨이 넘어 사용이 불가능합니다.", 0x04, 9010061);
            cm.dispose();
            return;
        }
        if (GameConstants.isYeti(cm.getPlayer().getJob()) || GameConstants.isPinkBean(cm.getPlayer().getJob())) {
            cm.sendOkS("#fs11#예티 직업군과 핑크빈 직업군은 이용이 불가능합니다.", 0x04, 9010061);
            cm.dispose();
            return;
        }
        cm.sendYesNoS("#fs11#정말 현재 캐릭터로 " + 노랑 + "#i2430046##z2430046##k 아이템을 사용하시겠습니까?", 0x04, 9010061);
        return;
    } else if (status == 1) {
