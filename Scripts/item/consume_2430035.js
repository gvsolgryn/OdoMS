...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (!cm.haveItem(2430034, 1)) {
            cm.sendOk("#fs11#" + 검정 + "#i2430034# #z2430034# 아이템이 없는거 같군.");
            cm.dispose();
            return;
        }
        if (selection == 0) {
            ...
            if (!c) {
                cm.sendOk("#fs11#" + 검정 + "이봐, #h # 각인서를 사용할 안드로이드가 없는거 같네.");
                cm.dispose();
            } else {
                cm.sendSimple(txt);
            }
        ...
        if (selection == 0) {
            ...
            NPCScriptManager.getInstance().startItem(cm.getPlayer().getClient(), 9401232, "consume_2430034");
        } else if (selection == 1) {
            cm.getClient().removeClickedNPC();
            cm.dispose();
        }
