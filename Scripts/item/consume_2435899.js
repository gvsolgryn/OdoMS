...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        var amed = "#fs11##d#h0#님, 원하시는 위대한 소울을 선택해 주세요.#k" + enter
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (leftslot < 2) {
            cm.sendOkS("#fs11#소비 인벤토리창 2칸 이상을 비워주세요.", 0x04, 9010061);
            cm.dispose();
            return;
        }
        ...
        cm.sendOkS("#fs11##i" + itemid + "# #b#z" + itemid + "# " + count + "개#k", 0x04, 9010061);
        cm.gainItem(2435899, -1);
        cm.dispose();
    }
}*/