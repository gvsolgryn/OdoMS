...
function action(mode, type, selection) {
	...
	if (St == 0) {
	  ...
	  text += "#L3##b#i1713002##z1713002#\r\n";
	  cm.sendYesNo(text);
	} else if (St == 1) {
		seld = selection;
		cm.sendGetNumber("사용하실 아이템 갯수를 선택해주세요.", 1, 1, 100);
	} else if (St == 2) {
		if (!cm.canHold(1713000+seld-1, 3*seld)) {
			cm.sendOk("죄송하지만 인벤토리 공간이 충분하지 않으신 것 같네요. #b장비#k탭의 인벤토리 공간을 비워주세요.");
			cm.dispose();
			return;
		}
		if (!cm.haveItem(2439999, selection)) {
			cm.sendOk("사용할 아이템이 부족합니다.");
			cm.dispose();
			return;
		}
		cm.gainItem(1713000+seld-1,3*selection);
		cm.gainItem(2439999,-1*selection);
