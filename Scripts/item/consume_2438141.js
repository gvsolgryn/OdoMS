...
function action(mode, type, selection) {
    ...
    if (status == 0) {
	말 = "#fs11#받고싶은 #d아케인심볼#k 을 선택해 주세요.\r\n\r\n"
	...
	seld = selection;
	cm.sendYesNo("#fs11#정말 받고싶은 아이템이 #i"+심볼[selection]+"# #d#z"+심볼[selection]+"##k (이)가 맞으신가요.?");
    } else if (status == 2) {
	if (cm.getPlayer().getInventory(MapleInventoryType.EQUIP).getNumFreeSlot() < 지급개수) {
		cm.sendOk("#fs11##r장비 슬롯 "+지급개수+" 칸 이상의 인벤토리 공간이 필요합니다.");
		cm.dispose();
		return;
	}
	...
	cm.sendOk("#fs11##i"+심볼[seld]+"# #d#z"+심볼[seld]+"# #b"+지급개수+" 개#k 를 지급 해드렸습니다.");
	cm.dispose();
    }
}