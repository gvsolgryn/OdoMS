...
var itemlist = [[2450064, 5], [4319998, 1]];
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        cm.gainItem(2437926, -1);
        for (i = 0; i < itemlist.length; i++) {
            cm.gainItem(itemlist[i][0], itemlist[i][1]);
        }
        //cm.getPlayer().gainDonationPoint(6000000);
        World.Broadcast.broadcastMessage(CWvsContext.serverNotice(2, "[알림] : " + cm.getPlayer().getName() + " 님이 후원 30만 패키지를 개봉 하셨습니다!"));
