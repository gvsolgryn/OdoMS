...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        말 = "#fs11#" + 검정 + "자네, 안드로이드에 #d신비한 힘#k" + 검정 + "을 새기는 #e" + 파랑 + "각인 시스템#k#n" + 검정 + "이라고 알고 있나?\r\n"
        말 += "#fc0xFFD5D5D5#────────────────────────────#k" + 검정 + "\r\n";
        말 += "#L0##b각인 시스템을 이용하겠습니다.\r\n"
        말 += "#L99##r설명을 듣겠습니다."
        cm.sendOkS(말, 0x04, 9401232);
    } else if (status == 1) {
        if (!cm.haveItem(2430215, 1)) {
            cm.sendOk("#fs11#" + 검정 + "#i2430215# #z2430215# 아이템이 없는거 같군.");
            cm.dispose();
            return;
        }
        if (selection == 0) {
            ...
            txt = 검정 + "#fs11#" + 파랑 + "각인#k" + 검정 + "을 사용할 안드로이드를 선택해보게.\r\n\r\n";
            ...
            if (!c) {
                cm.sendOkS("#fs11#" + 검정 + "이봐, #h # 각인서를 사용할 안드로이드가 없는거 같네.", 0x04, 9401232);
                cm.dispose();
            } else {
                cm.sendSimpleS(txt, 0x04, 9401232);
            }
        } else if (selection == 99) {
            말 = "#fs11#" + 파랑 + "#e각인 시스템#n#k" + 검정 + "이란?\r\n\r\n"
            말 += "안드로이드에 " + 보라 + "추가적인 성능#k" + 검정 + "을 부여하는 시스템입니다.\r\n"
            말 += "각인은 " + 빨강 + "최대 10회#k" + 검정 + "까지 가능하며, 각인의 옵션은 아래와 같습니다.\r\n\r\n"
            말 += "- 올스텟 최대 #r1 ~ 20" + 검정 + "\r\n"
            말 += "- 공격력 / 마력 최대 #r1 ~ 15" + 검정 + "\r\n"
            말 += "- 최대 체력 #r100 ~ 500" + 검정 + "\r\n"
            말 += "- 보스 몬스터 공격 시 데미지 최대 #r1 ~ 10%" + 검정 + "\r\n\r\n"
            말 += "#r10회의 각인을 마친 아이템은 더 이상 강화가 불가능하며, #z2430035#를 이용하여 초기화 한 뒤, 각인이 가능합니다."
            cm.sendOk(말);
        }
    } else if (status == 2) {
        inz = cm.getInventory(1).getItem(selection);
        txt = "#fs11#" + 검정 + "정말 #i"+inz.getItemId()+"# #b#z"+inz.getItemId()+"##k"+검정+"을(를) 각인 시키겠나?\r\n\r\n\r\n";
        if (!ItemFlag.UNTRADEABLE.check(inz.getFlag())) {
            txt += "#fs15##r※ 해당 아이템은 각인 시스템을 적용 시 교환불가 상태로 변경 됩니다.\r\n\r\n"
            txt += "계속 진행 하시겠다면 '예' 버튼을 눌러주세요.";
        }
        txt += "";
        cm.sendYesNoS(txt, 0x04, 9401232);
    } else if (status == 3) {
        if (Randomizer.isSuccess(40)) {
            올스텟 = Randomizer.rand(13, 15);
        } else if (Randomizer.isSuccess(35)) {
            올스텟 = Randomizer.rand(15, 20);
        } else {
            올스텟 = Randomizer.rand(1, 13);
        }
        if (Randomizer.isSuccess(40)) {
            공마 = Randomizer.rand(9, 12);
        } else if (Randomizer.isSuccess(30)) {
            공마 = Randomizer.rand(12, 15);
        } else {
            공마 = Randomizer.rand(1, 9);
        }
        if (Randomizer.isSuccess(40)) {
            보공 = Randomizer.rand(4, 7);
        } else if (Randomizer.isSuccess(15)) {
            보공 = Randomizer.rand(7, 10);
        } else {
            보공 = Randomizer.rand(1, 4);
        }
        체력 = Randomizer.rand(100, 500);
        if (inz.getEnhance() >= 10) {
            cm.sendOk("#fs11#" + 검정 + "현재 아이템은 각인이 완료되어 더 이상 각인이 불가능하다네. 크크크.");
            cm.dispose();
            return;
        }
        ...
        inz.setBossDamage(inz.getBossDamage() + 보공);
        ...
        말 = "#fs11#"+검정+""+각인횟수+" 각인의 결과라네. 크크.\r\n"
        말 += "#fc0xFFD5D5D5#─────────────────────────────#k#fs11#" + 파랑 + "\r\n";
        ...
        if (selection == 0) {
            ...
            NPCScriptManager.getInstance().startItem(cm.getPlayer().getClient(), 9401232, "consume_2430215");
        } else if (selection == 1) {
            cm.getClient().removeClickedNPC();
            cm.dispose();
        }
