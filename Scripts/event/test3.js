...
function playerEntry(eim, player) {
    ...
    if (player.getParty().getLeader().getId() == player.getId()) {
        spawnMonster(eim, 8830000);
        spawnMonster(eim, 8830004);
        spawnMonster(eim, 8830005);
    }
}

function spawnMonster(eim, mobid) {
    var map = eim.getMapInstance(0);
    var mob = em.getMonster(mobid);
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(483, 258));
}
