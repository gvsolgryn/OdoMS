...
function setup(mapid) {
    ...
    map = parseInt(mapid);
    while (em.getInstance("Hard_Lotus" + a) != null) {
        a = Randomizer.nextInt();
    }
    var eim = em.newInstance("Hard_Lotus" + a);
...
function playerEntry(eim, player) {
    ...
    if (player.getParty().getLeader().getId() == player.getId()) {
        spawnMonster(eim, 0, 8950000, 0, -16);
    }
}
...
function monsterValue(eim, mobId) {
    stage = parseInt(eim.getProperty("stage"));
    if (mobId == 8950000 && stage == 0) {
        eim.setProperty("stage", "1");
        eim.schedule("WarptoNextStage", 5000);
    } else if (mobId == 8950001 && stage == 1) {
...
function WarptoNextStage(eim) {
    ...
    spawnMonster(eim, stage, 8950000 + stage, 0, -16);
}
