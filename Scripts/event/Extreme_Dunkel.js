...
function setup(mapid) {
    ...
    map = parseInt(mapid);
    while (em.getInstance("Extreme_Dunkel" + a) != null) {
        a = Randomizer.nextInt();
    }
    var eim = em.newInstance("Extreme_Dunkel" + a);
...
function playerEntry(eim, player) {
    ...
    if (map.getMobsSize(8645066) <= 0)
    spawnMonster(eim);
}

function spawnMonster(eim) {
    var map = eim.getMapInstance(0);
    var mob = em.getMonster(8645066, true, false);
    eim.registerMonster(mob);
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(0, 20));
    var mob2 = em.getMonster(8645068, true, false);
