...
var questName = "ArcQuest3"
var quest1 = 39038;//퀘스트최소대역
var quest2 = 39048;//퀘스트최대대역
    ...
    if (status == 0) {
        ...
        if (replace) {
            ...
            if (selection > 0 && selection != 999) {
                ...
                str = qm.getPlayer().getV(questName);
                ab = str.split(",");
                txt = "제외된 " + changecount + "개의 일 대신 새로운 일 " + changecount + "개를 알려줄게. 오늘의 부탁은 모두 " + questcount + "가지야.\r\n\r\n"
            ...
            //수행
            qm.sendOk("모든 부탁을 완료하면 나한테로 다시 돌아오면 돼.\r\n꼭 #r#e자정 전#k#n에 돌아오는 것 잊지마.\r\n부탁에 대해 궁금한 것이 있으면 #b#e퀘스트 정보창#k#n을 통해 확인할 수 있으니 참고하도록 해.");
            startQuest();
        }
    }
}
...
function end(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        qm.sendNext("우리가 부탁한 " + questcount + "가지의 일을 모두 해줬구나?\r\n고마워. 여기#i1712004# #z1712004# 10개,\r\n#i2435719# #z2435719# 10개를 가져가.\r\n너희들에게 필요한 거라 들었어.");
    } else if (status == 1) {
        for (a = 0; a < 5; a++) {
            qm.gainItem(1712004, 10);
        }
        qm.gainItem(2435719, 10);
        qm.getPlayer().removeKeyValue(39037);
