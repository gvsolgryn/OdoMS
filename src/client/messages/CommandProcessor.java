...
public class CommandProcessor {
  ...
  public static boolean processCommand(MapleClient c, String line, ServerConstants.CommandType type) {
    ...
    if (c.getPlayer().getGMLevel() >= co.getReqGMLevel()) {
      ...
      try {
        ...
        FileoutputUtil.outputFileError("Log/Log_Command_Except.rtf", e);
      } 
  ...
  private static void logCommandToDB(MapleCharacter player, String command, String table) {
    ...
    try {
      ...
      FileoutputUtil.outputFileError("Log/Log_Packet_Except.rtf", ex);
      ex.printStackTrace();
    } finally {
      try {
        ps.close();
  ...
  static {
    ...
    for (Class<?> clasz : array) {
      try {
        ...
        ex2.printStackTrace();
        FileoutputUtil.outputFileError("Log/Log_Script_Except.rtf", ex2);
      } 
    } 
  }
}
