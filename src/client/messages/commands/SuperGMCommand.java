    ...
    public int execute(MapleClient c, String[] splitted) {
      ...
      public static class 후원포인트지급 extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
          ...
          public static class 프리미엄패스지급 extends CommandExecute {

        @Override
        public int execute(MapleClient c, String[] splitted) {
            MapleCharacter chrs = c.getChannelServer().getPlayerStorage().getCharacterByName(splitted[1]);
            if (chrs == null) {
                c.getPlayer().dropMessage(6, "현재 " + splitted[1] + "님은 접속중이지 않습니다.");
            } else {
                chrs.addKV("HAS_PREMIUM_PASS", "0");
                chrs.getClient().getSession().writeAndFlush(SLFCGPacket.OnYellowDlg(9010061, 7000, "#face0#운영자로부터 #r끼룩 프리미엄 패스권#k을 지급받으셨습니다.", ""));
                c.getPlayer().dropMessageGM(5, chrs.getName() +"님에게 프리미엄 패스를 지급하였습니다.");
         }
            return 1;
        }
    }
