...
public class InternCommand {
    ...
    public static class 워프 extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length < 2) {
                c.getPlayer().dropMessage(5, "사용법: !워프 [캐릭터 이름 또는 맵 ID] [포털 ID (옵션)]");
                return 0;
            }
            ...
            if (victim != null && c.getPlayer().getGMLevel() >= victim.getGMLevel() && !victim.inPVP() && !c.getPlayer().inPVP()) {
                if (splitted.length == 2) {
                    ...
                    try {
                        int targetMapId = Integer.parseInt(splitted[2]);
                        MapleMap targetMap = ChannelServer.getInstance(c.getChannel()).getMapFactory().getMap(targetMapId);
                        if (targetMap == null) {
                        ...
                        if (splitted.length > 3) {
                            try {
                                int portalId = Integer.parseInt(splitted[3]);
                                targetPortal = targetMap.getPortal(portalId);
                            } catch (IndexOutOfBoundsException e) {
                                c.getPlayer().dropMessage(5, "유효하지 않은 포털 ID입니다.");
                        ...
                        c.getPlayer().dropMessage(5, "유효하지 않은 맵 ID입니다.");
                    }
                }
            } else {
                try {
                    victim = c.getPlayer();
                    int targetChannel = World.Find.findChannel(splitted[1]);
                    if (targetChannel > 0) {
                        victim = ChannelServer.getInstance(targetChannel).getPlayerStorage().getCharacterByName(splitted[1]);
                        if (victim != null) {
                            c.getPlayer().dropMessage(6, "채널을 변경하였습니다. 잠시만 기다려주세요.");
                            if (victim.getMapId() != c.getPlayer().getMapId()) {
                                MapleMap targetMap = c.getChannelServer().getMapFactory().getMap(victim.getMapId());
                                c.getPlayer().changeMap(targetMap, targetMap.findClosestPortal(victim.getTruePosition()));
                            }
                            c.getPlayer().changeChannel(targetChannel);
                        } else {
                            c.getPlayer().dropMessage(6, "캐릭터를 찾을 수 없습니다.");
                            return 0;
                        }
                    } else {
                        int targetMapId = Integer.parseInt(splitted[1]);
                        ...
                        if (splitted.length > 2) {
                            try {
                                int portalId = Integer.parseInt(splitted[2]);
                                targetPortal = targetMap.getPortal(portalId);
                            } catch (IndexOutOfBoundsException e) {
                                c.getPlayer().dropMessage(5, "유효하지 않은 포털 ID입니다.");
                            } catch (NumberFormatException numberFormatException) {
