                ...
                if (!igskill) {
                    ...
                    if (chra.getBuffedEffect(SecondaryStat.Lotus) != null) {
                        ...
                        if (chra.getBuffedEffect(SecondaryStat.ReviveOnce).getSourceId() == 24111002) {
                            ...
                            chra.getClient().getSession().writeAndFlush((Object)CField.EffectPacket.showEffect(chra, 3000, 14111030, 62, 0, 0, (byte)(chra.isFacingLeft() ? 1 : 0), true, chra.getTruePosition(), null, null));
                            chra.getMap().broadcastMessage(chra, CField.EffectPacket.showEffect(chra, 3000, 14111030, 62, 0, 0, (byte)(chra.isFacingLeft() ? 1 : 0), false, chra.getTruePosition(), null, null), false);
                        }
                        this.hp = chra.getStat().getCurrentMaxHp() / 2L;
                        chra.updateSingleStat(MapleStat.HP, this.hp);
...
//        this.reduceCooltime = new ArrayList<Integer>();
    ...
    public void recalcLocalStats(final boolean first_login, final MapleCharacter chra) {
        ...
        if (chra.getBuffedEffect(SecondaryStat.DropItemRate) != null) {
            this.dropBuff += chra.getBuffedValue(SecondaryStat.DropItemRate);
        }
...
//                }
            ...
            case 15411:
            case 15412: {
                bx = SkillFactory.getSkill(154100007); //피지컬 트레이닝
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    this.localluk += eff.getLukX();
                }
                bx = SkillFactory.getSkill(154000004); //플레어
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    this.watk += bx.getEffect(bof).getX();
                ...
                bx = SkillFactory.getSkill(154000005); //페이션스 - 회피율 추가하기
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                ...
                bx = SkillFactory.getSkill(154000005); //임펄스
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    this.critical_rate = (short) (this.critical_rate + eff.getCr());
                ...
                bx = SkillFactory.getSkill(154110008); //인투이션
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    ...
                    this.InsertFinalDamage(eff.getPdR());
                    this.ignoreMobpdpR.add(Integer.valueOf(eff.getIgnoreMob()));
                    this.InsertDamageIncrease(154101001, bx.getEffect(bof).getDAMRate()); // 아츠 크레센텀
                }
                bx = SkillFactory.getSkill(154110009); //비질런스
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    this.percent_hp += bx.getEffect(bof).getPercentHP();
                ...
                bx = SkillFactory.getSkill(154120010); //어센드
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    this.InsertDamageIncrease(154110001, bx.getEffect(bof).getDAMRate());
                    this.InsertDamageIncrease(154111006, bx.getEffect(bof).getDAMRate());
                    this.InsertDamageIncrease(154101001, bx.getEffect(bof).getDAMRate()); // 아츠 크레센텀
                }
                bx = SkillFactory.getSkill(154120011); //리뎀션
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    this.InsertFinalDamage(eff.getPdR());
                ...
                bx = SkillFactory.getSkill(154100003); //인새니티 1
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    this.InsertDamageIncrease(154121009, bx.getEffect(bof).getDAMRate());
                }

                bx = SkillFactory.getSkill(154110003); //인새니티 2
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    this.InsertDamageIncrease(154121009, bx.getEffect(bof).getDAMRate());
                ...
                bx = SkillFactory.getSkill(154120008); //인새니티 3
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    this.InsertDamageIncrease(154121009, bx.getEffect(bof).getDAMRate());
