...
public class PlayerStats {
        ...
        if (chra != null) {
            if (hp <= 0) {
                if (chra.getSkillLevel(14110030) > 0 && !chra.skillisCooling(14110030)) {
                    ...
                    chra.Message("다크니스 어센션의 효과로 죽음의 운명을 회피했습니다.");
                    ...
                    if (Randomizer.isSuccess(10)) {
                        ...
                        chra.Message("구사 일생의 효과로 죽음의 운명을 회피했습니다.");
                        return true;
                    }
                    ...
                    return true;
                } else if (chra.getBuffedValue(BuffStats.SoulStone) != null) { // 소울 스톤
                ...
                if (chra.getDeathCount() > 0) {
                    ...
                    if (chra.getDeathCount() <= 0) {
                        chra.dropMessage(5, "데스카운트를 모두 소비하여, 광장으로 이동 되었습니다.");
                        chra.addHP(chra.getStat().getCurrentMaxHp());
                        chra.addMP(chra.getStat().getCurrentMaxMp());
    ...
    public void recalcLocalStats(MapleCharacter chra) {
        try {
            ...
            if (chra.getSkillLevel(36101003) > 0) { //제논 2차 에피션시 파이프라인
                percent_hp += 30;
                percent_mp += 30;
            }
...
*/
    ...
    public final void ZeroData(final WritingPacket packet, final MapleCharacter chr) {
        packet.writeShort(-1); // 제로 스탯 시작을 알림.
