...
import server.CustomSetItemInfo;
...
public class PlayerStats implements Serializable {
    ...
    public short str, dex, luk, int_, simbolstr, simboldex, simbolint_, simbolluk;
    ...
    private transient int magic, buffmagic, watk, buffwatk, hands, accuracy;
    public transient boolean equippedWelcomeBackRing, hasClone, hasPartyBonus, Berserk, Fury, canFish, canFishVIP;
    public transient double expBuff, dropBuff, mesoBuff, cashBuff, mesoGuard, mesoGuardMeso, expMod, pickupRange, eventExpRate;
    //same with incMesoProp/incRewardProp for now
    public transient double dam_r, bossdam_r;
    public transient int reduceCooltime, recoverHP, recoverMP, mpconReduce, mpconPercent, incMesoProp, incRewardProp, DAMreflect, DAMreflect_rate, mpRestore,
    ...
    public List<Integer> currentSetEffectID = new ArrayList<>();
    public List<Integer> currentSetEffectSkillID = new ArrayList<>();
    
    public transient CustomizeStat customizeStat;
    ...
    public final boolean setHp(int newhp, boolean silent, MapleCharacter chra) {
        ...
        if (chra != null) {
            ...
            if (oldHp > hp && !chra.isAlive()) {
                ...
                chra.playerDead();
                chra.getClient().getSession().write(MaplePacketCreator.enableActions());
            }
        }
        return hp != oldHp;
    }
    ...
    public void recalcLocalStats(boolean first_login, MapleCharacter chra) {
        this.currentSetEffectID.clear();
        this.currentSetEffectSkillID.clear();
        
        //여기서는 로그인 후 처음 접속 시에만 업데이트, 어차피 알아서 다른 곳에서 실시간 반영 됨.
        if (first_login) {
            if (this.customizeStat == null)
                this.customizeStat = new CustomizeStat(chra);
            
            this.customizeStat.setFrom(System.currentTimeMillis(), false);
        }
        ...
        wdef = 0;
        mdef = 0;
        localdex = getDex() + customizeStat.DEX;// + getSymbolDex();
        localint_ = getInt() + customizeStat.INT;// + getSymbolInt();
        localstr = getStr() + customizeStat.STR;// + getSymbolStr();
        localluk = getLuk() + customizeStat.LUK;// + getSymbolLuk();
        speed = 100;
        jump = 100;
        pickupRange = 0.0;
        decreaseDebuff = 0;
...
//                            bossdam_r += (add.getValue().right + 100.0) / 100.0;
                    ...
                    if (i > 0) {
                        if (ii.getPotentialInfo(i) != null) {
                            ...
                            if (pot != null) {
                                ...
                                dropBuff += pot.incRewardProp;
                                mesoBuff += pot.incMesoProp;
                            }
                        }
                    }
                }
            }
            final Map<String, Integer> eqStats = ii.getEquipStats(equip.getItemId());
            if (equip.getDurability() > 0 && eqStats.containsKey("durability")) {
            ...
            if (!maxLVBuff && !first_login) {
                int maxLVBuffItemID = 1112593; //영메링 아이템 코드 설정하는 곳
                if (equip.getItemId() == maxLVBuffItemID) {
                    maxLVBuff = true;
                    int incSkillLV = 0;
                    int incSpeed = equip.getSpeed();
                    //여기서 부터
                    if (incSpeed >= 5 && incSpeed < 10) { //5이상 10미만
                        incSkillLV = 1; //5이상 10미만일 시 스킬 레벨 +1
                    } else if (incSpeed >= 10 && incSpeed < 15) { //10이상 15미만
                        incSkillLV = 2;
                    } else if (chra.isGM()) { //GM일 때 강제로 레벨 설정
                        incSkillLV = 3; //현재 3레벨 버프 줌
                    }
                    //여기까지 이동속도 수치 별 스킬 레벨 설정해주는 곳
                    if (incSkillLV > 0) {
                        incSkillLV = Math.min(incSkillLV, 100); //최대 100레벨
                        int incSkillID = 1005; //getSkillByJob(1005, chra.getJob());
                        Skill incSkill = SkillFactory.getSkill(incSkillID);
                        if (incSkill != null) {
                            MapleStatEffect incSkillData = incSkill.getEffect(1);
                            if (incSkillData != null) {
                                int SLV = chra.getSkillLevel(incSkillID);
                                SLV += incSkillLV;
                                
                                if (SLV > 0) {
                                    SLV = Math.min(SLV, 100); //최대 100레벨, 공마력 상승치 = x(기본 위젯값 4) * 스킬 레벨
                                    Map<MapleBuffStat, Integer> maxLVBuffMask = new EnumMap<>(MapleBuffStat.class);
                                    maxLVBuffMask.put(MapleBuffStat.ECHO_OF_HERO, incSkillData.getX() * SLV);
                                    
                                    long cur = System.currentTimeMillis();
                                    chra.getClient().sendPacket(TemporaryStatsPacket.giveBuff(incSkillID, Integer.MAX_VALUE, maxLVBuffMask, incSkillData));
                                    MapleStatEffect.CancelEffectAction cancelAction = new MapleStatEffect.CancelEffectAction(chra, incSkillData, cur, maxLVBuffMask);
                                    ScheduledFuture<?> schedule = Timer.BuffTimer.getInstance().schedule(cancelAction, cur + Integer.MAX_VALUE - System.currentTimeMillis());
                                    chra.registerEffect(incSkillData, cur, schedule, maxLVBuffMask, true, Integer.MAX_VALUE, -1);
                                }
                            }
                        }
                    }
        ...
        int maxLVBuffLeaderID = chra.getBuffLeaderID(MapleBuffStat.ECHO_OF_HERO);
        if (!maxLVBuff && maxLVBuffLeaderID == -1 && chra.getBuffedValue(MapleBuffStat.ECHO_OF_HERO) != null) {
            chra.cancelEffectFromBuffStat(MapleBuffStat.ECHO_OF_HERO);
        }
        ...
        if ( setHandling != null) {
            ...
            while (iter.hasNext()) {
                ...
                final CustomSetItemInfo customItemInfo = ii.getCustomSetItem(entry.getKey());
                if (set != null) {
                ...
                if (customItemInfo != null && !first_login) {
                    customItemInfo.setItemEffect(chra);
                }
            }
        }
        ...
        buff = chra.getBuffedValue(MapleBuffStat.EMHP);
        if (buff != null) {
...
//                    damageIncrease.put(21100002, (int) eff.getDAMRate());
                    damageIncrease.put(21101002, (int) eff.getDAMRate());
...
//        bossdam_r += (pot.incDAMr + 100.0) / 100.0;
        ...
        if (first_login && pot.potentialID == 30040) {
            chra.changeSkillLevel_Skip(SkillFactory.getSkill(getSkillByJob(8006, chra.getJob())), (byte) 1, (byte) 0);
        }
        if (first_login && pot.potentialID == 30400) {
            chra.changeSkillLevel_Skip(SkillFactory.getSkill(getSkillByJob(8004, chra.getJob())), (byte) 1, (byte) 0);
        }
        if (first_login && pot.potentialID == 30401) {
            chra.changeSkillLevel_Skip(SkillFactory.getSkill(getSkillByJob(8005, chra.getJob())), (byte) 1, (byte) 0);
        }
    }
    ...
    public final short getSimbolStr() {
        return simbolstr;
    }

    public final short getSimbolDex() {
        return simboldex;
    }

    public final short getSimbolLuk() {
        return simbolluk;
    }

    public final short getSimbolInt() {
        return simbolint_;
    }
    
    public final void setSimbolStr(final int str) {
        this.simbolstr = (short) str;
    }

    public final void setSimbolDex(final int dex) {
        this.simboldex = (short) dex;
    }

    public final void setSimbolLuk(final int luk) {
        this.simbolluk = (short) luk;
    }

    public final void setSimbolInt(final int int_) {
        this.simbolint_ = (short) int_;
    }
    
    private transient int symbolStr, symbolDex, symbolInt, symbolLuk;
    
    public final int getSymbolStr() {
        return symbolStr;
    }

    public final int getSymbolDex() {
        return symbolDex;
    }

    public final int getSymbolInt() {
        return symbolInt;
    }

    public final int getSymbolLuk() {
        return symbolLuk;
    }

    public final void setSymbolStr(int asd) {
        symbolStr = asd;
    }

    public final void setSymbolDex(int asd) {
        symbolDex = asd;
    }

    public final void setSymbolInt(int asd) {
        symbolInt = asd;
    }

    public final void setSymbolLuk(int asd) {
        symbolLuk = asd;
    }

    public final void gainSymbolStr(int asd) {
        symbolStr += asd;
    }

    public final void gainSymbolDex(int asd) {
        symbolDex += asd;
    }

    public final void gainSymbolInt(int asd) {
        symbolInt += asd;
    }

    public final void gainSymbolLuk(int asd) {
        symbolLuk += asd;
    }
}
