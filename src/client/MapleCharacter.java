...
import client.serenityRank.MainRankInfo;
...
public class MapleCharacter extends AnimatedMapleMapObject implements Serializable {
    ...
    public int 발할라검격;
    public int 리인카네이션;
    public int 플레시미라주스택, 스킬카운트, addBuffCheck;
    public int 메이플용사;
    public long lastAstraTime;
    ...
    private Map<String, String> keyValues_boss;
    ...
    public int lightning, siphonVitality, armorSplit, PPoint, combination, stackbuff, combinationBuff, BULLET_SKILL_ID,
            SpectorGauge, LinkofArk, FlowofFight, Stigma, bulletParty;
    public int criticalGrowing, criticalDamageGrowing, bodyOfSteal;
    public java.util.Timer MulungTimer;
    ...
    private MapleCharacter(final boolean ChannelServer, final boolean firstIngame) {
        ...
        this.keyValues_boss = new HashMap<String, String>();
        ...
        if (ChannelServer) {
            ...
            this.deathcount = -1;
            this.secondaryStatEffectTimer = Timer.BuffTimer.getInstance().register(new MapleCharacterManagement(), 1000L);
        }
    }
                    ...
                    for (final MapleMonster mob : arrays) {
                        if (!mob.isBuffed(2111003)) {
                            if (mist.getBox().contains(mob.getTruePosition())) {
                                ...
                                applys.add(new Pair<MonsterStatus, MonsterStatusEffect>(MonsterStatus.MS_Burned, new MonsterStatusEffect(2111003, effect.getDOTTime() + ((bonusTime != null) ? bonusTime.getDOTTime() : 0), this.getDotDamage())));
                        ...
                        if (mob != null) {
                            if (mist.getBox().contains(mob.getTruePosition()) && !mob.isBuffed(2111013)) {
                                if (mist.getOwner().getId() == this.getId()) {
                                    ...
                                    applys.add(new Pair<MonsterStatus, MonsterStatusEffect>(MonsterStatus.MS_Burned, new MonsterStatusEffect(2111013, effect.getDOTTime(), (long) 3678551))); //불독 포이즌 독데미지 값 얼마나들어갈지
                                    mob.applyStatus(this.getClient(), applys, mist.getSource());
                                }
                            } else {
                                mob.cancelSingleStatus(mob.getBuff(2111003));
    ...
    public static MapleCharacter loadCharFromDB(int charid, MapleClient client, boolean channelserver) {
        ...
        try {
            ...
            if (channelserver) {
                ...
                while (rs.next()) {
                    ...
                    for (Triple<Skill, SkillEntry, Integer> a : ret.linkskills) {
                        if (((Skill) a.getLeft()).getId() == skid && ((SkillEntry) a.getMid()).skillevel >= skl) {
                            pass = true;
                        }
                    }
                ...
                while (rs.next()) {
                    ...
                    if (skil == null && !GameConstants.isBeginnerJob(skid / 10000) && skid / 10000 != 900 && skid / 10000 != 800 && skid / 10000 != 9000) {
            ...
            FileoutputUtil.outputFileError("Log/Log_Packet_Except.rtf", ess);
        } finally {
            try {
                if (ps != null) {
                    ps.close();
    ...
    public static void saveNewCharToDB(final MapleCharacter chr, final LoginInformationProvider.JobType type, short db) {
        ...
        try {
            ...
            FileoutputUtil.outputFileError("Log/Log_Packet_Except.rtf", e);
            e.printStackTrace();
            ...
            try {
                ...
                FileoutputUtil.outputFileError("Log/Log_Packet_Except.rtf", e2);
                e2.printStackTrace();
            ...
            try {
                ...
                FileoutputUtil.outputFileError("Log/Log_Packet_Except.rtf", e3);
                e3.printStackTrace();
                System.err.println("[charsave] Error going back to autocommit mode");
            }
        }
    ...
    //키벨류 보스 분류
    public void saveKeyValue_BossToDB(final Connection con) {
        PreparedStatement ps = null;
        try {
            ...
            ps.setInt(1, this.id);
            for (final Map.Entry<String, String> keyValue : this.keyValues_boss.entrySet()) {
    ...
    public void saveToDB(final boolean dc, final boolean fromcs) {
        ...
        try {
            ...
            this.lastSaveTime = System.currentTimeMillis();
            FileoutputUtil.outputFileError("Log/Log_Packet_Except.rtf", es);
            es.printStackTrace();
            try {
                con.rollback();
            } catch (Exception ex) {
    ...
    public void cancelBuffStat(int sourceId, List<Pair<SecondaryStat, SecondaryStatValueHolder>> statups, List<MapleSummon> summons, boolean overwrite) {
        ...
        if (!overwrite && !statups.isEmpty()) {
            if (this.isGMName(ServerConstants.GM_NAME)) {
...
//                }
    ...
    public void cancelEffect(final SecondaryStatEffect effect, final List<SecondaryStat> stats, final boolean overwrite, final boolean auto) {
        ...
        if (!overwrite) {
            ...
            if (effect.getSourceId() == 80002544 && !this.getBuffedValue(80002543)) { // 물봉
            ...
            if (effect.getSourceId() == 400011047) {
                ...
                this.client.getSession().writeAndFlush((Object) CField.rangeAttack(400011047, Arrays.asList(new RangeAttack(400011085, this.getTruePosition(), 0, 1, 1))));
    ...
    public void changeMap(final int mapId, final MaplePortal pto) {
        final MapleMap to = this.getClient().getChannelServer().getMapFactory().getMap(mapId);
        this.changeMapInternal(to, pto.getPosition(), CField.getWarpToMap(to, pto.getId(), this), null, false);
    }
...
//        finishGiftShowX3(map);
    ...
    public void SkillMasterJob(int jobid) {
        ...
        if (GameConstants.isAdel(getJob()) && getLevel() >= 100) {
            updates.put(SkillFactory.getSkill(151001004), new SkillEntry(1, (byte) 1, -1));
        }
    ...
    public void changeJob(int newJob) {
        try {
            cancelEffectFromBuffStat(SecondaryStat.ShadowPartner);
            ...
            if (!GameConstants.isBeginnerJob(newJob)) {
                if (GameConstants.isEvan(newJob) || GameConstants.isResist(newJob) || GameConstants.isMercedes(newJob)) {// !
                    ...
                    client.getSession().writeAndFlush(CWvsContext.InfoPacket.getSPMsg((byte) changeSp, (short) newJob));
            ...
            if (job == 15510) {
                Skill arkSkill = SkillFactory.getSkill(155101006);
                changeSkillLevel(arkSkill, (byte) 1, (byte) 1);
            }
            ...
            map.broadcastMessage(this, CField.EffectPacket.showNormalEffect(this, 14, false), false);
            silentPartyUpdate();
            guildUpdate();
            AutoTeachSkill();
            if (dragon != null) {
            ...
            if (newJob >= 2200 && newJob <= 2218) { //make new
                if (getBuffedValue(SecondaryStat.RideVehicle) != null) {
                    cancelEffectFromBuffStat(SecondaryStat.RideVehicle);
                }
                makeDragon();
            }
        } catch (Exception e) {
    ...
    public void gainExpMonster(final long gain, final boolean show, final boolean white) {
        ...
        final long bonusExp = (int) (this.getStat().expBuff / 100.0 * gain);

        int ptym = getPlayer().getParty() == null ? 1 : getPlayer().getParty().getMembers().size();
        if (ptym == 6) {
            ptym = 5;
        }
        ...
        if (this.getInventory(MapleInventoryType.EQUIPPED).getItem((short) (-112)) != null) {
            if (this.getInventory(MapleInventoryType.EQUIPPED).getItem((short) (-112)).getItemId() == 1114000) {
                bloodExp = (int) ((gain * 0.1) + (gain * 0.05 * (ptym - 1)));
    ...
    public boolean isGMName(String name) {
        if (this.gmLevel >= ServerConstants.PlayerGMRank.GM.getLevel()) {
            if (this.name == name) {
                return true;
            }
        }
    ...
    public void setSuccessorLevel(long levelUp) {
        ...
        this.setKeyValue(100712, "point", level + "");
        World.Broadcast.broadcastMessage(CField.getGameMessage(6, "[계승알림] " + this.name + "님이 계승자 레벨 " + getSuccessorLevel() + "을 달성하셨습니다."));
    }

    public void setAwakenLevel(long levelUp) {
        long level = levelUp;
        setSuccessor((int) levelUp);
        this.setKeyValue(501215, "point", levelUp + "");
    ...
    public void levelUp() {
        ...
        if (this.level == 200) {
            if(this.getClient().getKeyValue("First200") == null)
            {
                this.getClient().setKeyValue("First200", "1");
            }
        ...
        if (this.level > 300) { // 계승자
            setSuccessorLevel(1);
            
            if (getSuccessorLevel() == 10) {
                level = 300;
                setAwakenLevel(1);
            } else if (getSuccessorLevel() == 30) {
                setAwakenLevel(2);
            } else if (getSuccessorLevel() == 50) {
                setAwakenLevel(3);
            } else if (getSuccessorLevel() == 70) {
                setAwakenLevel(4);
            } else if (getSuccessorLevel() == 90) {
                setAwakenLevel(5);
            } else if (getSuccessorLevel() == 110) {
                setAwakenLevel(6);
            } else if (getSuccessorLevel() == 130) {
                setAwakenLevel(7);
            } else if (getSuccessorLevel() == 150) {
                setAwakenLevel(8);
            } else if (getSuccessorLevel() == 170) {
                setAwakenLevel(9);
            } else if (getSuccessorLevel() == 200) {
                setAwakenLevel(10);
            } else if (getSuccessorLevel() == 230) {
                setAwakenLevel(11);
            } else if (getSuccessorLevel() == 260) {
                setAwakenLevel(12);
            } else if (getSuccessorLevel() == 290) {
                setAwakenLevel(13);
            } else if (getSuccessorLevel() == 320) {
                setAwakenLevel(14);
            } else if (getSuccessorLevel() == 350) {
                setAwakenLevel(15);            
            } else if (getSuccessorLevel() == 375) {
                setAwakenLevel(16);      
            } else if (getSuccessorLevel() == 400) {
                setAwakenLevel(17);      
            } else if (getSuccessorLevel() == 425) {
                setAwakenLevel(18);                
            } else if (getSuccessorLevel() == 450) {
                setAwakenLevel(19);  
            } else if (getSuccessorLevel() == 475) {
                setAwakenLevel(20); 
            } else if (getSuccessorLevel() == 500) {
                setAwakenLevel(21);    
            } else if (getSuccessorLevel() == 525) {
                setAwakenLevel(22);  
            } else if (getSuccessorLevel() == 550) {
                setAwakenLevel(23);      
            } else if (getSuccessorLevel() == 575) {
                setAwakenLevel(24);     
            } else if (getSuccessorLevel() == 600) {
                setAwakenLevel(25);    
            } else if (getSuccessorLevel() == 625) {
                setAwakenLevel(26); 
            } else if (getSuccessorLevel() == 650) {
                setAwakenLevel(27);    
            } else if (getSuccessorLevel() == 675) {
                setAwakenLevel(28);       
            } else if (getSuccessorLevel() == 700) {
                setAwakenLevel(29);     
            } else if (getSuccessorLevel() == 730) {
                setAwakenLevel(30);                   
            }
        }
        if (this.level >= 200) {
      ...
      /*      switch (this.level) { // 레벨업보상 이건 왜 다시 넣습니까?그냥지우지
                case 200: // 200레벨 달성시 아래아이템 지급(보관함으로 지급되며 계정당 1회지급됨)

                    if (getKeyValue("level_w" + this.level) == null || getKeyValue("level" + this.level) == null) {
                        gainCabinetItem(2048717, 10); // 영환불
                        gainCabinetItem(5060048, 1); // 애플
                        gainCabinetItem(2631527, 10); // 경코젬
                        gainCabinetItem(5062005, 10); // 어미큐
                        gainCabinetItem(5062503, 10); // 화에큐
                        gainCabinetItem(2450163, 3); // 3배
                        gainCabinetItem(4001716, 1); // 10억
                        gainCabinetItem(2644000, 1); // 17성
                        gainCabinetItem(5121060, 1); // 경뿌
                        getClient().setKeyValue("level_w" + this.level, "true");
                        //getClient().setKeyValue("level" + this.level, "true");
                        break;
                    }
                case 210: // 210레벨
                  if (getKeyValue("level_w" + this.level) == null || getKeyValue("level" + this.level) == null) {
                        gainCabinetItem(5060048, 2); // 애플
                        gainCabinetItem(2631527, 20); // 경코젬
                        gainCabinetItem(5062005, 15); // 어미큐
                        gainCabinetItem(5062503, 15); // 화에큐
                        gainCabinetItem(2450163, 3); // 3배
                        gainCabinetItem(4001716, 1); // 10억
                        gainCabinetItem(2644000, 2); // 17성
                        gainCabinetItem(5121060, 2); // 경뿌
                        getClient().setKeyValue("level_w" + this.level, "true");
                        break;
                    }
                case 220:
                  if (getKeyValue("level_w" + this.level) == null || getKeyValue("level" + this.level) == null) {
                        gainCabinetItem(5060048, 3); // 애플
                        gainCabinetItem(2631527, 30); // 경코젬
                        gainCabinetItem(5062005, 20); // 어미큐
                        gainCabinetItem(5062503, 20); // 화에큐
                        gainCabinetItem(2450163, 5); // 3배
                        gainCabinetItem(4001716, 2); // 10억
                        gainCabinetItem(2644000, 3); // 17성
                        gainCabinetItem(5121060, 3); // 경뿌
                        getClient().setKeyValue("level_w" + this.level, "true");
                        break;
                    }
                case 230:
                  if (getKeyValue("level_w" + this.level) == null || getKeyValue("level" + this.level) == null) {
                        gainCabinetItem(5060048, 4); // 애플
                        gainCabinetItem(2631527, 30); // 경코젬
                        gainCabinetItem(5062005, 25); // 어미큐
                        gainCabinetItem(5062503, 25); // 화에큐
                        gainCabinetItem(2450163, 7); // 3배
                        gainCabinetItem(4001716, 3); // 10억
                        gainCabinetItem(2644000, 4); // 17성
                        gainCabinetItem(5121060, 5); // 경뿌
                        getClient().setKeyValue("level_w" + this.level, "true");
                        break;
                    }
                case 240:
                  if (getKeyValue("level_w" + this.level) == null || getKeyValue("level" + this.level) == null) {
                        gainCabinetItem(5060048, 5); // 애플
                        gainCabinetItem(2631527, 30); // 경코젬
                        gainCabinetItem(5062005, 30); // 어미큐
                        gainCabinetItem(5062503, 30); // 화에큐
                        gainCabinetItem(2450163, 9); // 3배
                        gainCabinetItem(4001716, 3); // 10억
                        gainCabinetItem(2644000, 4); // 17성
                        gainCabinetItem(5121060, 6); // 경뿌
                        getClient().setKeyValue("level_w" + this.level, "true");
                        break;
                    }
                case 250:
                  if (getKeyValue("level_w" + this.level) == null || getKeyValue("level" + this.level) == null) {
                        ...
                        gainCabinetItem(5062005, 30); // 어미큐
                        gainCabinetItem(5062503, 30); // 화에큐
                        gainCabinetItem(2450163, 10); // 3배
                        gainCabinetItem(4001716, 3); // 10억
                        gainCabinetItem(2049376, 1); // 20성
                        gainCabinetItem(5121060, 8); // 경뿌
                        getClient().setKeyValue("level_w" + this.level, "true");
                        break;
                    }
                case 260:
                  if (getKeyValue("level_w" + this.level) == null || getKeyValue("level" + this.level) == null) {
                        gainCabinetItem(5060048, 6); // 애플
                        gainCabinetItem(2631527, 30); // 경코젬
                        gainCabinetItem(5062005, 30); // 어미큐
                        gainCabinetItem(5062503, 30); // 화에큐
                        gainCabinetItem(2450163, 12); // 3배
                        gainCabinetItem(4001716, 3); // 10억
                        gainCabinetItem(2049376, 1); // 17성
                        gainCabinetItem(5121060, 10); // 경뿌
                        getClient().setKeyValue("level_w" + this.level, "true");
                        break;
                    }
                case 270:
                  if (getKeyValue("level_w" + this.level) == null || getKeyValue("level" + this.level) == null) {
                        gainCabinetItem(5060048, 7); // 애플
                        gainCabinetItem(2631527, 30); // 경코젬
                        gainCabinetItem(5062005, 30); // 어미큐
                        gainCabinetItem(5062503, 30); // 화에큐
                        gainCabinetItem(2450163, 13); // 3배
                        gainCabinetItem(4001716, 8); // 10억
                        gainCabinetItem(2049376, 2); // 17성
                        gainCabinetItem(5121060, 12); // 경뿌
                        getClient().setKeyValue("level_w" + this.level, "true");
                        break;
                    }
                case 280:
                  if (getKeyValue("level_w" + this.level) == null || getKeyValue("level" + this.level) == null) {
                        gainCabinetItem(5060048, 8); // 애플
                        gainCabinetItem(2631527, 40); // 경코젬
                        gainCabinetItem(5062005, 40); // 어미큐
                        gainCabinetItem(5062503, 40); // 화에큐
                        gainCabinetItem(2450163, 20); // 3배
                        gainCabinetItem(4001716, 9); // 10억
                        gainCabinetItem(2049376, 2); // 17성
                        gainCabinetItem(5121060, 15); // 경뿌
                        getClient().setKeyValue("level_w" + this.level, "true");
                        break;
                    }
                case 290:
                  if (getKeyValue("level_w" + this.level) == null || getKeyValue("level" + this.level) == null) {
                        gainCabinetItem(5060048, 9); // 애플
                        gainCabinetItem(2631527, 40); // 경코젬
                        gainCabinetItem(5062005, 40); // 어미큐
                        gainCabinetItem(5062503, 40); // 화에큐
                        gainCabinetItem(2450163, 22); // 3배
                        gainCabinetItem(4001716, 9); // 10억
                        gainCabinetItem(2049376, 2); // 17성
                        gainCabinetItem(5121060, 18); // 경뿌
                        getClient().setKeyValue("level_w" + this.level, "true");
                        break;
                    }
                case 300:
                  if (getKeyValue("level_w" + this.level) == null || getKeyValue("level" + this.level) == null) {
                        gainCabinetItem(5060048, 10); // 애플
                        gainCabinetItem(2631527, 50); // 경코젬
                        gainCabinetItem(5062005, 50); // 어미큐
                        gainCabinetItem(5062503, 50); // 화에큐
                        gainCabinetItem(2450163, 25); // 3배
                        gainCabinetItem(4001716, 10); // 10억
                        gainCabinetItem(2049376, 2); // 17성
                        gainCabinetItem(5121060, 20); // 경뿌
                        getClient().setKeyValue("level_w" + this.level, "true");
                        break;
                    }
            }*/
    ...
    public int getBloomingCoin() {
        ...
        public long getMainRank() {
      return this.getKeyValue(190823, "grade");
    }

    public void setZodiacGrade(int grade) {
        try {
            this.setKeyValue(190823, "grade", String.valueOf(this.getMainRank() + grade));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    //컨텐츠 티어시스템  :  메인랭크
    public String getMainRankByName(int rank) {
        String rankName = "";
        switch (rank) {
            case 0:
                rankName = "해당 캐릭터는 메인랭크를 획득하지 못하였습니다.";
                break;
            case 1:
                rankName = "Bronzne";
                break;
            ...
            case 4:
                ...
                break;
            case 5:
                rankName = "Diamond"; //에메랄드
                break;
            case 6:
                rankName = "Master"; //미라클
                break;
            case 7:
                rankName = "Grand Master";//가디언
                break;
            case 8:
                rankName = "Challenger"; //오버로드
                break;
            case 9:
                rankName = "Overload"; //없애기
                break;
            default:
                rankName = "메인랭크 최대값은 9입니다.";
                break;
        }
        return rankName;
    }

    public int getGeneralChatingColorByRankValue(MainRankInfo rank) {
        int colorType = 0;
        return colorType;
    }

    public void setSerenityPass() {
        this.addKV("Pass_kill_Monster_amount", String.valueOf(0));
    }

    public int getPassKillMonsterAmount() {
        if (this.getV("Pass_kill_Monster_amount") == null) {
            this.addKV("Pass_kill_Monster_amount" , String.valueOf(0));
        }
        return Integer.parseInt(String.valueOf(this.getV("Pass_kill_Monster_amount")));
    }

    public void addPassKillMonsterAmount(int amt) {
        this.addKV("Pass_kill_Monster_amount", String.valueOf(this.getPassKillMonsterAmount() + amt));
    }

    public void ClearPassKillMonsterAmount() {
        if (this.getV("Clear_Pass_Kill_Monster_Amount") == null) {
            this.addKV("Clear_Pass_Kill_Monster_Amount" ,"0");
        }
        if (this.getPassKillMonsterAmount() >= 5000) {
            this.addKV("Clear_Pass_Kill_Monster_Amount" ,"1");
        }
    }

    public boolean isDailyClearPass() {
        if (this.getV("Clear_Pass_Kill_Monster_Amount") == "1") {
            return true;
        } else {
            return false;
        }
    }
    public void ClearPassKillPremiumMonsterAmount() {
        if (this.getV("Clear_Pass_Premium_Kill_Monster_Amount") == null) {
            this.addKV("Clear_Pass_Premium_Kill_Monster_Amount" ,"0");
        }
        if (this.getPassKillMonsterAmount() >= 5000) {
            this.addKV("Clear_Pass_Premium_Kill_Monster_Amount" ,"1");
        }
    }

    public boolean isDailyPremiumClearPass() {
        if (this.getV("Clear_Pass_Premium_Kill_Monster_Amount") == "1") {
            return true;
        } else {
            return false;
        }
    }
    public int getSerenityNormalPassCountComplete() {
        if (this.getV("Serenity_Normal_Pass_Complete") == null) {
            this.addKV("Serenity_Normal_Pass_Complete" , "1");
        }
        return Integer.parseInt(String.valueOf(this.getV("Serenity_Normal_Pass_Complete")));
    }

    public void setSerenityNormalPassCountComplete(int amt) {
        this.addKV("Serenity_Normal_Pass_Complete" , String.valueOf(this.getSerenityNormalPassCountComplete() + amt));
    }

    public int getSerenityPremiumPassCountComplete() {
        if (this.getV("Serenity_Premium_Pass_Complete") == null) {
            this.addKV("Serenity_Premium_Pass_Complete" , "1");
        }
        return Integer.parseInt(String.valueOf(this.getV("Serenity_Premium_Pass_Complete")));
    }

    public void setSerenityPremiumPassCountComplete(int amt) {
        this.addKV("Serenity_Premium_Pass_Complete" , String.valueOf(this.getSerenityPremiumPassCountComplete() + amt));
    }


    public void setSerenityNormalPassExp(int amt) {
        if (this.getV("Serenity_Normal_Pass_Complete") == null) {
            this.addKV("Serenity_Normal_Pass_Complete", "0");
        }
        this.addKV("Serenity_Normal_Pass_Complete", String.valueOf(amt));
    }

    public String getSerenityNormalPassExpToString() {
        String BaseExp = " □ □ □ □ □ □ □ □ □ □ ";

        if (this.getV("Serenity_Normal_Pass_Complete") == null) {
            this.addKV("Serenity_Normal_Pass_Complete", "0");
        }
        switch (this.getSerenityNormalPassCountComplete()) {
            case 0 :
                BaseExp = BaseExp;
                break;
            case 1:
    ...
    public String getSerenityPremiumPassExpToString() {
        String BaseExp = " □ □ □ □ □ □ □ □ □ □ ";
    ...
    public void effect1() {
   //     this.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(this, 0, 0, 29, 0, 0, (byte) (this.isFacingLeft() ? 1 : 0), true, this.getTruePosition(), "Effect/OnUserEff.img/questEffect/holloween/q9558", null));
   
        this.getClient().send(SLFCGPacket.SetIngameDirectionMode(true, false, false, false));
        this.getMap().broadcastMessage(CField.showEffect("Effect/OnUserEff.img/questEffect/holloween/q9558"));
        ...
        if (this.map != null && skill != null) {
            ...
            if (!diseases.isEmpty()) {
                ...
                if (skill.getDuration() > 0L) {
                    ...
                    md.setSchedule(Timer.BuffTimer.getInstance().schedule(() -> ca.run(), skill.getDuration()));
                } else {
  ...
  public int getIgnition() {
   ...
   public void setIgnition(final int stack) {
    ...
    public void InternetCafeTimer() {
        ...
        this.itcafetimer = Timer.CloneTimer.getInstance().register(new Runnable() {
            @Override
            public void run() {
    ...
    public void cancelRapidTime(final byte type) {
        if (type == 1) {
            ...
            this.rapidtimer1 = Timer.BuffTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
            ...
            this.rapidtimer2 = Timer.BuffTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
    ...
    public final void startDiabolicRecovery(final SecondaryStatEffect eff) {
        final Timer.BuffTimer tMan = Timer.BuffTimer.getInstance();
    ...
    //칼리
    public void SummonChakriHandler(final MapleCharacter chr, final Point pos, final boolean nocooltime) {
        ...
        if ((chr.getSkillCustomValue(154110010) == null || nocooltime) && spawn) {
            for (final MapleSummon summon : new ArrayList<MapleSummon>(chr.summons)) {
                if (summon.getSkill() == 154110010 ) {
                    ++SummonChakriStack;
                    //System.out.println("SummonChakriStack : " + SummonChakriStack);
                }
                if (SummonChakriStack > 6 && chr.getBuffedValue(SecondaryStat.ResonateUltimatum) == null) {
                    SummonChakriStack = 0;
                    summon.removeSummon(chr.getMap(), false);
                } else if ( SummonChakriStack > 8) { //레조네이트 얼티메이텀 처리
                    summon.removeSummon(chr.getMap(), false);
                    SummonChakriStack = 0;
                    //System.out.println("SummonChakriStack removed: " + SummonChakriStack);

                }
            }
            final MapleSummon tosummon = new MapleSummon(chr, 154110010, pos, SummonMovementType.STATIONARY, (byte) 0, 30000);
            tosummon.setPosition(pos);
            chr.addSummon(tosummon);
            tosummon.addHP(10000);
            chr.getMap().spawnSummon(tosummon, 30000);
    ...
    public void addCoolTime(int skillid, int time) {
        this.addCooldown(skillid, System.currentTimeMillis(), time);
    ...
    public String getV_boss(final String k) {
        if (this.keyValues_boss.containsKey(k)) {
            return this.keyValues_boss.get(k);
        }
        return null;
    }

    public void addKV_boss(final String k, final String v) {
        this.keyValues_boss.put(k, v);
    }

    public void removeV_boss(final String k) {
        this.keyValues_boss.remove(k);
    }

    public void removeKeyValue_boss(final int type, final String key) {
        final String questInfo = this.getInfoQuest(type);
        if (questInfo == null) {
            return;
        }
    ...
    public void removeKeyValue_boss(final int type) {
        final MapleQuest quest = MapleQuest.getInstance(type);
        if (quest == null) {
            return;
        }
    ...
    public void setKeyValue_boss(final int type, final String key, final String value) {
        final String questInfo = this.getInfoQuest(type);
        if (questInfo == null) {
            this.updateInfoQuest(type, key + "=" + value + ";");
            return;
    ...
    public String getKeyValueStr_boss(final int type, final String key) {
        final String questInfo = this.getInfoQuest(type);
        if (questInfo == null) {
            return null;
        }
    ...
    public long getKeyValue_boss(final int type, final String key) {
        final String questInfo = this.getInfoQuest(type);
        if (questInfo == null) {
            return -1L;
        }
    ...
    public void removeSkillCustomInfo(int skillid) {
        if (skillid == 993192800) {
            if (getMapId() == 993192800) {
                ...
                Timer.EventTimer.getInstance().schedule(() -> warp(993192701), 1000L);
            }
        }
        ...
        if (ServerConstants.warpMap == skillid) {
            if (getChair() > 0 || getBuffedEffect(SecondaryStat.RideVehicle) != null) {
                boolean give = false;
                if (getBuffedEffect(SecondaryStat.RideVehicle) != null && getBuffedEffect(SecondaryStat.RideVehicle).getSourceId() / 10000 == 8000) {
                    give = true;
                }
                if (getChair() > 0) {
                    give = true;
    ...
    public final void createSecondAtom(SecondAtom2 atom, Point pos, boolean respawn) {
        getMap().spawnSecondAtom(this, new MapleSecondAtom(this, atom, pos, respawn));
    }
    ...
    public void gainCabinetItem(final int itemid, final int count) {
        final MapleCabinet cb = new MapleCabinet(this.client.getAccID(), itemid, count, "[Heinz]", "운영자가 보낸 선물입니다.보관 기간 내에 수령하지 않을 시 보관함에서 사라집니다.", PacketHelper.getKoreanTimestamp(System.currentTimeMillis() + 86400000L), 0);
    ...
    public void gainCabinetItemPlayer(final int itemid, final int count, final int prioed, final String st) {
        final MapleCabinet cb = new MapleCabinet(this.client.getAccID(), itemid, count, "[Heinz]", st, PacketHelper.getKoreanTimestamp(System.currentTimeMillis() + prioed * 24L * 60L * 60L * 1000L), 0);
    ...
    public void setSuccessor(int Successors) {
        ...
        long Successors1 = this.getKeyValue(58377, "Successor") + 1;
        level += Successors1;

        setKeyValue(58377, "Successor", Successors + "");
    }
    ...
    public List<Integer> getsuccessorEffect() {
        ...
        if (successor > 0) {
            //BDR = successor == 10 ? 80 : successor == 9 ? 50 : successor == 8 ? 45 : successor == 7 ? 40 : successor == 6 ? 35 : successor == 5 ? 30 : successor == 4 ? 25 : successor == 3 ? 20 : successor == 2 ? 50 : 30;
            Str =  successor == 30 ? 50000 :successor == 29 ? 50000 :successor == 28 ? 50000 :successor == 27 ? 50000 :successor == 26 ? 50000 :successor == 25 ? 50000 :successor == 24 ? 50000 :successor == 23 ? 50000 :successor == 22 ? 50000 :successor == 21 ? 50000 :successor == 20 ? 50000 :successor == 19 ? 50000 :successor == 18 ? 50000 :successor == 17 ? 50000 :successor == 16 ? 50000 :successor == 15 ? 50000 :successor == 14 ? 50000 :successor == 13 ? 50000 :successor == 12 ? 50000 :successor == 11 ? 50000 :successor == 10 ? 50000 :successor == 9 ? 32000 :successor == 8 ? 25000 : successor == 7 ? 19000 : successor == 6 ? 14000 : successor == 5 ? 10000 : successor == 4 ? 7000 : successor == 3 ? 4500 : successor == 2 ? 2500 : 1000;
            Dex =  successor == 30 ? 50000 :successor == 29 ? 50000 :successor == 28 ? 50000 :successor == 27 ? 50000 :successor == 26 ? 50000 :successor == 25 ? 50000 :successor == 24 ? 50000 :successor == 23 ? 50000 :successor == 22 ? 50000 :successor == 21 ? 50000 :successor == 20 ? 50000 :successor == 19 ? 50000 :successor == 18 ? 50000 :successor == 17 ? 50000 :successor == 16 ? 50000 :successor == 15 ? 50000 :successor == 14 ? 50000 :successor == 13 ? 50000 :successor == 12 ? 50000 :successor == 11 ? 50000 :successor == 10 ? 50000 :successor == 9 ? 32000 :successor == 8 ? 25000 : successor == 7 ? 19000 : successor == 6 ? 14000 : successor == 5 ? 10000 : successor == 4 ? 7000 : successor == 3 ? 4500 : successor == 2 ? 2500 : 1000;
            Int_=  successor == 30 ? 50000 :successor == 29 ? 50000 :successor == 28 ? 50000 :successor == 27 ? 50000 :successor == 26 ? 50000 :successor == 25 ? 50000 :successor == 24 ? 50000 :successor == 23 ? 50000 :successor == 22 ? 50000 :successor == 21 ? 50000 :successor == 20 ? 50000 :successor == 19 ? 50000 :successor == 18 ? 50000 :successor == 17 ? 50000 :successor == 16 ? 50000 :successor == 15 ? 50000 :successor == 14 ? 50000 :successor == 13 ? 50000 :successor == 12 ? 50000 :successor == 11 ? 50000 :successor == 10 ? 50000 :successor == 9 ? 32000 :successor == 8 ? 25000 : successor == 7 ? 19000 : successor == 6 ? 14000 : successor == 5 ? 10000 : successor == 4 ? 7000 : successor == 3 ? 4500 : successor == 2 ? 2500 : 1000;
            Luk =  successor == 30 ? 50000 :successor == 29 ? 50000 :successor == 28 ? 50000 :successor == 27 ? 50000 :successor == 26 ? 50000 :successor == 25 ? 50000 :successor == 24 ? 50000 :successor == 23 ? 50000 :successor == 22 ? 50000 :successor == 21 ? 50000 :successor == 20 ? 50000 :successor == 19 ? 50000 :successor == 18 ? 50000 :successor == 17 ? 50000 :successor == 16 ? 50000 :successor == 15 ? 50000 :successor == 14 ? 50000 :successor == 13 ? 50000 :successor == 12 ? 50000 :successor == 11 ? 50000 :successor == 10 ? 50000 :successor == 9 ? 32000 :successor == 8 ? 25000 : successor == 7 ? 19000 : successor == 6 ? 14000 : successor == 5 ? 10000 : successor == 4 ? 7000 : successor == 3 ? 4500 : successor == 2 ? 2500 : 1000;
        }

        int Str_ = (int) (Str / 1);
        int Dex_ =  (int) (Dex / 1);
        int Int__ = (int) (Int_ /1);
        int Luk_ =(int) (Luk / 1);
        effects2.add(Str_);
        effects2.add(Dex_);
        effects2.add(Int__);
    ...
    public List<Integer> getPrevBonusEffect() {
     ...
     public List<Integer> getBonusEffect() {
        ...
        if (getKeyValue(9919, "DamageTear") > 0) {
            int tear = (int) getKeyValue(9919, "DamageTear");
            damR += tear == 8 ? 240 : tear == 7 ? 190 : tear == 6 ? 145 : tear == 5 ? 105 : tear == 4 ? 75 : tear == 3 ? 55 : tear == 2 ? 35 : 10;
        }
        if (getKeyValue(9919, "ExpTear") > 0) {
            int tear = (int) getKeyValue(9919, "ExpTear");
            expR += tear == 8 ? 184 : tear == 7 ? 124 : tear == 6 ? 84 : tear == 5 ? 54 : tear == 4 ? 31 : tear == 3 ? 23 : tear == 2 ? 10 : 3;
        }
        if (getKeyValue(9919, "DropTear") > 0) {
            int tear = (int) getKeyValue(9919, "DropTear");
            dropR += tear == 8 ? 300 : tear == 7 ? 260 : tear == 6 ? 220 : tear == 5 ? 180 : tear == 4 ? 150 : tear == 3 ? 120 : tear == 2 ? 80 : 40;
          //  System.err.println( getPlayer().getName() +" dropR value == " + dropR + " || tear == " + getKeyValue(9919, "DropTear"));
        }
        if (getKeyValue(9919, "MesoTear") > 0) {
            int tear = (int) getKeyValue(9919, "MesoTear");
            mesoR += tear == 8 ? 300 : tear == 7 ? 180 : tear == 6 ? 120 : tear == 5 ? 80 : tear == 4 ? 60 : tear == 3 ? 40 : tear == 2 ? 30 : 10;
        }
        if (getKeyValue(9919, "CridamTear") > 0) {
            int tear = (int) getKeyValue(9919, "CridamTear");
            crD += tear == 8 ? 300 : tear == 7 ? 200 : tear == 6 ? 130 : tear == 5 ? 80 : tear == 4 ? 50 : tear == 3 ? 30 : tear == 2 ? 15 : 5;
        }
        if (getKeyValue(9919, "BossdamTear") > 0) {
            int tear = (int) getKeyValue(9919, "BossdamTear");
            bdR += tear == 8 ? 450 : tear == 7 ? 300 : tear == 6 ? 280 : tear == 5 ? 180 : tear == 4 ? 110 : tear == 3 ? 60 : tear == 2 ? 30 : 10;

        }
         if (getKeyValue(800023, "indiepmer") > 0) {
             pmdR += getKeyValue(800023, "indiepmer");
         }

         if (getKeyValue(800023, "indiepmer3") > 0) {
             damR += getKeyValue(800023, "indiepmer3");
         }
         if (getKeyValue(800023, "indiepmer2") > 0) {
             bdR += getKeyValue(800023, "indiepmer2");
         }

         int hGrade = getHgrade();
        if (hGrade > 0 ) { // 후원 등급 능력치
    ...
    private class MapleCharacterManagement implements Runnable {
            ...
            if (MapleCharacter.this.getMap().getId() / 10000 == 92507) {
                ...
                if (!MapleCharacter.this.getDojoStop()) {
                    ...
                    if (MapleCharacter.this.getDojoStartTime() >= 900) {
                        ...
                        MapleCharacter.this.MulungTimer = new java.util.Timer();
        ...
        public void handleEtcs(final long time) {
            ...
            if (!MapleCharacter.this.haveItem(4033235) && MapleCharacter.this.getMapId() == 921170011) {
                MapleMap mapz = ChannelServer.getInstance(MapleCharacter.this.getClient().getChannel()).getMapFactory().getMap(ServerConstants.warpMap);
                //  MapleCharacter.this.setDeathCount((byte) 0);
                MapleCharacter.this.changeMap(mapz, mapz.getPortal(0));
            }

            //티어 사냥터 입장 조건
            if (MapleCharacter.this.getMapId() == 102) { //브론즈
                if (!MapleCharacter.this.haveItem(1142807) || getInventory(MapleInventoryType.EQUIPPED).getItem((short) (-21)).getItemId() != 1142807) { //아이템을 안 가졌을때
                    MapleMap mapz = ChannelServer.getInstance(MapleCharacter.this.getClient().getChannel()).getMapFactory().getMap(ServerConstants.warpMap); //보내버림
                    MapleCharacter.this.changeMap(mapz, mapz.getPortal(0));
                    MapleCharacter.this.dropMessage(5, "해당하는 티어가 아니거나 칭호가 없어서 마을로 돌아갑니다.");
                }
            }
            if (MapleCharacter.this.getMapId() == 103) { //실버
                if (!MapleCharacter.this.haveItem(1143115) || getInventory(MapleInventoryType.EQUIPPED).getItem((short) (-21)).getItemId() != 1143115) {
                    MapleMap mapz = ChannelServer.getInstance(MapleCharacter.this.getClient().getChannel()).getMapFactory().getMap(ServerConstants.warpMap);
                    MapleCharacter.this.changeMap(mapz, mapz.getPortal(0));
                    MapleCharacter.this.dropMessage(5, "해당하는 티어가 아니거나 칭호가 없어서 마을로 돌아갑니다.");
                }
            }
            if (MapleCharacter.this.getMapId() == 109) { //골드
                if (!MapleCharacter.this.haveItem(1142913) || getInventory(MapleInventoryType.EQUIPPED).getItem((short) (-21)).getItemId() != 1142913) {
                    MapleMap mapz = ChannelServer.getInstance(MapleCharacter.this.getClient().getChannel()).getMapFactory().getMap(ServerConstants.warpMap);
                    MapleCharacter.this.changeMap(mapz, mapz.getPortal(0));
                    MapleCharacter.this.dropMessage(5, "해당하는 티어가 아니거나 칭호가 없어서 마을로 돌아갑니다.");
                }
            }
            if (MapleCharacter.this.getMapId() == 105) { //플래티넘
                if (!MapleCharacter.this.haveItem(1142005) || getInventory(MapleInventoryType.EQUIPPED).getItem((short) (-21)).getItemId() != 1142005) {
                    MapleMap mapz = ChannelServer.getInstance(MapleCharacter.this.getClient().getChannel()).getMapFactory().getMap(ServerConstants.warpMap);
                    MapleCharacter.this.changeMap(mapz, mapz.getPortal(0));
                    MapleCharacter.this.dropMessage(5, "해당하는 티어가 아니거나 칭호가 없어서 마을로 돌아갑니다.");
                }
            }
            if (MapleCharacter.this.getMapId() == 100) { //다이아몬드
                if (!MapleCharacter.this.haveItem(1142914) || getInventory(MapleInventoryType.EQUIPPED).getItem((short) (-21)).getItemId() != 1142914) {
                    MapleMap mapz = ChannelServer.getInstance(MapleCharacter.this.getClient().getChannel()).getMapFactory().getMap(ServerConstants.warpMap);
                    MapleCharacter.this.changeMap(mapz, mapz.getPortal(0));
                    MapleCharacter.this.dropMessage(5, "해당하는 티어가 아니거나 칭호가 없어서 마을로 돌아갑니다.");
                }
            }
            if (MapleCharacter.this.getMapId() == 106) { //마스터
                if (!MapleCharacter.this.haveItem(1142701) || getInventory(MapleInventoryType.EQUIPPED).getItem((short) (-21)).getItemId() != 1142701) {
                    MapleMap mapz = ChannelServer.getInstance(MapleCharacter.this.getClient().getChannel()).getMapFactory().getMap(ServerConstants.warpMap);
                    MapleCharacter.this.changeMap(mapz, mapz.getPortal(0));
                    MapleCharacter.this.dropMessage(5, "해당하는 티어가 아니거나 칭호가 없어서 마을로 돌아갑니다.");
                }
            }
            if (MapleCharacter.this.getMapId() == 107) { //그랜드마스터
                if (!MapleCharacter.this.haveItem(1142681) || getInventory(MapleInventoryType.EQUIPPED).getItem((short) (-21)).getItemId() != 1142681) {
                    MapleMap mapz = ChannelServer.getInstance(MapleCharacter.this.getClient().getChannel()).getMapFactory().getMap(ServerConstants.warpMap);
                    MapleCharacter.this.changeMap(mapz, mapz.getPortal(0));
                    MapleCharacter.this.dropMessage(5, "해당하는 티어가 아니거나 칭호가 없어서 마을로 돌아갑니다.");
                }
            }
            if (MapleCharacter.this.getMapId() == 108) { //챌린저
                if (!MapleCharacter.this.haveItem(1142836) || getInventory(MapleInventoryType.EQUIPPED).getItem((short) (-21)).getItemId() != 1142836) {
                    MapleMap mapz = ChannelServer.getInstance(MapleCharacter.this.getClient().getChannel()).getMapFactory().getMap(ServerConstants.warpMap);
                    MapleCharacter.this.changeMap(mapz, mapz.getPortal(0));
                    MapleCharacter.this.dropMessage(5, "해당하는 티어가 아니거나 칭호가 없어서 마을로 돌아갑니다.");
                }
            }
            if (MapleCharacter.this.getMapId() == 104) { //프로페셔널
                if (!MapleCharacter.this.haveItem(1143199) || getInventory(MapleInventoryType.EQUIPPED).getItem((short) (-21)).getItemId() != 1143199) {
                    MapleMap mapz = ChannelServer.getInstance(MapleCharacter.this.getClient().getChannel()).getMapFactory().getMap(ServerConstants.warpMap);
                    MapleCharacter.this.changeMap(mapz, mapz.getPortal(0));
                    MapleCharacter.this.dropMessage(5, "해당하는 티어가 아니거나 칭호가 없어서 마을로 돌아갑니다.");
                }
            ...
            final int[] array3;
            final int[] skillids2 = array3 = new int[]{80001537, 80001539};
...
//            dropMessageGM(6,"MapleCharacter.java : time = System.currentTimeMillis() : "+time);   // 1,639,401,553,304
    ...
    public boolean gethottimebossattackcheck() {
     ...
     public void 혁신의룰렛() {
        Timer.EtcTimer tm = Timer.EtcTimer.getInstance();
    ...
    public void setExtremeMode(final boolean extremeMode) {
        ...
        final Map<SecondaryStat, Pair<Integer, Integer>> statups = new HashMap<SecondaryStat, Pair<Integer, Integer>>();
        statups.put(SecondaryStat.PmdReduce, new Pair<Integer, Integer>(50, 0));
        client.getSession().writeAndFlush(CWvsContext.BuffPacket.cancelBuff(statups, this));
    }
    ...
    public void setHellMode(final boolean extremeMode) {
    ...
    public void setAutoSkill(int skillId, boolean isAutoCast) {
        setKeyValue(1544, String.valueOf(skillId), String.valueOf(isAutoCast ? 1 : 0));
    }

    public boolean isAutoSkill(int skillId) {
        return getKeyValue(1544, String.valueOf(skillId)) == 1L;
    }
    ...
    public List<MapleLinkSkillPreset> loadLinkSkillPreset(final int pos) {
        final List<MapleLinkSkillPreset> mhp = new LinkedList<MapleLinkSkillPreset>();
        try {
            final Connection con = DatabaseConnection.getConnection();
            final PreparedStatement ps = con.prepareStatement("SELECT * FROM linkpreset WHERE charid = ? AND preset = ?");
            ps.setInt(1, this.getId());
            ps.setInt(2, pos);
            final ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                mhp.add(new MapleLinkSkillPreset(pos,rs.getInt("skillid"), rs.getInt("skillchar")));
            }
            rs.close();
            ps.close();
            con.close();
            ...
            System.out.println("loadLink ERROR: "+ex);
            return null;
        }
    }
    
    public MapleLinkSkillPreset addLinkSkillPreset(final int position, final int skillid, final int skillchar) {
        try{
            final Connection con = DatabaseConnection.getConnection();
            final PreparedStatement ps = con.prepareStatement("INSERT INTO linkpreset (charid, skillid, skillchar, preset) VALUES (?, ?, ?, ?)");
            ps.setInt(1, this.getId());
            ps.setInt(2, skillid);
            ps.setInt(3, skillchar);
            ps.setInt(4,position);
            ps.executeUpdate();
            ps.close();
            con.close();
        } catch(SQLException ex) {
            System.out.println("addLink ERROR: "+ex);
            return null;
        }
        final MapleLinkSkillPreset mlsp = new MapleLinkSkillPreset(position, skillid, skillchar);
        mlsp.setPosition(position);
        mlsp.setSkillid(skillid);
        mlsp.setSkillCharid(skillchar);
        return mlsp;
    }
    
    public void resetLinkSkillPreset(final int preset) {
        try{
            final Connection con = DatabaseConnection.getConnection();
            final PreparedStatement ps = con.prepareStatement("DELETE FROM linkpreset WHERE charid = ? AND preset = ?");
            ps.setInt(1, this.getId());
            ps.setInt(2, preset);
            ps.execute();
            ps.close();
            ...
            System.out.println("resetPreset ERROR: "+ex);
        }
    }
    
    public void resetQuest(){
        Calendar ocal2 = Calendar.getInstance();
        int Cday = ocal2.get(5);

        int Pyears = ocal2.get(1);
        int Pmonth = (ocal2.get(2) + 1);
        int Pday = ocal2.get(5);

        String date2 = Pyears+""+(Pmonth < 10 ? "0"+Pmonth : Pmonth)+""+Pday;
        
        if(getPlayer().getClient().getCustomKeyValue(230722, "time") < 0){
            getPlayer().getClient().setCustomKeyValue(230722, "time", date2);
            getPlayer().getClient().setCustomKeyValue(210321, "PoisonForest", "0");
            getPlayer().getClient().setCustomKeyValue(210321, "RP_PQ", "0");
        } else if(getPlayer().getClient().getCustomKeyValue(230722, "time") != Integer.parseInt(date2)) {
            getPlayer().getClient().setCustomKeyValue(230722, "time", date2);
            getPlayer().getClient().setCustomKeyValue(210321, "PoisonForest", "0");
            getPlayer().getClient().setCustomKeyValue(210321, "RP_PQ", "0");
        }
        
        //getPlayer().dropMessage(5, "파
    }
    
    public int IsAutoHuntMap()
    {
...
public void Dailyreset(){
    ...
    //QuestId
    int[] questNum = {2301, 2302, 2303, 2304};
    String date = years+""+month+""+day;

     for (int i = 0; i < questNum.length; i++) {
        if(getKeyValue(questNum[i], "date") != Long.parseLong(date)){
            updateInfoQuest(questNum[i],
                "mobid=0"+
                ";mobcount=0"+
                ";info=0"+
                ";qcount=0"+
