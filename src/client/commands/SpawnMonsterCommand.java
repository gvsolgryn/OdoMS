...
public class SpawnMonsterCommand implements Command {
    ...
    public void execute(MapleClient c, String[] splitted) throws Exception, IllegalCommandSyntaxException {
        ...
        for (int i = 0; i < num; i++) {
            ...
            for (Pair<Integer, Integer> ms : mob.getSkills()) {
                c.getPlayer().dropMessage(6, ms.getLeft() + ":" + ms.getRight());
            }
        }
    }
    ...
    public CommandDefinition[] getDefinition() {
        return new CommandDefinition[]{new CommandDefinition("스폰",
            "<몬스터ID> (<hp HP설정>) (<exp 경험치설정>) (<php 퍼센트HP설정>) (<pexp 퍼센트경험치설정>)", "해당 고유번호ID의 몬스터를 소환합니다.", 6)};
    }
}
