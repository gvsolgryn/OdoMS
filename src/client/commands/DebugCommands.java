...
public class DebugCommands implements Command {
    ...
    public void execute(MapleClient c, String[] splitted) throws Exception, IllegalCommandSyntaxException {
        ...
        if (splitted[0].equals("!퀘스트초기화")) {
        ...
        } else if (splitted[0].equals("!퀘스트완료")) {
            MapleQuest.getInstance(Integer.parseInt(splitted[1])).forceComplete(c.getPlayer(), 0);
            ...
            c.getPlayer().dropMessage(6,
                    portal.getName() + " 포탈아이디: " + portal.getId() + " 스크립트: " + portal.getScriptName());
            ...
            c.getPlayer().dropMessage(6,
                    "확성기 사용가능 상태 : " + (c.getChannelServer().getMegaphoneMuteState() ? "불가능" : "가능"));
            ...
            for (MapleMapObject reactorL : reactors) {
                ...
                c.getPlayer().dropMessage(6,
                        "리액터: 오브젝트id: " + reactor2l.getObjectId() + " 리액터ID: " + reactor2l.getReactorId() + " 위치: "
                        + reactor2l.getPosition().toString() + " 상태: " + reactor2l.getState());
            }
            ...
            List<MapleMapObject> items = c.getPlayer().getMap().getMapObjectsInRange(c.getPlayer().getPosition(),
            ...
            for (MapleMapObject i : items) {
                ...
                c.getPlayer().getMap().broadcastMessage(MainPacketCreator.removeItemFromMap(i.getObjectId(), 0, 0),
                        i.getPosition());
            }
            ...
            for (MaplePortal mp : map.getPortals()) {
                c.getPlayer().Message(7, mp.getId() + " : " + mp.getName() + " 스크립트 : " + mp.getScriptName() + " 목적지 : "
                        + mp.getTarget());
            }
    ...
    public CommandDefinition[] getDefinition() {
        return new CommandDefinition[]{new CommandDefinition("모두튕", "", "현재 채널의 모든 플레이어를 강제로 접속종료합니다.", 6),
            new CommandDefinition("드롭삭제", "", "현재 맵의 모든 아이템을 삭제합니다.", 4),
            new CommandDefinition("퀘스트초기화", "", "현재 받아있는 퀘스트를 모두 지워버립니다.", 4),
            new CommandDefinition("퀘스트시작", "", "현재 받아있는 퀘스트를 모두 지워버립니다.", 5),
            new CommandDefinition("퀘스트완료", "", "현재 받아있는 퀘스트를 모두 지워버립니다.", 5),
            ...
            new CommandDefinition("드롭토글", "", "현재 맵에서 몬스터의 드롭가능상태를 변경합니다.", 4),
            ...
            new CommandDefinition("현재맵엔피시", "", "현재맵에 있는 엔피시의 목록을 모두 출력합니다.", 6),
            new CommandDefinition("현재맵포탈", "", "현재맵에 있는 포탈의 목록을 모두 출력합니다.", 6),
