...
public class BanningCommands implements Command {
    ...
    public CommandDefinition[] getDefinition() {
        return new CommandDefinition[]{
            new CommandDefinition("밴", "<캐릭터이름> <이유>", "해당 ip와 mac주소, 계정을 영구적으로 밴 시킵니다.", 1),
            new CommandDefinition("밴풀기", "<캐릭터이름>", "밴 된 ip와 mac주소, 계정의 밴을 해제합니다.", 1),
            new CommandDefinition("기간밴", "<캐릭터이름> <이유> <밴 될 일수>", "해당 계정을 해당 일 수 동안 밴 시킵니다.", 1),
            new CommandDefinition("접속끊기", "[-f] <캐릭터이름>", "해당 캐릭터를 강제로 접속종료시킵니다. 현접에걸렸다면 -f 옵션을 사용하세요.", 1)};
    }
}
