...
public class CommandProcessor {
    ...
    private CommandProcessor() {
        ...
        registerCommand(new HelpCommand()); // register the helpcommand first so it appears first in the list
        // (LinkedHashMap)
    ...
    private boolean processCommandInternal(MapleClient c, String line) {
        if (line.charAt(0) == '!' || line.charAt(0) == '@') {
            if (line.charAt(0) == '!') {
                /*if (!ServerConstants.isAdminIp(c.getIp()))
				{
					return false;
				}*/
            }
            ...
            if (splitted.length > 0 && splitted[0].length() > 1) {
                ...
                if (definitionCommandPair != null
                    ...
                    if (line.charAt(0) == '!' && definitionCommandPair.getDefinition().getRequiredLevel() != 1) {
                        /*if (!ServerConstants.isAdminIp(c.getIp()))
						{
							return false;
						}*/
                    ...
                    try {
                        if (line.charAt(0) == '!') {
                            ...
                            LoggerChatting.writeLog(LoggerChatting.commandLog,
                                    LoggerChatting.getChatLogType("GM 명령어", c.getPlayer(), line));
                        } else {
                            definitionCommandPair.getCommand().execute(c, splitted);
                            LoggerChatting.writeLog(LoggerChatting.commandLog2,
                                    LoggerChatting.getChatLogType("유저 명령어", c.getPlayer(), line));
                        }
