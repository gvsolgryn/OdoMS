...
import static tools.MapleCustomEncryption.encryptData;

/**
 *
 * @author main
 */
public class CustomPacketCreator {
    ...
    public static byte[] shuffleIV(byte inputByte, byte[] in) {
        in[0] += funnyBytes[in[1] & 0xFF] - (inputByte & 0xFF);
        in[1] -= in[2] ^ funnyBytes[inputByte & 0xFF];
        in[2] ^= (inputByte & 0xFF) + funnyBytes[in[3] & 0xFF];
