...
//dumpItems(item, psa, psr, ps, pse, false);    topdir=Cash, ifile=0501.img iz=Cash/0501.img의 데이터들
//dumpItems(character, psa, psr, ps, pse, true);
    ...
    public void dumpItem(PreparedStatement psa, PreparedStatement psr, PreparedStatement ps, PreparedStatement pse, MapleData iz) throws Exception {
        // if(MapleDataTool.getIntConvert("info/cash", iz , 0) > 0) {
        try {// iz에서 .img가 있으면 지우고, 없으면 int값으로 추출해서 id에 넣기
        ...
        if (smEntry == null) {
            if (GameConstants.getInventoryType(id) == MapleInventoryType.EQUIP) {
                ...
                if (GameConstants.isThrowingStar(id) || GameConstants.isBullet(id)) {
        ...
        if (id == 5150054 || GameConstants.isPet(id)) {
            ret = 1; //볼
        }
        if (GameConstants.isPet(id)) {
            ret = 1;//캐시창합치기 했을때 팅김방지
        }
        ps.setInt(5, ret);
        double pEntry = 0.0;
                ...
                switch (d.getName()) {
                    ...
                    case "boss":
                        for (MapleData subKey : d.getChildren()) {
                            ...
                            } else if (subKey.getName().equals("damage") || subKey.getName().equals("prob")) {
                                psa.setString(2, d.getName().equals("elemBoost") ? "elemboost" : d.getName());
                                psa.setInt(3, MapleDataTool.getInt("prob", d, 0));
                                psa.setInt(4, MapleDataTool.getInt("damage", d, 0));
                                psa.addBatch();
                            } else {
                                psa.setString(2, d.getName().equals("elemBoost") ? "elemboost" : d.getName());
                                psa.setString(3, "0");
    ...
    protected final MapleData getStringData(final int itemId) {
        ...
        if (itemId >= 5010000) {
            ...
            data = eqpStringData;
            cat = "Eqp/Accessory";
            /*} else if (itemId >= 1172000 && itemId < 2173000) {
            data = eqpStringData;   
            cat = "Eqp/MonsterBook";*/
 /*} else if (itemId >= 1662000 && itemId < 1680000) {
            ...
            data = eqpStringData;
            cat = "Eqp/Taming";
        } else if ((itemId >= 1300000 && itemId < 1610000) || (itemId >= 1700000 && itemId < 1800000)) { 
            data = eqpStringData;
            cat = "Eqp/Weapon";
