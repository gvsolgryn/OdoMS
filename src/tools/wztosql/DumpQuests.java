...
public class DumpQuests {
    ...
    public DumpQuests(boolean update) throws Exception {
        this.update = update;
        this.quest = MapleDataProviderFactory.getDataProvider(new File(ServerProperties.WZ_PATH + "Quest.wz"));
        if (quest == null) {
            hadError = true;
        }
    }
                    ...
                    for (MapleData req : reqData.getChildren()) {
                        ...
                        if (req.getName().equals("fieldEnter")) { //diff
                            ...
                            for (int x = 0; x < child.size(); x++) {
                                ...
                                string.append(MapleDataTool.getInt(child.get(x), -1));
                            }
                            psr.setString(4, string.toString());
