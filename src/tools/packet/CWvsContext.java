...
import handling.world.buddy.MapleBuddyType;
import handling.world.party.MapleParty;
import handling.world.party.MaplePartyCharacter;
import handling.world.party.MaplePartyType;
...
//            }
    ...
    public static byte[] LoadLinkPreset(final MapleCharacter chr) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.LINK_PRESET_LOAD.getValue());
        mplew.write(3);
        for(int i = 3; i <= 3; ++i) {
            mplew.writeInt(i);
            mplew.writeInt(chr.loadLinkSkillPreset(i).size());
            for(final MapleLinkSkillPreset mlsp : chr.loadLinkSkillPreset(i)) {
                mplew.writeInt(mlsp.getSkillid());
                mplew.writeInt(mlsp.getSkillCharid());
            }
        }
        return mplew.getPacket();
    }
    
    public static byte[] updateLinkPreset(final MapleCharacter chr, final List<Pair<Integer,Integer>> list, final int preset) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.LINK_PRESET.getValue());
        mplew.writeInt(preset);
        mplew.writeInt(list.size());
        for(int i=0; i < list.size(); ++i) {
            mplew.writeInt(list.get(i).getLeft());
            mplew.writeInt(list.get(i).getRight());
        }
        mplew.write(0);
        return mplew.getPacket();
    }
