...
public class MonsterCarnivalPacket {
    ...
    public static byte[] startMonsterCarnival(final MapleCharacter chr, final int enemyavailable, final int enemytotal) {
        ...
        mplew.writeOpcode(SendPacketOpcode.MONSTER_CARNIVAL_START.getValue());
    ...
    public static byte[] playerDiedMessage(String name, int lostCP, int team) { //CPQ
        ...
        mplew.writeOpcode(SendPacketOpcode.MONSTER_CARNIVAL_DIED.getValue());
    ...
    public static byte[] CPUpdate(boolean party, int curCP, int totalCP, int team) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        if (!party) {
            mplew.writeOpcode(SendPacketOpcode.MONSTER_CARNIVAL_OBTAINED_CP.getValue());
        } else {
            mplew.writeOpcode(SendPacketOpcode.MONSTER_CARNIVAL_PARTY_CP.getValue());
            mplew.write(team);
        }
    ...
    public static byte[] playerSummoned(String name, int tab, int number) {
        ...
        mplew.writeOpcode(SendPacketOpcode.MONSTER_CARNIVAL_SUMMON.getValue());
        mplew.write(tab);
        mplew.write(number);
        mplew.writeMapleAsciiString(name);
