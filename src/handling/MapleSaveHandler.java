...
public class MapleSaveHandler implements Runnable {
  ...
  static {
    ...
    // Serenity Pass
    todayKeyValues.put("Clear_Pass_Kill_Monster_Amount", "0");
  }
  ...
  public void run() {
    ...
    public static void reset(Iterator<ChannelServer> channels) {
        ...
        String date3 = Pyears+"-"+Pmonth+"-"+Pday;
        ...
        while (channels.hasNext()) {
            ...
            while (chrs.hasNext()) {
                ...
                chr.Dailyreset();
                if(chr.getClient().getCustomKeyValueStr(2307, "date") == null){
                    chr.getClient().setCustomKeyValue(2307, "date", date3);
                    chr.getClient().setCustomKeyValue(230731, "Roulette", "3");
                  } else if(!chr.getClient().getCustomKeyValueStr(2307, "date").equals(date3)){
                    chr.getClient().setCustomKeyValue(2307, "date", date3);
                    chr.getClient().setCustomKeyValue(230731, "Roulette", "3");
                    //chr.dropMessage(5, "초기화");
                  }
                if (ocal2.get(7) == 2) {
                    if(chr.getKeyValueStr(230802, "reset") == null){
                      chr.setKeyValue(230802, "reset", date3);
                      chr.setKeyValue(230731,"Enhance", "1");
                    } else if(!date3.equals(chr.getKeyValueStr(230802, "reset"))){
                      chr.setKeyValue(230802, "reset", date3);
                      chr.setKeyValue(230731,"Enhance", "1");
                    }
                }
    ...
    if (Calendar.getInstance().get(7) == 2) {
      ...
      PreparedStatement preparedStatement1 = null, ps1 = null, ps2 = null;
      ResultSet resultSet1 = null, rs1 = null;
      try {
        connection = DatabaseConnection.getConnection();
        
        ps2 = connection.prepareStatement("DELETE FROM enhanceswordlog");
        ps2.executeUpdate();
