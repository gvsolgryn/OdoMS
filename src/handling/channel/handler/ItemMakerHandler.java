        ...
        switch (makerType) {
            case 1: { // Gem
                ...
                if (GameConstants.isGem(toCreate)) {
                    ...
                    c.getSession().write(MaplePacketCreator.ItemMakerAction());
                    ...
                    int[] incGems = new int[numEnchanter];
                    if (numEnchanter > 0) {
                        for (int i = 0; i < numEnchanter; i++) {
                            incGems[i] = slea.readInt();
                        }
                    }
                    ...
                    if (!stimulator || Randomizer.nextInt(95) != 0) {
                        MapleInventoryManipulator.addbyItem(c, toGive);
                        c.sendPacket(MaplePacketCreator.onMakerResult(0, makerType, false, toGive.getItemId(), toGive.getQuantity(), 0, create.getReqItems(), create.getCost(), incGems, stimulator ? create.getStimulator() : 0));
                        c.getPlayer().getMap().broadcastMessage(c.getPlayer(), MaplePacketCreator.ItemMaker_Success_3rdParty(c.getPlayer().getId()), false);
                    } else {
//                        c.getPlayer().dropMessage(5, "촉진제의 부작용으로 아이템이 파괴되었습니다.");
                        c.sendPacket(MaplePacketCreator.onMakerResult(1, makerType, true, toGive.getItemId(), toGive.getQuantity(), 0, create.getReqItems(), create.getCost(), incGems, stimulator ? create.getStimulator() : 0));
                    }
                    c.getSession().write(MaplePacketCreator.ItemMaker_Success());
                    c.getSession().write(MaplePacketCreator.ItemMakerAction());
            ...
            case 3: { // Making Crystals
                ...
                if (c.getPlayer().haveItem(etc, 100, false, true)) {
                    int targetItem = getCreateCrystal(etc);
                    MapleInventoryManipulator.addById(c, targetItem, (short) 1, "Made by Maker " + etc + " on " + FileoutputUtil.CurrentReadable_Date());
                    MapleInventoryManipulator.removeById(c, MapleInventoryType.ETC, etc, 100, false, false);

                    c.sendPacket(MaplePacketCreator.onMakerResult(makerType, false, targetItem, etc, 0, new ArrayList<>(), 0, new int[0], 0));
                    c.getSession().write(MaplePacketCreator.ItemMaker_Success());
                    c.getSession().write(MaplePacketCreator.ItemMakerAction());
                    c.getPlayer().getMap().broadcastMessage(c.getPlayer(), MaplePacketCreator.ItemMaker_Success_3rdParty(c.getPlayer().getId()), false);
                }
            ...
            case 4: { // Disassembling EQ.
                ...
                if (!ii.isDropRestricted(itemId) && !ii.isAccountShared(itemId)) {
                    ...
                    items.add(new Pair<>(toGive[0], toGive[1]));
                }
                c.sendPacket(MaplePacketCreator.onMakerResult(makerType, false, 0, 0, itemId, items, 0, new int[0], 0));
    ...
    public static final void StartHarvest(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (lastTime + (5000) > System.currentTimeMillis()) {
            ...
            c.getSession().write(MaplePacketCreator.harvestMessage(reactor.getObjectId(), GameConstants.GMS ? 13 : 11)); //ok to harvest, gogo
