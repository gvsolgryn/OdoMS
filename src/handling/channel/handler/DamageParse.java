...
public class DamageParse {
    ...
    public static void applyAttack(final AttackInfo attack, Skill theSkill, final MapleCharacter player, double maxDamagePerMonster, SecondaryStatEffect effect, AttackType attack_type, boolean BuffAttack, boolean energy) {
        if (player.getPlayer().getMapId() == 224000131) {//공격차단맵
            player.getClient().getSession().writeAndFlush(CWvsContext.enableActions(player.getPlayer()));
            return;
        }
        MapleSummon summon;
                ...
                for (Object oned : attack.allDamage) {
                   ...
                   Integer[] 격투스킬 = new Integer[]{5101012, 5111002, 5121016};
                    if (player.getBuffedValue(SecondaryStat.SeaSerpent) != null) {
                        ...
                        if (Arrays.asList(격투스킬).contains(attack.skill)) {
                            ...
                            if (!chr.skillisCooling(skillid) && !chr.getBuffedValue(5110020)) {
                                ...
                                chr.getClient().getSession().writeAndFlush(CWvsContext.enableActions(chr.getPlayer()));
                    ...
                    if (GameConstants.isFPMage(player.getJob())) {
                        if (attack.skill == 2121052) {
                            ...
                            for (MapleMonster mob : player.getMap().getAllMonster()) {
                                if (effect.calculateBoundingBox(monster.getPosition(), true).contains(mob.getPosition()) || effect.calculateBoundingBox(monster.getPosition(), false).contains(mob.getPosition())) {
                                    mobs.add(mob);
                                }
                            }
                    ...
                    if (player.getBuffedValue(SecondaryStat.QuiverCatridge) != null && attack.skill != 400031021 && attack.skill != 95001000 && attack.skill != 3111013 && attack.skill != 3100010 && attack.skill != 400031000) {
                        ...
                        if (quiverFoolburst) {
                            SecondaryStatEffect adquivers = SkillFactory.getSkill(3120022).getEffect(player.getSkillLevel(3120022));
                            ...
                            if (Randomizer.isSuccess(quiverEff.getW()) || player.getBuffedEffect(SecondaryStat.DebuffIncHp) == null) {
                                player.addHP((int) ((double) player.getStat().getCurrentMaxHp() * ((double) adquivers.getX() / 100.0)));
                            }
                            ...
                            switch (player.getQuiverType()) {
                                ...
                                case 2: {
                                    SecondaryStatEffect adquivers = SkillFactory.getSkill(3120022).getEffect(player.getSkillLevel(3120022));
                                    if (!Randomizer.isSuccess(adquivers.getW()) || player.getBuffedEffect(SecondaryStat.DebuffIncHp) != null) {
                                        break;
                                    }
                                    MapleAtom atom = new MapleAtom(false, player.getId(), 10, true, 3100010, player.getTruePosition().x, player.getTruePosition().y);
                                    atom.setDwFirstTargetId(0);
                    ...
                    if (player.getBuffedValue(SecondaryStat.ShadowBatt) != null && (attack.skill == 14121001 || attack.skill == 14101028 || attack.skill == 14111020 || attack.skill == 14101020 || attack.skill == 14001020) && attack.targets > 0) {
                        int[] skillids;
                        SecondaryStatEffect b_eff = SkillFactory.getSkill(14120017).getEffect(player.getSkillLevel(14121016));
                        int BatLimit = 3;
                        ...
                        if (summons.size() > 0 && Randomizer.isSuccess(Chance)) {
                            ...
                            MapleAtom atom = new MapleAtom(false, player.getId(), 15, true, 14120018, deleted.getTruePosition().x, deleted.getTruePosition().y);
                        ...
                        if (player.getBuffedValue(SecondaryStat.ShadowBatt) != null && (attack.skill == 14121001 || attack.skill == 14101028 || attack.skill == 14111020 || attack.skill == 14101020 || attack.skill == 14001020) && attack.targets > 0) {
                            ...
                            if (summons.size() > 0 && Randomizer.isSuccess(Chance)) {
                                ...
                                if (player.battAttackCount == 10 || deleted != null) {
                                    deleted.removeSummon(player.getMap(), false, false);
                                    player.removeSummon(deleted);
                                }
                    ...
                    Integer[] skills = new Integer[]{3100001, 3100010, 3101009, 3111015, 3111016, 3120021}; // 미라주 발동 제외 스킬
                    if (player.getBuffedValue(SecondaryStat.FlashMirage) != null && !Arrays.asList(skills).contains(attack.skill)) {
                        SecondaryStatEffect flashmirage = SkillFactory.getSkill(3111015).getEffect(player.getSkillLevel(3111015));
                        ...
                        if (player.getSkillLevel(3120021) > 0) {
                            ...
                            count2 = SkillFactory.getSkill(3120021).getEffect(player.getSkillLevel(3120021)).getW();
                        }
                        if (player.플레시미라주스택 < count) {
                            ...
                            for (int i = 0; i < count2; i++) {
                                ...
                                if (objs.size() > 0) {
                                    ...
                                    atoms.add(new SecondAtom(0x27, player.getId(), 0, 3111016, 3000, 0, 6, pos, Arrays.asList(0)));
                                    player.spawnSecondAtom(atoms);
                                    player.getClient().getSession().writeAndFlush((Object) CField.EffectPacket.showEffect(player, 0, 3111015, 83, 0, 0, (byte) 0, true, pos, null, null));
                                    player.getMap().broadcastMessage(player, CField.EffectPacket.showEffect(player, 0, 3111015, 83, 0, 0, (byte) 0, false, pos, null, null), false);
                                }
                            }
                        }
...
//              ");
                ...
                //921170011
                if (!GameConstants.사출기(attack.skill)
                        || attack.targets > 0 && player.getMapId() == 921170011
                        || attack.targets > 0 && player.getMapId() == 921170004
                        || attack.targets > 0 && player.getMapId() == 13000 
                        || attack.targets > 0 && player.getMapId() == 13001
                        || attack.targets > 0 && player.getMapId() == 13002                        
                        || attack.targets > 0 && player.getMapId() == 13003
                        || attack.targets > 0 && player.getMapId() == 13004
                        || attack.targets > 0 && player.getMapId() == 13005
                        || attack.targets > 0 && player.getMapId() == 13006
                        || attack.targets > 0 && player.getMapId() == 13007) {                                
                    if (attack.targets > 0 && player.getKeyValue(99999, "tripling") > 0
                            && attack.skill != 1311020
                            && !GameConstants.isTryFling(attack.skill)
                            && attack.skill != 400031031
                            && attack.skill != 400031001
                            && attack.skill != 13111020
                            && attack.skill != 13121054
                            && attack.skill != 13101022
                            && attack.skill != 13110022
                            && attack.skill != 13120003
                            && attack.skill != 13111020
                            && attack.skill != 400001018
                            || attack.targets > 0 && player.getMapId() == 921170011
                            || attack.targets > 0 && player.getMapId() == 921170004
                            || attack.targets > 0 && player.getMapId() == 13000
                            || attack.targets > 0 && player.getMapId() == 13001
                            ...
                            || attack.targets > 0 && player.getMapId() == 13006
                            || attack.targets > 0 && player.getMapId() == 13007                            
                            || attack.targets > 0 && attack.skill != 1311020 && player.getBuffedEffect(SecondaryStat.TryflingWarm) != null && !GameConstants.isTryFling(attack.skill) && attack.skill != 400031031 && attack.skill != 400031001 && attack.skill != 13111020 && attack.skill != 13121054 && attack.skill != 13101022 && attack.skill != 13110022 && attack.skill != 13120003 && attack.skill != 13111020 && attack.skill != 400001018 && attack.targets > 0) {
                        int skillid = 0;
                        if (player.getKeyValue(99999, "tripling") > 0 || player.IsAutoHuntMap() != 0 || player.getKeyValue(1234, "ww_whim") > 0 || player.getSkillLevel(SkillFactory.getSkill(13120003)) > 0) {
                            skillid = 13120003;
                        ...
                        if (skillid != 0) {
                            ...
                            int maxcount = eff.getX() + (int) (player.getKeyValue(99999, "triplingBonus") > 0 ? (player.getKeyValue(99999, "triplingBonus") * 1) : 0);
                            if (objs.size() > 0) {
                                // int trychance = trskill.getEffect(player.getSkillLevel(trskill)).getProp() + (int) (player.getKeyValue(99999, "triplingBonus") > 0 ? (player.getKeyValue(99999, "triplingBonus") * 10) : 0);
                                int trychance = 100;
                                ...
                                if (player.IsAutoHuntMap() != 0) {
                                    trychance = 100;
                                    maxcount = 2;
                                    if ((player.자동사냥 + 5000) < System.currentTimeMillis()) {
        ...
        if (GameConstants.isNightWalker(player.getJob())) {
            ...
            if (player.getSkillLevel(154121009) > 0) {  //웜에 안플리기
                prop = 100;
            }
        ...
        if (player.getSkillLevel(400051015) > 0 && monster != null) {
            if (monster.getStats().isBoss()) {
                if (monster.isAlive()) {
                    ++player.Serpent;
                }
        ...
        if (player.getSkillLevel(154001003) > 0 && player.getSkillCustomValue(154001003) == null) {
            ...
            if (player.getSkillLevel(154110005) > 0 && player.getSkillCustomValue(154110005) == null) {
                ...
                player.setSkillCustomInfo(154110005, 0L, 10000L);
                 player.getMap().broadcastMessage(player, CField.EffectPacket.showEffect(player, 0, 154110005, 1, 0, 0, (byte) (monster.isFacingLeft() ? 1 : 0), false, monster.getTruePosition(), null, null), false);
                player.getClient().getSession().writeAndFlush((Object) CField.bonusAttackRequest(154110005, new ArrayList<Triple<Integer, Integer, Integer>>(), true, 0, new int[0]));

            }
        }

        if (player.getBuffedValue(SecondaryStat.SummonChakri) != null && player.getBuffedValue(SecondaryStat.ResonateUltimatum) == null) { //서먼차크리만 갖고 있을때

            for (MapleSummon s : player.getMap().getAllSummons(154110010)) { //차크리만 사라지도록
                if (s != null && s.getOwner().getId() == player.getId() && MapleSkillManager.isKhaliHexSkills(attack.skill) && monster != null) {

                    final List<SecondAtom> atoms = new ArrayList<SecondAtom>();
                    atoms.add(new SecondAtom(43, player.getId(), monster.getId(), 154110001, 5000, 0, 3, new Point(s.getTruePosition().x, s.getTruePosition().y), Arrays.asList(0)));
                    player.spawnSecondAtom(atoms);
                    s.removeSummon(player.getMap(), false);
                    player.getSummons().remove(s);
                    player.SummonChakriStack = 0; //초기화

                }
            }
        }
        if (player.getBuffedValue(SecondaryStat.ResonateUltimatum) != null) { //얼티메이텀 있을때
            for (MapleSummon s : player.getMap().getAllSummonsThreadsafe()) {
                if (s != null && s.getOwner().getId() == player.getId() && MapleSkillManager.isKhaliHexSkills(attack.skill) && monster != null) {

                    final List<SecondAtom> atoms = new ArrayList<SecondAtom>();
                    for (int i = 0; i < 2; ++i) {
                        atoms.add(new SecondAtom(44, player.getId(), monster.getId(), 400041091, 5000, 0, 3, new Point(s.getTruePosition().x, s.getTruePosition().y), Arrays.asList(0)));
                    }
                    player.spawnSecondAtom(atoms);
                    s.removeSummon(player.getMap(), false);
                    player.getSummons().remove(s);
        ...
        //400041087 아츠 : 아스트라
        if (player.getSkillLevel(400041087) > 0 && MapleSkillManager.isKhaliAttackSkills(attack.skill) && attack.targets > 0 && System.currentTimeMillis() - player.lastAstraTime >= 14000){
            player.lastAstraTime = System.currentTimeMillis();
            int attackid = 400041087;
            player.getClient().getSession().writeAndFlush((Object) CField.EffectPacket.showEffect(player, 0, attackid, 1, 0, 0, (byte) (monster.isFacingLeft() ? 1 : 0), true, monster.getTruePosition(), null, null));
            player.getMap().broadcastMessage(player, CField.EffectPacket.showEffect(player, 0, attackid, 1, 0, 0, (byte) (monster.isFacingLeft() ? 1 : 0), false, monster.getTruePosition(), null, null), false);
            player.getClient().getSession().writeAndFlush((Object) CField.bonusAttackRequest(400041087, new ArrayList<Triple<Integer, Integer, Integer>>(), true, 0, new int[0]));
        }
        ...
        if (GameConstants.isAdel(player.getJob())) {
            ...
            if (player.getBuffedEffect(SecondaryStat.AbyssalLightning) != null && player.getSkillCustomValue(400021113) == null && monster != null) {
                ...
                List<MapleMapObject> objs = player.getMap().getMapObjectsInRange(player.getTruePosition(), 100000.0, Arrays.asList(new MapleMapObjectType[]{MapleMapObjectType.MONSTER}));
                if (objs != null && objs.size() >= 0) {
                    MapleMonster mob__ = player.getMap().getAllMonster().get(Randomizer.rand(0, objs.size()));
                    ArrayList<Triple<Integer, Integer, Integer>> mobList = new ArrayList<Triple<Integer, Integer, Integer>>();
                    mobList.add(new Triple<Integer, Integer, Integer>(monster.getObjectId(), 0, 0));
                    player.getClient().getSession().writeAndFlush((Object) CField.Abyssal_Lightning(400021113, 5, mob__.getPosition().x));
        ...
        if (player.getBuffedValue(13121017) && attack.skill != 13121017 && !GameConstants.isTryFling(attack.skill) && attack.targets > 0 && Randomizer.isSuccess(30)) {
            MapleAtom atom = new MapleAtom(false, player.getId(), 8, true, 13121017, player.getTruePosition().x, player.getTruePosition().y);
            atom.setDwFirstTargetId(0);
        ...
        if (player.getBuffedValue(13121055) && attack.skill != 13121055 && !GameConstants.isTryFling(attack.skill) && attack.targets > 0 && Randomizer.isSuccess(30)) {
            final List<SecondAtom> atoms = new ArrayList<SecondAtom>();
            atoms.add(new SecondAtom(41, player.getId(), 1, 13121055, 0, 0, 1, new Point(player.getPosition().x, player.getPosition().y), Arrays.asList(8)));
            player.spawnSecondAtom(atoms);
        }
        ...
        if (GameConstants.isFlameWizard(player.getJob()) && attack.skill == 400021092 && player.getBuffedEffect(400021092) != null) {
    ...
    public static final void applyAttackMagic(AttackInfo attack, Skill theSkill, MapleCharacter player, SecondaryStatEffect effect, double maxDamagePerHit) {
       ...
       if (attack != null && attack.allDamage != null) {
        for (AttackPair oned : attack.allDamage) {
            ...
            if (!GameConstants.사출기(attack.skill) || player.IsAutoHuntMap() != 0) {
                if (attack.targets > 0 && player.getKeyValue(99999, "tripling") > 0
                        && attack.skill != 1311020
                        && !GameConstants.isTryFling(attack.skill)
                        && attack.skill != 400031031
                        ...
                        && attack.skill != 13111020
                        && attack.skill != 400001018
                        || player.IsAutoHuntMap() != 0 || attack.targets > 0 && attack.skill != 1311020 && player.getBuffedEffect(SecondaryStat.TryflingWarm) != null && !GameConstants.isTryFling(attack.skill) && attack.skill != 400031031 && attack.skill != 400031001 && attack.skill != 13111020 && attack.skill != 13121054 && attack.skill != 13101022 && attack.skill != 13110022 && attack.skill != 13120003 && attack.skill != 13111020 && attack.skill != 400001018 && attack.targets > 0) {
                    int skillid = 0;
                    if (player.getKeyValue(99999, "tripling") > 0 || player.getSkillLevel(SkillFactory.getSkill(13120003)) > 0) {
                        skillid = 13120003;
                        if (player.getSkillLevel(SkillFactory.getSkill(13120003)) < 30) {
                            player.teachSkill(13120003, 30);
                        }
                    ...
                    //  player.dropMessageGM(6, "발동1 : " + player.getSkillLevel(SkillFactory.getSkill(13120003)));
                    //    player.dropMessageGM(6, "발동1 : " + attack.skill);
                    if (skillid != 0) {
                        Skill trskill = SkillFactory.getSkill(skillid);
                        if (Randomizer.rand(1, 100) <= (skillid == 13100022 ? 5 : (skillid == 13110022 ? 10 : 20))) {
                        ...
                        if (objs.size() > 0) {
                            ...
                            if (Randomizer.isSuccess(trychance) || player.IsAutoHuntMap() != 0) {
                                MapleAtom atom = new MapleAtom(false, player.getId(), 7, true, skillid, player.getTruePosition().x, player.getTruePosition().y);
                                ArrayList<Integer> monsters = new ArrayList<Integer>();
                                if (player.IsAutoHuntMap() != 0) {
                                    maxcount = 1;
            ...
            if (effect != null && monster.isAlive()) {
                ...
                switch (attack.skill) {
                    ...
                    case 12111022: {
                        statusz.add(new Triple(MonsterStatus.MS_Freeze, new MonsterStatusEffect(attack.skill, effect.getDuration()), Long.valueOf(effect.getDuration())));
                        break;
                    }
                    default: {
...
//                        applys.add(new Pair<MonsterStatus, MonsterStatusEffect>(MonsterStatus.MS_Burned, new MonsterStatusEffect(12101024, ((SecondaryStatEffect) ignition).getDOTTime(), player.getSkillLevel(12120050) > 0 ? dam + dam / 100L * 20L : dam)));
    ...
    public static final AttackInfo parseDmgM(LittleEndianAccessor lea, MapleCharacter chr, boolean dot) {
        ...
        try {
            ...
            if (ret.skill == 400031010 || ret.skill == 80002823) {
                lea.skip(4);
                lea.skip(4);
                System.out.println("test: " + lea.toString());
            }
            if (ret.skill == 400041019 || ret.skill == 14101028 || ret.skill == 14101029) { // 1142++
                lea.skip(4);
                lea.skip(4);
            }
    ...
    public static final AttackInfo parseDmgR(LittleEndianAccessor lea, MapleCharacter chr) {
        ...
        try {
            ...
            ret.display = lea.readByte();
            ret.facingleft = lea.readByte();
            //  chr.dropMessageGM(6, "ret.facingleft  : " + ret.facingleft);
            lea.skip(4);
            ret.attacktype = lea.readByte();

            //  chr.dropMessageGM(6, "ret.attacktype   : " + ret.attacktype  );
            ...
            //   chr.dropMessageGM(6, "ret.csstar  : " + ret.csstar );
            lea.skip(1);
            lea.skip(2);
    ...
    public static void WFinalAttackRequest(MapleCharacter chr, int skillid, MapleMonster monster) {
        if (SkillFactory.getSkill(skillid) != null) {
            if (chr.getJob() == 512 && chr.skillisCooling(5121013)) {
                ...
                if (GameConstants.isSoulMaster(chr.getJob())) {
                    if (chr.isAutoSkill(11121018) && chr.getCosmicCount() >= 5) {
                        chr.getClient().getSession().writeAndFlush(CField.rangeAttack(11121018, Arrays.asList(new RangeAttack(11121018, chr.getTruePosition(), 1, 330, 1))));
                    }
                }
            }
