    ...
    public static final void Guild(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (slea.readByte()) { //AFTERSHOCK: most are +1
            case 0x02: // Create guild
                ...
                if (guildId == 0) {
                    c.getSession().write(MaplePacketCreator.genericGuildMessage((byte) 30));//33 동의문제 34파끝 35 이미 길드가 있다! 30 이미 쓰고있는길드
                    return;
                }
                ...
                //c.getSession().write(MaplePacketCreator.genericGuildMessage((byte)35));//33 동의문제 34파끝 35 이미 길드가 있다! 30 이미 쓰고있는길드
                c.getSession().write(MaplePacketCreator.completeGuildCreate(c.getPlayer()));//
                //World.Guild.gainGP(c.getPlayer().getGuildId(), 500, c.getPlayer().getId());
                c.getPlayer().dropMessage(1, "길드를 성공적으로 만들었습니다.");
