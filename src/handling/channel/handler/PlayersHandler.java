...
"      MarriageDataEntry entry = MarriageManager.getInstance().getMarriag다.");
  ...
  public static void LinkPreset(final LittleEndianAccessor slea, final MapleClient c) {
      final List<Pair<Integer,Integer>> list = new ArrayList<Pair<Integer,Integer>>();
      final MapleCharacter chr = c.getPlayer();
      final int preset = slea.readInt();
      final int size = slea.readInt();
      chr.resetLinkSkillPreset(preset);
      for(int i  = 0; i < size; ++i) {
          final int skillid = slea.readInt();
          final int skillchar = slea.readInt();
          list.add(new Pair<Integer, Integer>(skillid, skillchar));
          chr.addLinkSkillPreset(preset, skillid, skillchar);
      }
      c.getSession().writeAndFlush(CWvsContext.updateLinkPreset(chr, list, preset));
  }
