    ...
    public static final boolean UseRewardItem(final short slot, final int itemId, final MapleClient c, final MapleCharacter chr) {
        ...
        if (toUse != null && toUse.getQuantity() >= 1 && toUse.getItemId() == itemId) {
            if (chr.getInventory(MapleInventoryType.EQUIP).getNextFreeSlot() > -1 && chr.getInventory(MapleInventoryType.USE).getNextFreeSlot() > -1 && chr.getInventory(MapleInventoryType.SETUP).getNextFreeSlot() > -1 && chr.getInventory(MapleInventoryType.ETC).getNextFreeSlot() > -1) {
                ...
                } else if (itemId == 2028181) {
                    NPCScriptManager.getInstance().startItem(c, 9062764, "consume_" + itemId);
                } else {
            ...
            case 5155005:
                ...
                switch (j) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        if (GameConstants.isAdel(c.getPlayer().getJob()) || GameConstants.isArk(c.getPlayer().getJob()) || GameConstants.isHoyeong(c.getPlayer().getJob())) {
                            j = 0;
                        }
    ...
    public static void pickupItem(MapleMapObject ob, MapleClient c, MapleCharacter chr) {
        ...
        try {
            ...
            switch (mapitem.getItemId()) {
                case 2633304:
                case 2633609: {
                    String[] bossname = new String[]{"이지 시그너스", "하드 힐라", "카오스 핑크빈", "노멀 시그너스", "카오스 자쿰", "카오스 피에르", "카오스 반반", "카오스 블러디퀸", "하드 매그너스", "카오스 벨룸", "카오스 파풀라투스", "노멀 스우", "노멀 데미안", "이지 루시드", "노멀 루시드", "노멀 윌", "노멀 더스크", "노멀 듄켈", "하드 데미안", "하드 스우", "하드 루시드", "하드 윌", "카오스 더스크", "하드 듄켈", "진 힐라", "세렌", "칼로스"};
    ...
    public static final void Pickup_Pet(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        try {
            ...
            if (mapitem.getItemId() == 2633304 || mapitem.getItemId() == 2633609) {
                final String[] bossname = {"이지 시그너스", "하드 힐라", "카오스 핑크빈", "노멀 시그너스", "카오스 자쿰", "카오스 피에르", "카오스 반반", "카오스 블러디퀸", "하드 매그너스", "카오스 벨룸", "카오스 파풀라투스", "노멀 스우", "노멀 데미안", "이지 루시드", "노멀 루시드", "노멀 윌", "노멀 더스크", "노멀 듄켈", "하드 데미안", "하드 스우", "하드 루시드", "하드 윌", "카오스 더스크", "하드 듄켈", "진 힐라", "세렌", "칼로스"};
                String[] wishCoinCheck = null;
                if (ServerConstants.Event_MapleLive) {
                    wishCoinCheck = chr.getClient().getCustomKeyValueStr(501468, "reward").split("");
                }
