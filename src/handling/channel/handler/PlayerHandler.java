...
public class PlayerHandler {
    ...
    public static final void SpecialMove(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (chr.getPlayer().getMapId() == 224000131) {//공격차단맵
            c.getSession().writeAndFlush(CWvsContext.enableActions(c.getPlayer()));
           return;
        }
                
        if (chr.getPlayer().getMapId() == ServerConstants.warpMap) {
            return;
        }
...
//                }
    ...
    public static final void rangedAttack(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) { // 원거리 공격
        ...
        if (attack.skill != 0) {
            ...
            if (attack.isLink && (attack.skill < 400051059 || attack.skill > 400051067) && attack.skill != 0) {
                if (chr.getCooldownLimit(15101028) == 0) {
                    chr.getClient().getSession().writeAndFlush(CField.ChainingSkills(chr.getId()));
                }
            }
    ...
    public static final void MagicDamage(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr, boolean chilling, boolean orbital) {
        ...
        if (GameConstants.isFPMage(chr.getJob())) {
            if (chr.getMap().getMist(chr.getId(), 2111013) != null) {
               // chr.getMap().broadcastMessage(CField.PoisonRegionMetastasis(2111013, 2111014, chr.getPositionX(), (byte)0));
        ...
        for (int i = 0; i < wsize; ++i) {
            ...
            if (skillId != 400041023) {
                if (c.getPlayer() == null || SkillFactory.getSkill(skillId) == null) {
                    if (uu != 0) {
                        FileoutputUtil.log("Log/AbsorbingRegen.txt", "원래 스킬아이디 : " + uu + "가 " + skillId + "로 리턴됌 \r\n" + sl);
                    }
                    return;
                }
...
/*                chr.addCoolTime(12120022, 30000);
    ...
    public static void SymbolMultiExp(LittleEndianAccessor slea, MapleClient c) {
        try {
            ...
            if (removeitems.size() != count) {
                FileoutputUtil.log("Log/Log_Packet_Except.rtf", c.getPlayer().getName() + " 캐릭터 심볼 비정상 사용발견");
            }
    ...
    public static void useMoonGauge(MapleClient c) {//mush will
        if (c.getPlayer().getMapId() == 450008150 || c.getPlayer().getMapId() == 450008750) {
            SkillFactory.getSkill(1221054).getEffect(1).applyTo(c.getPlayer(), 3000);
            ...
            c.getPlayer().getBuffedValue(SecondaryStat.FastCharge);
    ...
    public static void eventUIResult(LittleEndianAccessor slea, MapleClient c) {
        ...
        switch (type) {
            ...
            case 18:
                ...
                break;
            case 19: // 유니온 코인
                if (c.getPlayer().getUnionCoin() <= 0) {
                    c.getPlayer().dropMessage(1, "획득가능한 유니온 코인이 없습니다.");
                    return;
                }
                c.getPlayer().dropMessage(1, "유니온 코인 " + c.getPlayer().getUnionCoin() + "개를 획득하였습니다!\r\n인벤토리를 확인하여 주세요.");
                c.getPlayer().gainItem(4310229, c.getPlayer().getUnionCoin());
                c.getPlayer().setUnionCoin(0);
                break;
            case 29:
            ...
            case 41: {
                ...
                if (idx == 1300) {
                    //NPCScriptManager.getInstance().start(c, 3001604, "UI09"); // 워프
