...
public class PlayerInteractionHandler {
    ...
    private enum Interaction {
        ...
        RESET_HIRED(0x0D),
        SET_ITEMS(GameConstants.GMS ? 0 : 0x0E),
        SET_MESO(GameConstants.GMS ? 1 : 0x0F),
        CONFIRM_TRADE(GameConstants.GMS ? 2 : 0x10),
        PLAYER_SHOP_ADD_ITEM(GameConstants.GMS ? 40 : 0x14),
        BUY_ITEM_PLAYER_SHOP(GameConstants.GMS ? 22 : 0x15),
        KICK_STORE(0x1A),
        ADD_ITEM(GameConstants.GMS ? 23 : 0x1F),
        ITEM_REMOVE(0x22),
        BUY_ITEM_STORE(GameConstants.GMS ? 24 : 0x20),
        BUY_ITEM_HIREDMERCHANT(GameConstants.GMS ? 26 : 0x21),
        REMOVE_ITEM(GameConstants.GMS ? 28 : 0x24),
        MAINTANCE_OFF(GameConstants.GMS ? 29 : 0x25), //This is misspelled...
        MAINTANCE_ORGANISE(GameConstants.GMS ? 30 : 0x26),
        CLOSE_MERCHANT(0x27),
        ADMIN_STORE_NAMECHANGE(GameConstants.GMS ? 35 : 0x2B),
        VIEW_MERCHANT_VISITOR(GameConstants.GMS ? 36 : 0x2C),
        VIEW_MERCHANT_BLACKLIST(GameConstants.GMS ? 37 : 0x2D),
        MERCHANT_BLACKLIST_ADD(GameConstants.GMS ? 38 : 0x2E),
        MERCHANT_BLACKLIST_REMOVE(GameConstants.GMS ? 39 : 0x2F),
        REQUEST_TIE(GameConstants.GMS ? 51 : 0x30),
        ANSWER_TIE(GameConstants.GMS ? 52 : 0x31),
        GIVE_UP(GameConstants.GMS ? 53 : 0x32),
        REQUEST_REDO(GameConstants.GMS ? 55 : 0x34),
        ANSWER_REDO(GameConstants.GMS ? 56 : 0x35),
        EXIT_AFTER_GAME(GameConstants.GMS ? 57 : 0x36),
        CANCEL_EXIT(GameConstants.GMS ? 58 : 0x37),
        READY(GameConstants.GMS ? 59 : 0x38),
        UN_READY(GameConstants.GMS ? 60 : 0x39),
        EXPEL(GameConstants.GMS ? 61 : 0x3A),
        START(GameConstants.GMS ? 62 : 0x3B),
        SKIP(GameConstants.GMS ? 64 : 0x3D),
        MOVE_OMOK(GameConstants.GMS ? 65 : 0x3E),
        SELECT_CARD(GameConstants.GMS ? 68 : 0x42);
        public int action;
    ...
    public static final void PlayerInteraction(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        switch (action) { // Mode
                    ...
                    if (ob instanceof IMaplePlayerShop && chr.getPlayerShop() == null) {
                        ...
                        if (ob instanceof HiredMerchant) {
                            ...
                            if (ips instanceof MaplePlayerShop && ((MaplePlayerShop) ips).isBanned(chr.getName())) {
                                chr.dropMessage(1, "강퇴당한 상점에 재방문이 불가능합니다.");
                                return;
                            } else {
                        ...
                        if (p != null) { 
                            if (p.getRight() != null) {
                                ...
                                if (tchr.getName().toLowerCase().equals(name.toLowerCase())) {
                                    ...
                                    tchr.dropMessage(1, "방에서 강퇴당했습니다.111");
            ...
            case SET_ITEMS: {
                ...
                final short quantity = (short) Math.min(slea.readShort(), ii.getSlotMax(item.getItemId()));
                final byte targetSlot = slea.readByte();
                if (quantity <= 0 && !GameConstants.isThrowingStar(item.getItemId()) && !GameConstants.isBullet(item.getItemId()) || 
                    quantity > 30000 ||
                    quantity > (short) ii.getSlotMax(item.getItemId())
                    ) { 
