    ...
    private void loadMakeCharInfos() {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            for (MapleData d : data.getChildren()) {
                int type = -1;
                if (i == 0) {
                    if (d.getName().equalsIgnoreCase("ResistanceCharMale") || d.getName().equalsIgnoreCase("Charmale") || d.getName().equalsIgnoreCase("EvanCharMale") || d.getName().equalsIgnoreCase("OrientCharMale")) {
                        type = 0; //레지스탕스 캐릭터생성 캐생
                    }
                } else {
                    if (d.getName().equalsIgnoreCase("ResistanceCharFemale") || d.getName().equalsIgnoreCase("Charfemale") || d.getName().equalsIgnoreCase("EvanCharFemale") || d.getName().equalsIgnoreCase("OrientCharFemale")) {
                        type = 1;
                    }
                }
                // face : 20100 hair : 30000 top : 1050175 bottom : 0 shoes  : 1072037 weapon : 1312004
                if (type != -1) {
