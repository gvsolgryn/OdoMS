    ...
    public final void writeToDB(final boolean bDisband) {
        ...
        try {
            ...
            if (!bDisband) {
                ...
                buf.append("capacity = ?, notice = ?, alliance = ?, leader = ?, level = ? WHERE guildid = ?");
...
//		changed_skills = false;
            ...
            while (itr.hasNext()) {
                ...
                if (mgc.getId() == cid && initiator.getGuildRank() < mgc.getGuildRank()) {
                    ...
                    if (mgc.isOnline()) {
                        ...
                        MapleCharacterUtil.sendNote(mgc.getName(), initiator.getName(), "길드에서 추방당했습니다.", 0);
    ...
    public void setLevel2(int a) {
        ...
        try {
            ...
            ps.setInt(1, this.level2);
            ps.setInt(2, this.id);
            ps.execute();
            ps.close();
        } catch (SQLException e) {
            System.err.println("Saving guild level ERROR");
            e.printStackTrace();
        } finally {
            if (con != null) {
                try {
