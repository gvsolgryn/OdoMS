package org.extalia.handling.world.guild;

import org.extalia.client.MapleCharacter;
import org.extalia.client.MapleCharacterUtil;
...
public class MapleGuild implements Comparable<MapleGuild> {
    ...
    public final void setOnline(int cid, boolean online, int channel) {
        ...
        if (bBroadcast) {
            broadcast(CWvsContext.GuildPacket.guildMemberOnline(this.id, cid, online), cid);
            if (this.allianceid > 0) {
    ...
    public final void leaveGuild(MapleGuildCharacter mgc) {
        ...
        while (itr.hasNext()) {
            ...
            if (mgcc.getId() == mgc.getId()) {
                for (int i = 1; i <= 5; i++) {
                    RankBroadCast(CWvsContext.GuildPacket.memberLeft(mgcc, (i != 5)), i);
                }
        ...
        for (MapleGuildCharacter member : this.members) {
            if (member.getId() == mgc.getId()) {
                ...
                if (old_level != mgc.getLevel() && mgc.getLevel() >= 200) {
                    broadcast(CWvsContext.sendLevelup(false, mgc.getLevel(), mgc.getName()), mgc.getId());
                }
                if (old_job != mgc.getJobId()) {
                    broadcast(CWvsContext.sendJobup(false, mgc.getJobId(), mgc.getName()), mgc.getId());
                }
                broadcast(CWvsContext.GuildPacket.guildMemberLevelJobUpdate(mgc));
                if (this.allianceid > 0) {
    ...
    public final void changeRankTitle(MapleCharacter chr, String[] ranks) {
        ...
        updateRankRole();
        broadcast(CWvsContext.GuildPacket.rankTitleChange(125, chr, ranks, roles));
    }
    ...
    public final void changeRankRole(MapleCharacter chr, int[] roles) {
        ...
        updateRankRole();
        broadcast(CWvsContext.GuildPacket.rankTitleChange(127, chr, ranks, roles));
    }
    ...
    public final void changeRankTitleRole(MapleCharacter chr, String ranks, int roles, byte type) {
        ...
        updateRankRole();
        broadcast(CWvsContext.GuildPacket.rankTitleChange(122, chr, ranks1, roles1));
    }
    ...
    public final void gainGP(int amount, boolean broadcast, int cid) {
        ...
        broadcast(CWvsContext.GuildPacket.GainGP(this, chr, mg.getGuildContribution()));
        if (broadcast) {
    ...
    public boolean activateSkill(int skill, String name) {
        ...
        writeToDB(false);
        broadcast(CWvsContext.GuildPacket.guildSkillPurchased(this.id, skill, ourSkill.level, ourSkill.timestamp, ourSkill.purchaser, name));
        return true;
    }
    ...
    public boolean purchaseSkill(int skill, String name, int cid) {
        ...
        writeToDB(false);
        broadcast(CWvsContext.GuildPacket.guildSkillPurchased(this.id, skill, ourSkill.level, ourSkill.timestamp, name, name));
        return true;
    }
