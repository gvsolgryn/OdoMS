...
import org.extalia.constants.CubeOption;
import org.extalia.constants.EdiCubeOption;
...
import org.extalia.log.DBLogger;
import org.extalia.log.LogType;
...
import org.extalia.server.life.MobSkillFactory;
import org.extalia.server.maps.*;
import org.extalia.server.quest.MapleQuest;
import org.extalia.server.shops.MapleShopFactory;
        ...
        switch (itemId) {
            ...
            case 5064200:
                ...
                if (equip2 != null) {
                    ...
                    c.getSession().writeAndFlush(CWvsContext.InventoryPacket.updateInventoryItem(false, MapleInventoryType.EQUIP, (Item) equip2));
            ...
            case 5062500:
                up = false;
                n = slea.readInt();
                item10 = c.getPlayer().getInventory((n < 0) ? MapleInventoryType.EQUIPPED : MapleInventoryType.EQUIP).getItem((short) n);
                if (item10 != null && c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                    ...
                    if (c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                        ...
                        if (equip.getPotential6() > 0) {
                            equip.setPotential6(EdiCubeOption.getEdiCubePotentialId(item10.getItemId(), level, 3, new int[]{equip.getPotential4(), equip.getPotential5()}));
                            while (!GameConstants.getPotentialCheck(equip.getPotential6(), equip.getPotential4(), equip.getPotential5())) {
                                equip.setPotential6(EdiCubeOption.getEdiCubePotentialId(item10.getItemId(), level, 3, new int[]{equip.getPotential4(), equip.getPotential5()}));
                            }
                        }
                        if (GameConstants.isZeroWeapon(equip.getItemId())) {
                            ...
                            zeroequip.setPotential6(equip.getPotential6());
                            if (zeroequip != null) {
                                c.getSession().writeAndFlush(CWvsContext.InventoryPacket.updateInventoryItem(false, MapleInventoryType.EQUIP, (Item) zeroequip));
                            }
                        }
            ...
            case 5062503:
                ...
                if (item8 != null && c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                    ...
                    if (neq.getPotential4() >= 0) {
                        int level = 0;
                        level = (neq.getPotential4() >= 10000) ? (neq.getPotential4() / 10000) : (neq.getPotential4() / 100);
                        if (level >= 4) {
                            level = 4;
                        }
                        ...
                        if (neq.getPotential6() > 0) {
                            neq.setPotential6(EdiCubeOption.getEdiCubePotentialId(item8.getItemId(), level, 3, new int[]{neq.getPotential4(), neq.getPotential5()}));
                        ...
                        c.getSession().writeAndFlush(CField.getWhiteCubeStart(c.getPlayer(), (Item) neq, bool4, 5062503, c.getPlayer().itemQuantity(toUse.getItemId()) - 1));
            ...
            case 5062010:
                ...
                k = slea.readInt();
                item9 = c.getPlayer().getInventory((k < 0) ? MapleInventoryType.EQUIPPED : MapleInventoryType.EQUIP).getItem((short) k);
                bool5 = false;
                if (item9 != null && c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                    ...
                    if (neq.getState() >= 17) {
                        ...
                        if (neq.getPotential3() > 0) {
                            neq.setPotential3(CubeOption.getBlackCubePotentialId(item9.getItemId(), level, 3, new int[]{neq.getPotential1(), neq.getPotential2()}));
                        ...
                        c.getPlayer().gainMeso(-GameConstants.getCubeMeso(neq.getItemId()), false);
                        c.getSession().writeAndFlush(CField.getBlackCubeStart(c.getPlayer(), (Item) neq, bool5, 5062010, toUse.getPosition(), c.getPlayer().itemQuantity(toUse.getItemId()) - 1));
            ...
            case 5062006:
                ...
                if (item7 != null && c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                    ...
                    eq.setPotential1(CubeOption.getPlatinumUnlimitiedCubePotentialId(item7.getItemId(), level, 1, new int[0]));
                    eq.setPotential2(CubeOption.getPlatinumUnlimitiedCubePotentialId(item7.getItemId(), level, 2, new int[0]));

                    if (eq.getPotential3() > 0) {
                        eq.setPotential3(CubeOption.getPlatinumUnlimitiedCubePotentialId(item7.getItemId(), level, 3, new int[]{eq.getPotential1(), eq.getPotential2()}));
                        while (!GameConstants.getPotentialCheck(eq.getPotential3(), eq.getPotential1(), eq.getPotential2())) {
                            eq.setPotential3(CubeOption.getPlatinumUnlimitiedCubePotentialId(item7.getItemId(), level, 3, new int[]{eq.getPotential1(), eq.getPotential2()}));
                        }
                    }
                    if (GameConstants.isZeroWeapon(eq.getItemId())) {
...
//                slea.readByte();
                ...
                // AC FD 01 04 04 00 2C 9E 4E 00 00 00 EA 88 00 00 // 35 0 50
                // B2 BA 02 04 04 00 2C 9E 4E 00 00 00 3A CF 00 00 // 53 0 50
                ...
                i4 = ordinaryColor / 10000; // 메인컬러
                c.getPlayer().dropMessageGM(6, "i4 : " + i4);
                i5 = ordinaryColor / 1000 - (i4 * 10); // 서브컬러
                c.getPlayer().dropMessageGM(6, "i5 : " + i5);

                i6 = ordinaryColor % 100; // 채도
                c.getPlayer().dropMessageGM(6, "i6 : " + i6);
                while (i5 == i4) { // 같으면 랜덤 생성
...
//5000930, 5000931, 5000932,5002079,5002080,5002081,5002254,5002255,5002256
            ...
            case 5069001:
                ...
                try {
                    ...
                    if (마라벨인가) {
                        World.Broadcast.broadcastMessage(CWvsContext.serverMessage(11, c.getPlayer().getClient().getChannel(), "", c.getPlayer().getName() + "님이 프리미엄 마스터피스에서 {} 을 획득하였습니다.", true, (Item) equip));
                    }
                    used = true;
                } catch (Exception e) {
                    c.getPlayer().dropMessage(1, "마스터 피스가 준비되지 않았습니다. 다시 시도해주세요.");
            ...
            case 5064000:
                ...
                toScroll.setFlag(flag);
                c.getSession().writeAndFlush(CWvsContext.InventoryPacket.updateInventoryItem(false, MapleInventoryType.EQUIP, (Item) toScroll));
            ...
            case 5064300:
                ...
                toScroll.setFlag(flag);
                c.getSession().writeAndFlush(CWvsContext.InventoryPacket.addInventorySlot(MapleInventoryType.EQUIP, (Item) toScroll));
                c.getSession().writeAndFlush(CWvsContext.InventoryPacket.updateInventoryItem(false, MapleInventoryType.EQUIP, (Item) toScroll));
                c.getPlayer().getMap().broadcastMessage(c.getPlayer(), CField.getScrollEffect(c.getPlayer().getId(), Equip.ScrollResult.SUCCESS, false, toUse.getItemId(), toScroll.getItemId()), true);
                used = true;
                break;
    ...
    public static void pickupItem(MapleMapObject ob, MapleClient c, MapleCharacter chr) {
        ...
        try {
            ...
            if (mapitem.getMeso() > 0) {
                ...
                FileoutputUtil.log(FileoutputUtil.아이템획득로그, "[PickUP_Player] 계정번호 : " + chr.getClient().getAccID() + " | " + chr.getName() + "이(가)  맵 :" + c.getPlayer().getMapId() + "에서 " + MapleItemInformationProvider.getInstance().getName(mapitem.getItem().getItemId()) + "(" + mapitem.getItem().getItemId() + ")를 " + mapitem.getItem().getQuantity() + " 개 획득");
