...
import org.extalia.client.messages.CommandProcessor;
...
import org.extalia.handling.world.MapleMessenger;
...
public class ChatHandler {
    ...
    public static final void GeneralChat(final String text, final byte unk, final MapleClient c, final MapleCharacter chr, final LittleEndianAccessor slea, final RecvPacketOpcode recv) {
        if (text.length() > 0 && chr != null && chr.getMap() != null && !CommandProcessor.processCommand(c, text, ServerConstants.CommandType.NORMAL)) {
            ...
            int[] colorArray;
            int color = 8;
            final Item medal = chr.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -21);
            medalArray = new int[]{1149990, 1149991, 1149992, 1149993, 1149994, 1149995, 1149996, 1149997, 1149998, 1149999};
            colorArray = new int[]{5, 6, 7, 9, 4, 11, 12, 13, 15, 16};
            String medalName = "";
            if (medal != null) {
            ...
            if (c.getChatBlockedTime() == 0L) {
                if (chr.isHidden()) {
                    ...
                    for (ChatListener listener : listeners) {
                        listener.someoneWroteAMessage(chr.getName() + "#CH." + chr.getClient().getChannel(), chr.getName() + " : " + text.replaceAll("~", ""));
                    }
                } else {
                    AdminTool.addMessage(0, "[" + chr.getClient().getChannel() + "채널] " + chr.getName() + " : " + text.replaceAll("~", ""));
                    FileoutputUtil.log(FileoutputUtil.일반채팅로그, "[일반] [" + chr.getClient().getChannel() + "채널] " + chr.getName() + " : " + text);
        ...
        switch (slea.readByte()) {
            ...
            case 6:
                if (messenger != null) {
                    ...
                    FileoutputUtil.log(FileoutputUtil.메신저채팅로그, "[메신저] [" + c.getChannel() + "채널] 채팅방번호 : " + messenger.getId() + " | " + c.getPlayer().getName() + " : " + text);
                }
                break;
            case 9:
                if (messenger != null) {
