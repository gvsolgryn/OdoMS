...
import org.extalia.server.Obstacle;
import org.extalia.server.Randomizer;
import org.extalia.server.Timer;
import org.extalia.server.field.boss.MapleBossManager;
import org.extalia.server.field.boss.demian.MapleFlyingSword;
import org.extalia.server.field.boss.lotus.MapleEnergySphere;
...
public class MobHandler {
    ...
    public static final void MoveMonster(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        try {
            ...
            int skillLevel = (int) (targetInfo >> 16L & 0xFFFFL);
            int option = (int) (targetInfo >> 32L & 0xFFFFL);
            ...
            boolean movingAttack = ((bOption & 0x1) == 1);
            ...
            for (int j = 0; j < count; j++) {
                randTimeForAreaAttack.add(Short.valueOf(slea.readShort()));
            }
                ...
                if (action >= 13 && action <= 29) {
                    ...
                    if (attackIdx < monster.getStats().getAttacks().size()) {
                        ...
                        if (attack.getAfterAttack() >= 0) {
                            chr.dropMessageGM(-8, "mobId : " + monster.getId() + " afterattack : " + attack.getAfterAttack() + " attackcount : " + attack.getAfterAttackCount() + " attackidx : " + action);
                            monster.getMap().broadcastMessage(MobPacket.setAfterAttack(oid, attack.getAfterAttack(), (attack.getAfterAttack() < 0) ? 0 : attack.getAfterAttackCount(), action, ((actionAndDir & 0x1) != 0)));
                        ...
                        for (Triple<Integer, Integer, Integer> skill : (Iterable<Triple<Integer, Integer, Integer>>) attack.getSkills()) {
                            MobSkill msi = MobSkillFactory.getMobSkill(((Integer) skill.left).intValue(), ((Integer) skill.mid).intValue());
                            if (msi != null) {
                                if (((Integer) skill.right).intValue() > 0) {
                                    msi.setMobSkillDelay(c.getPlayer(), monster, ((Integer) skill.right).intValue(), (short) 0, ((actionAndDir & 0x1) != 0));
                                    continue;
                                }
                                if (msi.getSkillAfter() > 0) {
                                    msi.setMobSkillDelay(c.getPlayer(), monster, msi.getSkillAfter(), (short) 0, ((actionAndDir & 0x1) != 0));
                                    continue;
                                }
                                if (option > 0) {
                            ...
                            if (!msi.checkDealyBuff(chr, monster)) {
                                ...
                                if (msi.getSkillAfter() > 0) {
                                    ...
                                    if (skillId == 170) {
                                        ...
                                        switch (skillLevel) {
                                            case 13:
                                                monster.getMap().broadcastMessage(MobPacket.setAfterAttack(monster.getObjectId(), 8, 1, 30, ((actionAndDir & 0x1) != 0)));
                                                break;
                                            case 42:
                                                ...
                                                monster.getMap().broadcastMessage(MobPacket.AfterAttacklist(monster, skillId, skillLevel, ((actionAndDir & 0x1) != 0), 1, pos));
                                                delaytime -= 500;
                                                break;

                                            case 45:
                                                monster.getMap().broadcastMessage(MobPacket.AfterAttack(monster, skillId, skillLevel, ((actionAndDir & 0x1) != 0), 1, 3));
                                                break;
                                            case 46:
                                                monster.getMap().broadcastMessage(MobPacket.AfterAttack(monster, skillId, skillLevel, ((actionAndDir & 0x1) != 0), 1, 1));
                                                break;
                                            case 47:
                                            case 61:
                                                x = ((actionAndDir & 0x1) != 0) ? -600 : 600;
                                                monster.getMap().broadcastMessage(MobPacket.AfterAttack(monster, skillId, skillLevel, ((actionAndDir & 0x1) != 0), 1, 7));
                                ...
                                if (msi.getAfterAttack() >= 0) {
                                    monster.getMap().broadcastMessage(MobPacket.setAfterAttack(oid, msi.getAfterAttack(), msi.getAfterAttackCount(), action, ((actionAndDir & 0x1) != 0)));
                                }
                            }
                    ...
                    if (attackInfo != null && attackInfo.getSkill() != null) {
                        ...
                        for (MobSkill msi : monster.getSkills()) {
                            ...
                            if (time - monster.getLastSkillUsed(msi.getSkillId(), msi.getSkillLevel()) >= 0L) {
                                ...
                                if (monster.getHPPercent() <= msi.getHP() && !msi.isOnlyOtherSkill() && !msi.checkCurrentBuff(chr, monster) && (!msi.isOnlyFsm() || msi.getSkillId() == 223)) {
            ...
            if (monster.getEliteGradeInfo().size() > 0) {
                if (monster.getEliteGrade() >= 0 && monster.getCustomValue(9999) == null) {
                    if (monster.isAlive()) {
                        monster.setNextSkill(((Integer) ((Pair) monster.getEliteGradeInfo().get(0)).getLeft()).intValue());
                        monster.setNextSkillLvl(((Integer) ((Pair) monster.getEliteGradeInfo().get(0)).getRight()).intValue());
                        MobSkill msi2 = MobSkillFactory.getMobSkill(((Integer) ((Pair) monster.getEliteGradeInfo().get(0)).getLeft()).intValue(), ((Integer) ((Pair) monster.getEliteGradeInfo().get(0)).getRight()).intValue());
                        msi2.applyEffect(chr, monster, true, ((actionAndDir & 0x1) != 0));
                        monster.setCustomInfo(9999, 0, (int) msi2.getInterval() * 1000);
                    }
                } else if (monster.getCustomValue(9999) != null
                        && monster.getNextSkill() > 0) {
                    monster.setNextSkill(0);
                    monster.setNextSkillLvl(0);
                }
            }
            ...
            if (monster.getId() == 8800002 || monster.getId() == 8800102) {
                ...
                boolean phase1 = !(monster.getId() == 8500002 || monster.getId() == 8500012 || monster.getId() == 8500022);
            ...
            if (monster.getId() == 8880000 || monster.getId() == 8880002 || monster.getId() == 8880010) {
                ...
                if ((actionAndDir == 30 || actionAndDir == 31)
                        && bOption != 1 && bOption != 0) {
            ...
            } else if ((monster.getId() == 8880111) ? ((actionAndDir != 26 && actionAndDir != 27)
                    || bOption == 1 || bOption != 0) : ((monster.getId() != 8880341 && monster.getId() != 8880301) || (actionAndDir != 32 && actionAndDir != 33)
                    || bOption == 1 || bOption != 0)) {

            }

            if (monster.getId() == 8880405
                    && monster.getMap().getCustomValue0(28002) != 0
                    && monster.getMap().getCustomValue0(28002) - 150 < (monster.getPosition()).x && monster.getMap().getCustomValue0(28002) + 150 > (monster.getPosition()).x) {
    ...
    public static void demianBind(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (result == 0
                && chr.getDiseases().containsKey(SecondaryStat.Lapidification)) {
    ...
    public static void lucidStateChange(MapleCharacter chr) {
        ...
        if (lucid != null
            ...
            chr.getMap().broadcastMessage(MobPacket.BossLucid.setButterflyAction(Butterfly.Mode.ERASE, new int[]{700, -600}));
        }
    }
    ...
    public static void BloodyQueenMirror(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (queen != null) {
            ...
            c.send(CField.EffectPacket.showEffect(c.getPlayer(), 0, minushp, 36, 0, 0, (byte) 0, true, null, null, null));
            c.getPlayer().getMap().broadcastMessage(c.getPlayer(), CField.EffectPacket.showEffect(c.getPlayer(), 0, minushp, 36, 0, 0, (byte) 0, false, null, null, null), false);
    ...
    public static void VanVanTimeOver(LittleEndianAccessor slea, MapleClient c) {
        if ((c.getPlayer().getMapId() >= 105200120 && c.getPlayer().getMapId() <= 105200129) || (c.getPlayer().getMapId() >= 105200520 && c.getPlayer().getMapId() <= 105200529)) {
            ...
            if (suc) {
                ...
                if (vanvan != null
                        && vanvan.getBuff(MonsterStatus.MS_Stun) == null) {
    ...
    public static final void RemoveObstacle(LittleEndianAccessor slea, MapleClient c) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (hit == 1) {
                Obstacle ob = null;
                for (Obstacle o : c.getPlayer().getMap().getAllObstacle()) {
            ...
            for (Obstacle ob : c.getPlayer().getMap().getAllObstacle()) {
                if (ob.getObjectId() == objid) {
                    remove.add(ob);
                }
            }
            for (Obstacle o : remove) {
                c.getPlayer().getMap().removeMapObject((MapleMapObject) o);
            }
        }
    }
