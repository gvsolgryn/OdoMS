...
import org.extalia.handling.RecvPacketOpcode;
...
import org.extalia.provider.MapleDataProvider;
...
import org.extalia.server.events.MapleSnowball;
import org.extalia.server.field.boss.will.SpiderWeb;
import org.extalia.server.field.skill.*;
import org.extalia.server.games.BloomingRace;
import org.extalia.server.games.DetectiveGame;
...
import org.extalia.server.polofritto.FrittoDancing;
import org.extalia.server.polofritto.FrittoEagle;
import org.extalia.server.quest.MapleQuest;
import org.extalia.tools.*;
...
public class PlayerHandler {
    ...
    public static final void TakeDamage(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if (type > -2) {
            ...
            if (chr.getBuffedValue(SecondaryStat.SilhouetteMirage) != null) {
                SecondaryStatEffect eff = SkillFactory.getSkill(400031053).getEffect(chr.getSkillLevel(400031053));
                if (damage >= chr.getStat().getCurrentMaxHp() / 100 * eff.getY()) {
                    if (chr.getSkillCustomValue0(400031053) > 0) {
                        int 데미지감소 = damage * eff.getQ() / 100;
                        damage -= 데미지감소;
                        chr.setSkillCustomInfo(400031053, chr.getSkillCustomValue0(400031053) - 1, 0);
    ...
    public static final void SpecialMove(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (skillid == 400021032) {
            SecondaryStatEffect eff = SkillFactory.getSkill(400021033).getEffect(c.getPlayer().getSkillLevel(400021033));
        ...
        if (skillid == 5011007) { // 몽키와 함께라면!
            ...
            chr.getMap().broadcastMessage(CField.MonkeyTogether(113, c.getPlayer().getKeyValue(7786, "sw") == 1));
        }
        if (skillid == 5311013) {
        ...
        switch (skillid) {
            ...
            case 400001043:
                chr.메이플용사 --;
                HashMap<SecondaryStat, Pair<Integer, Integer>> statups = new HashMap<SecondaryStat, Pair<Integer, Integer>>();
                statups.put(SecondaryStat.Bless5th2, new Pair<Integer, Integer>(chr.메이플용사, 0));
                break;
        }
            ...
            switch (skillid) {
                case 11121018: {
                    ...
                    while (i < chr.getCosmicCount()) {
                        ...
                        atoms2.add(new SecondAtom(0x2A, c.getPlayer().getId(), mobid, 420 + delay, skillid, 2000, 0, 1, new Point(x + x_[i], y + y_[i]), Arrays.asList(0, 0)));
                        ++i;
                        delay += 60;
                    }
                ...
                case 400021122: {
                    ...
                    for (int j = 0; j < 7 + ((skillid == 400021122) ? 0 : size); ++j) {
                        ...
                        atoms2.add(new SecondAtom(0x16, c.getPlayer().getId(), mobid2, 1140, skillid, 4000, 0, 1, new Point(ret.plusPosition2.x + Randomizer.rand(-200, 200), ret.plusPosition2.y + Randomizer.rand(-350, -50)), Arrays.asList(new Integer[0])));
                ...
                case 162121019: {
                    ...
                    for (int l = 0; l < ((skillid == 162101011) ? 4 : 5); ++l) {
                        atoms3.add(new SecondAtom(0x15, c.getPlayer().getId(), 0, 0, skillid, 4000, 0, 1, new Point(pos2.x + Randomizer.rand(-250, 250), pos2.y + Randomizer.rand(-460, 0)), Arrays.asList(unks3, unk2)));
                    }
                ...
                case 162121012: {
                    ...
                    if (skillid == 162101003) {
                        ...
                        for (int m = 0; m < 3; ++m) {
                            skills2.add(new RangeAttack(162121014, c.getPlayer().getPosition(), 0, 0, 8));
                            skills2.add(new RangeAttack(162121013, c.getPlayer().getPosition(), 0, 0, 8));
                            skills2.add(new RangeAttack(162121013, c.getPlayer().getPosition(), 0, 0, 8));
                        }
                    }
                    effect.applyTo(chr, pos);
                ...
                case 400001064: {
                    pos = slea.readPos();
                    final MapleSummon summon = new MapleSummon(chr, 400001064, chr.getTruePosition(), SummonMovementType.STATIONARY, (byte) 0, effect.getDuration());
                    chr.getMap().spawnSummon(summon, effect.getDuration());
                    chr.addSummon(summon);
                    effect.applyTo(chr);
                    break;
...
//                }
                ...
                case 400041058: {
                    ...
                    Integer[] custom = {-45, -70, -135, -165, -150, -35, -55, -50, -160, -48, -140, -167, -40, -170, -52,
                            -10, -15, -170, -165, -175, -5, -20, -25, -160, -30, -155, -150, -32, -177, -38};
                    List<AdelProjectile> atoms = new ArrayList<AdelProjectile>() {
                        {
                            int i = 0;
                            for (Pair<Integer, Integer> object : ret.attackObjects) {
                                MapleMapObject obj = chr.getMap().getMapObject(object.left, MapleMapObjectType.MONSTER);
                                for (int a = 0; a < object.right; ++a) {
                                    add(new AdelProjectile(9, chr.getId(), object.left, 400041058, 10000, custom[i], 1, obj.getTruePosition(), Arrays.asList(custom)));
                                    ++i;
                                }
                                if (i >= custom.length) {
                                    break;
                ...
                case 400051074: {
                    ...
                    effect.applyTo(chr);
                   // chr.dropMessageGM(6, "전송됨2"); @렉
                    break;
                }
                case 400001021: {
                    SecondaryStatEffect Memorize = SkillFactory.getSkill(c.getPlayer().unstableMemorize).getEffect(c.getPlayer().getSkillLevel(c.getPlayer().unstableMemorize));
                    c.getSession().writeAndFlush((Object) CField.unstableMemorize(chr.unstableMemorize));
                    Memorize.applyToBuff(chr);
                    c.getSession().writeAndFlush((Object) CWvsContext.enableActions(chr, true, false));
                    break;
                }
                // 블러드 피스트
                ...
                default: {
                    ...
                    if (skillid == 131001025) {
                        ...
                        if (skillid == 35111002) {
                            ...
                            chr.dropMessageGM(6, "type4 : " + type4);
                            if (type4 == 1) {
                                pos = slea.readPos();
        ...
        if (attack.skill != 0) {
            ...
            if (attack.skill == 61121105 || attack.skill == 61121222 || attack.skill == 24121052) {
                ...
                SkillFactory.getSkill(attack.skill).getEffect(attack.skilllevel).applyTo(chr, attack.position, false);
                    ...
                    && attack.skill != 400011087 && attack.skill != 11121052 && attack.skill != 11121055 && attack.skill != 11121056 && attack.skill != 400011056 && attack.skill != 400011142
                     && !energy) {
            ...
            if (chr.getSkillLevel(80002416) > 0) {
                ...
                if (weapon_item != null) {
                    ...
                    if (weapon_name != null && weapon_name.startsWith("아스칼론 ") && !chr.getBuffedValue(1221054) && !chr.skillisCooling(1221054) && c.getPlayer().getSkillCustomValue0(1221054) == 0L) {
                        ...
                        SecondaryStatEffect effcts2 = SkillFactory.getSkill(32121017).getEffect(chr.getSkillLevel(32121017));
                        chr.addBuffCheck = 1;
                        effcts2.applyTo(chr);
                    }
                }
            ...
            if (GameConstants.isStriker(chr.getJob())) {
            ...
            if (GameConstants.isKhali(chr.getJob())) {
                if (!chr.getBuffedValue(150030241)) { // 이네이트 기프트
                    SecondaryStatEffect eff = SkillFactory.getSkill(150030241).getEffect(chr.getSkillLevel(150030241));
                    eff.applyTo(chr, eff.getDuration());
                    chr.getClient().getSession().writeAndFlush((Object) CField.EffectPacket.showEffect(chr, 0, 150030241, 1, 0, 0, (byte) 0, true, chr.getTruePosition(), null, null));
                }
    ...
    public static final void rangedAttack(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) { // 원거리 공격
        ...
        if (attack.skill == 4121017) {
            if (c.getPlayer().getCooldownLimit(4121020) == 0) {
                final List<MapleMapObject> objs = c.getPlayer().getMap().getMapObjectsInRange(new Point(attack.position.x + ((attack.display & 0x8000) == 0 ? 230 : -230), attack.position.y), 200000, Arrays.asList(MapleMapObjectType.MONSTER));
                ...
                for (int i = 0; i < 6; i++) {
                    ...
                    AdelProjectile sword = new AdelProjectile(0x21, c.getPlayer().getId(), objs.size() == 0 ? 0 : objs.get(Randomizer.nextInt(objs.size())).getObjectId(), 4121020, durations[i], (attack.facingleft & 0x8000) == 0 ? startXs[i] : (-startXs[i]), 1, new Point(attack.position.x + ((attack.facingleft & 0x8000) == 0 ? diff.x : (-diff.x)), attack.position.y + diff.y), new ArrayList<>());
        ...
        if (!AOE && chr.getBuffedValue(SecondaryStat.SoulArrow) == null && !noBullet) { // 여기서 이걸 스킵해줘야하는데 스킵이 안되니까 겉뎀나는거임 에러나서
    ...
    public static final void MagicDamage(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr, boolean chilling, boolean orbital) {
        ...
        if (chr.getSkillLevel(2100000) > 0 || chr.getSkillLevel(2200000) > 0 || chr.getSkillLevel(2300000) > 0) {
            if (attack.targets > 0) {
                if (Randomizer.isSuccess(30)) {
                    double mp = c.getPlayer().getStat().getCurrentMaxMp(chr) * 0.05;
            ...
            if (orbital && attack.targets > 0 && attack.skill != 12120010) {
                ...
                if (stack == 10) {
                    ...
                    if (finalMobList.size() > 0) {
                        c.getSession().writeAndFlush(CField.bonusAttackRequest(12101030, finalMobList, false, 0, new int[0]));
                    }
...
/*        if (ServerConstants.ConnectorSetting && !chr.isGM()) {
            if (getConnectorCheckLong() > 100) {
                if (!c.getConnectorCheck()) { // Check Connector Connection status
                    c.getSession().close();
                    return;
                }
                resetConnectorCheckLong();
            } else {
                getConnectorCheckLong();
            }
        }*/
        if (c.getPlayer().isGM()) {
    ...
    public static final void UpdateMedalDisplay(LittleEndianAccessor slea, MapleCharacter chr) {
        ...
        chr.setKeyValue(100, "medal", state + "");
        chr.getClient().getSession().writeAndFlush(CField.showMedalDisplay(chr, (byte) chr.getKeyValue(100, "medal")));
        chr.updateInfoQuest(101149, "1007=" + chr.getKeyValue(100, "medal") + ";1009=" + chr.getKeyValue(100, "title"));
    ...
    public static final void UpdateTitleDisplay(LittleEndianAccessor slea, MapleCharacter chr) {
        byte state = slea.readByte();
        chr.setKeyValue(100, "title", state + "");
        chr.getClient().getSession().writeAndFlush(CField.showTitleDisplay(chr, (byte) chr.getKeyValue(100, "title")));
        chr.updateInfoQuest(101149, "1007=" + chr.getKeyValue(100, "medal") + ";1009=" + chr.getKeyValue(100, "title"));
        chr.reloadChar();
    }
    ...
    public static void OrbitalFlame(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (flame != null && chr.getSkillLevel(elementid) > 0) {
            SecondaryStatEffect element = SkillFactory.getSkill(elementid).getEffect(chr.getSkillLevel(elementid));
        ...
        if (chr.getBuffedEffect(SecondaryStat.AddRange) != null) {
            atom.setnArriveRange(flame.getRange() + chr.getBuffedValue(SecondaryStat.AddRange).intValue());
        } else {
            atom.setnArriveRange(flame.getRange());
        }
        atom.setnArriveDir(direction);
        chr.getMap().spawnMapleAtom(atom);
        chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, 0, 12120006, 1, 0, 0, (byte) (chr.isFacingLeft() ? 1 : 0), true, chr.getTruePosition(), null, null));
    ...
    public static void MatrixSkill(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (enable2) {
            ...
            data.add(slea.readInt());
            data.add((int) slea.readByte()); // 1149++
        }
...
/*                chr.addCoolTime(12120022, 30000);
    ...
    public static void UpdateSymbol(LittleEndianAccessor slea, MapleClient c, int plus) {
        try {
            ...
            //아 이거 구글에는 공식이 안나오네 완화된거같은데 나중에 체크만
    ...
    public static void activePrayBuff(MapleClient c) {
        ...
        if (effect != null) {
            ...
            if (incPMdR > 0) {
                statups.put(SecondaryStat.IndiePmdR, new Pair<>(at + Integer.valueOf(incPMdR), Integer.valueOf(2000)));
            }
            if (incBooster > 0) {
    ...
    public static void bless5th(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (skill == 400001050 ) {
            ...
            localstatups.put(SecondaryStat.Bless5th2, new Pair<>(stack, Integer.valueOf((int) chr.getBuffLimit(400001050))));
        ...
        if (skill == 400001042 || skill == 400001043) { //여신의 축복
            SecondaryStatEffect yoyo = SkillFactory.getSkill(400001042).getEffect(1);
            statups.put(SecondaryStat.Bless5th2, new Pair<Integer, Integer>(stack, 0));
            c.getSession().writeAndFlush((Object) CWvsContext.BuffPacket.giveBuff(statups, yoyo, c.getPlayer()));
            c.getSession().writeAndFlush(CField.V_BLESS(skill, check));
        }
    }
    ...
    public static void eventUIResult(LittleEndianAccessor slea, MapleClient c) {
        ...
        switch (type) {
            ...
            case 41: {
                idx = slea.readInt();
                if (idx == 1200) {
                    NPCScriptManager.getInstance().start(c, 3001604, "UI08"); // 후원
                }
                // NPCScriptManager.getInstance().start(c, 3001604, "UI01"); // 후원
                if (idx == 1400) {
                ...
                if (idx == 1100) { // 윗줄 라인
                    NPCScriptManager.getInstance().start(c, 3001604, "UI04"); // 워프
                } else if (idx == 1101) {
                    NPCScriptManager.getInstance().start(c, 3001604, "UI02"); // 편의
                } else if (idx == 1102) {
                    NPCScriptManager.getInstance().start(c, 3001604, "UI06"); // 상점
                }

                if (idx == 1103) { // 윗줄 라인
                    NPCScriptManager.getInstance().start(c, 3001604, "UI05"); // 워프
                } else if (idx == 1104) {
                    NPCScriptManager.getInstance().start(c, 3001604, "UI07"); // 제작
                } else if (idx == 1105) {
                    NPCScriptManager.getInstance().start(c, 3001604, "UI03"); // 상점
                }
    ...
    public static void DojangHandler(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (type == 0x65) {
            ...
            int[][] monsterList = {
                    // 보스반감, 보스, 반감, 일반
                    {9834020, 9834021, 9834022, 9834023},
                    {9834024, 9834025, 9834026, 9834027},
    ...
    public static void vSkillSpecial(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (skillId) {
            ...
            case 400021089: //어비셜 라이트닝
                ...
                final List<SpecialPortal> sp = new ArrayList<SpecialPortal>();
                final SpecialPortal s3 = new SpecialPortal(chr.getId(), 2, 400021089, chr.getMapId(), pos_x, pos_y , (int) (chr.getBuffedValue(400021088) ? chr.getBuffLimit(400021088) : effect.getDuration()));
        ...
        switch (skillid) {
            ...
            case 80003046:
                ...
                if ((c.getPlayer().getMap().isTown() || !c.getPlayer().getMap().isLevelMob(c.getPlayer())) && !c.getPlayer().getBuffedValue(skillid)) {
                    ...
                    if (skillid == 80003046 && c.getPlayer().getKeyValue(100794, "today") >= ((Calendar.getInstance().get(7) == 7 || Calendar.getInstance().get(7) == 1) ? 6000L : 3000L)) {
    ...
    public static final void handlePoisonRegion(final LittleEndianAccessor slea, final MapleClient c) {
