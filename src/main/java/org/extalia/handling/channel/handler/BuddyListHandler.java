        ...
        if (mode == 1) {
            ...
            if (!buddylist.isFull()) {
                ...
                if (ch > 0) {
                    ...
                    if (otherName != null) {
                        ...
                        buddylist.put(ble);
                        c.getSession().writeAndFlush(CWvsContext.BuddylistPacket.updateBuddylist(buddylist.getBuddies(), ble, (byte) 21));
                        notifyRemoteChannel(c, otherChar.getClient().getChannel(), otherChar.getId(), BuddyList.BuddyOperation.ADDED, otherMemo);
                    }
                }
            }
            ...
            if (ch > 0) {
                ...
                if (buddylist.containsVisible(accId) && otherChar != null) {
                    notifyRemoteChannel(c, otherChar.getClient().getChannel(), otherChar.getId(), BuddyList.BuddyOperation.DELETED, otherMemo);
                }
                if (otherChar != null) {
