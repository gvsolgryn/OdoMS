...
import org.extalia.client.inventory.AuctionHistory;
import org.extalia.client.inventory.AuctionItem;
...
import org.extalia.handling.auction.handler.AuctionHistoryIdentifier;
...
import org.extalia.server.life.MapleMonsterInformationProvider;
import org.extalia.tools.CurrentTime;
  ...
  public void run() {
            ...
            while (chrs.hasNext()) {
             ...
             MapleCharacter chr = chrs.next();
                ...
                if (chr.isAlive()) {
                    ...
                    for (int k = 0; k < curEffects2.size(); k++) {
                        if (prevEffects2.get(k) != curEffects2.get(k)) {
    ...
    if ((new Date()).getHours() == 21 && (new Date()).getMinutes() == 0 && !this.give) {
      ...
      for (MapleCharacter chr2 : chrs) {
        ...
        chr2.dropMessage(1, "[HOT] Heinz 핫타임 - 접속 보상이 지급되었습니다. 메이플 보관함을 확인해주세요.");
        chr2.dropMessage(6, "[HOT] Heinz 핫타임 - 접속 보상이 지급되었습니다. 메이플 보관함을 확인해주세요.");
      } 
      ...
      this.give = false;
        ...
        this.give = false;
        //길드 대항전 의뢰
    } else if ((((new Date()).getHours() == 19 && (new Date()).getMinutes() == 00) ||  ((new Date()).getHours() == 22 && (new Date()).getMinutes() == 00)) && !this.worldboss) {
        worldboss = true;
        for (ChannelServer cserv : ChannelServer.getAllInstances()) {
            for (MapleCharacter player : cserv.getPlayerStorage().getAllCharacters().values()) {
                player.getClient().getSession().writeAndFlush(SLFCGPacket.OnYellowDlg(2012041, 3500, "월드보스 팀대항전이 시작되었다네 참여를 하고 싶으면 나에게 말을 걸어 주게나.", ""));
            }
        }
    } else if ((((new Date()).getHours() == 20 && (new Date()).getMinutes() == 00) ||  ((new Date()).getHours() == 23 && (new Date()).getMinutes() == 00)) && !this.worldboss) {
        worldboss = true;
        int outmap = 100000000;
        EventManager em = null;
        EventInstanceManager eim = null;
        for (ChannelServer cserv : ChannelServer.getAllInstances()) {
            //if (cserv.getChannel() == 1) {
            //전 채널 소환 1마리
            ...
            for (MapleCharacter player : cserv.getPlayerStorage().getAllCharacters().values()) {
                if(player.getMapId() == 993059200){
                    em = player.getClient().getChannelServer().getEventSM().getEventManager("WorldBoss");
                    if (em == null) {
    ...
    } else if ((((new Date()).getHours() == 19 && (new Date()).getMinutes() == 01) ||  ((new Date()).getHours() == 22 && (new Date()).getMinutes() == 01)) && this.worldboss == true) {
        this.worldboss = false;
    } else if ((((new Date()).getHours() == 20 && (new Date()).getMinutes() == 01) ||  ((new Date()).getHours() == 23 && (new Date()).getMinutes() == 01)) && this.worldboss == true) {
        this.worldboss = false;
    } 
