...
// 
package org.extalia.handling.netty;
...
import org.extalia.handling.auction.handler.AuctionHandler;
import org.extalia.handling.cashshop.handler.CashShopOperation;
import org.extalia.handling.channel.handler.*;
import org.extalia.handling.farm.handler.FarmHandler;
...
import org.extalia.tools.MapleAESOFB;
...
import org.extalia.tools.packet.CSPacket;
...
import static org.extalia.constants.ServerType.LOGIN;
...
public class MapleNettyHandler extends SimpleChannelInboundHandler<LittleEndianAccessor> {
    ...
    public MapleNettyHandler(final ServerType serverType, final int channel) {
      ...
      /*  if(this.serverType == LOGIN) {
            System.out.println("[debug] 핸들러에 로그인 서버가 등록되었습니다.");
        }
        else if(this.serverType == ServerType.CHANNEL) {
            System.out.println("[debug] 핸들러에 채널 서버가 등록되었습니다.");
        }
        else if(this.serverType == ServerType.CASHSHOP) {
            System.out.println("[debug] 핸들러에 캐시샵 서버가 등록되었습니다.");
        }
        else if(this.serverType == ServerType.BUDDYCHAT) {
            System.out.println("[debug] 핸들러에 친구 서버가 등록되었습니다.");
        }
        else if(this.serverType == ServerType.AUCTION) {
            System.out.println("[debug] 핸들러에 경매장 서버가 등록되었습니다.");
        }
        else if(this.serverType == ServerType.FARM) {
            System.out.println("[debug] 핸들러에 농장 서버가 등록되었습니다.");
        }
        else {
            System.out.println("[debug] 핸들러에 명시되지 않은 함수가 등록되었습니다.");
        }*/
    }
    ...
    protected void channelRead0(final ChannelHandlerContext ctx, final LittleEndianAccessor slea) throws Exception {
        ...
        if (show &&
                ServerConstants.DEBUG_RECEIVE
                && header_num != RecvPacketOpcode.HACKSHIELD.getValue()
             //   && header_num != RecvPacketOpcode.SPECIAL_MOVE.getValue()
                ...
                && header_num != RecvPacketOpcode.SHOW_ICBM.getValue()
