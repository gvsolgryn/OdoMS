...
import org.extalia.client.custom.inventory.CustomItem;
import org.extalia.client.damage.CalcDamage;
import org.extalia.client.inventory.*;
import org.extalia.client.status.MonsterStatus;
import org.extalia.client.status.MonsterStatusEffect;
import org.extalia.constants.GameConstants;
import org.extalia.constants.KoreaCalendar;
import org.extalia.constants.ServerConstants;
import org.extalia.database.DatabaseConnection;
import org.extalia.database.DatabaseException;
...
import org.extalia.handling.channel.handler.PlayerHandler;
import org.extalia.handling.login.LoginInformationProvider;
import org.extalia.handling.login.LoginServer;
import org.extalia.handling.world.*;
import org.extalia.handling.world.guild.MapleGuild;
import org.extalia.handling.world.guild.MapleGuildCharacter;
import org.extalia.handling.world.party.MapleParty;
import org.extalia.handling.world.party.MaplePartyCharacter;
import org.extalia.handling.world.party.PartyOperation;
import org.extalia.provider.MapleData;
import org.extalia.provider.MapleDataProviderFactory;
import org.extalia.provider.MapleDataTool;
...
import org.extalia.server.Timer;
import org.extalia.server.events.MapleBattleGroundCharacter;
import org.extalia.server.events.MapleTyoonKitchen;
import org.extalia.server.field.boss.MapleBossManager;
import org.extalia.server.field.boss.lucid.FieldLucid;
import org.extalia.server.field.skill.MapleFieldAttackObj;
import org.extalia.server.field.skill.MapleMagicWreck;
import org.extalia.server.field.skill.MapleSecondAtom;
import org.extalia.server.field.skill.SecondAtom;
import org.extalia.server.games.*;
import org.extalia.server.life.*;
import org.extalia.server.maps.*;
import org.extalia.server.marriage.MarriageMiniBox;
import org.extalia.server.movement.LifeMovementFragment;
import org.extalia.server.polofritto.*;
import org.extalia.server.quest.MapleQuest;
import org.extalia.server.quest.party.MapleNettPyramid;
import org.extalia.server.shops.IMaplePlayerShop;
import org.extalia.server.shops.MapleShop;
import org.extalia.tools.FileoutputUtil;
import org.extalia.tools.Pair;
import org.extalia.tools.StringUtil;
import org.extalia.tools.Triple;
import org.extalia.tools.data.MaplePacketLittleEndianWriter;
import org.extalia.tools.packet.*;
    ...
    public void levelUp() {
        ...
        if (this.level > 300) { // 계승자
            ...
            if (getSuccessorLevel() == 10) {
                level = 300;
                setAwakenLevel(1);
            } else if (getSuccessorLevel() == 100) {
                setAwakenLevel(2);
            } else if (getSuccessorLevel() == 200) {
                setAwakenLevel(3);
            } else if (getSuccessorLevel() == 300) {
                setAwakenLevel(4);
                ...
                setAwakenLevel(8);
            } else if (getSuccessorLevel() == 800) {
                setAwakenLevel(9);
            } else if (getSuccessorLevel() == 900) {
                setAwakenLevel(10);
            }
        }
        if (this.level >= 200) {
...
//    }
    ...
    private class MapleCharacterManagement implements Runnable {
        ...
        public void handleEtcs(final long time) {
            ...
            //티어 사냥터 입장 조건
            if (MapleCharacter.this.getMapId() == 100) {
                if (!MapleCharacter.this.haveItem(1143197) && !MapleCharacter.this.haveItem(1143197)) { //아이템을 안 가졌을때
            ...
            if (MapleCharacter.this.getMapId() == 106) {
                if (!MapleCharacter.this.haveItem(1143198) && !MapleCharacter.this.haveItem(1143198)) {
                    MapleMap mapz = ChannelServer.getInstance(MapleCharacter.this.getClient().getChannel()).getMapFactory().getMap(ServerConstants.warpMap);
                    MapleCharacter.this.changeMap(mapz, mapz.getPortal(0));
                }
            }
            if (MapleCharacter.this.getMapId() == 107) {
                if (!MapleCharacter.this.haveItem(1143198) && !MapleCharacter.this.haveItem(1143198)) {
                    MapleMap mapz = ChannelServer.getInstance(MapleCharacter.this.getClient().getChannel()).getMapFactory().getMap(ServerConstants.warpMap);
                    MapleCharacter.this.changeMap(mapz, mapz.getPortal(0));
                }
            }
            if (MapleCharacter.this.getMapId() == 108) {
                if (!MapleCharacter.this.haveItem(1143199) && !MapleCharacter.this.haveItem(1143199)) {
                    MapleMap mapz = ChannelServer.getInstance(MapleCharacter.this.getClient().getChannel()).getMapFactory().getMap(ServerConstants.warpMap);
                    MapleCharacter.this.changeMap(mapz, mapz.getPortal(0));
                }
            }
            if (MapleCharacter.this.getMapId() == 104) {
                if (!MapleCharacter.this.haveItem(1143199) && !MapleCharacter.this.haveItem(1143199)) {
                    MapleMap mapz = ChannelServer.getInstance(MapleCharacter.this.getClient().getChannel()).getMapFactory().getMap(ServerConstants.warpMap);
                    MapleCharacter.this.changeMap(mapz, mapz.getPortal(0));
                }
