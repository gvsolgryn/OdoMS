package org.extalia.server.games;

import org.extalia.client.MapleCharacter;
import org.extalia.client.MapleCoolDownValueHolder;
...
public class MultiYutGame {
    ...
    public class MultiYutPlayer {
        ...
        private List<PlayersHorses> Horses;

        private List<PlayersSkill> skilllist;
        ...
        public MultiYutPlayer(MapleCharacter player, byte position) {
            ...
            for (i = 0; i < 4; i++) {
                PlayersHorses horse = new PlayersHorses((byte) i);
                this.Horses.add(horse);
            }
            for (i = 0; i < 2; i++) {
                PlayersSkill skill = new PlayersSkill(Randomizer.rand(0, 7));
        ...
        public void MoveHorse(int horsePos, int movecount, int yuttype) {
            PlayersHorses horse = getHorses().get(horsePos);
            ...
            if (horse != null && horse.getInvposition() == horsePos) {
                ...
                if (horse.getOverlap().size() > 0) {
                    for (PlayersHorses hor : getHorses()) {
                ...
                for (PlayersHorses ophor : MultiYutGame.this.getOpponent(getChr()).getHorses()) {
        ...
        public List<PlayersHorses> getHorses() {
            return this.Horses;
        }

        public void setHorses(List<PlayersHorses> Horses) {
            this.Horses = Horses;
        }

        public List<PlayersSkill> getSkilllist() {
            return this.skilllist;
        }

        public void setSkilllist(List<PlayersSkill> skilllist) {
