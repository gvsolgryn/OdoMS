...
public class MonsterPyramid {
    ...
    public static void StartGame(int n) {
        if (monsterPyramidMatchingQueue.size() == n) {
            ...
            org.extalia.server.Timer.EtcTimer.getInstance().schedule(() -> {
    ...
    private void startTimer(PyramidPlayer pp) {
        this.MonsterPyramidTimer = org.extalia.server.Timer.EventTimer.getInstance().schedule(() -> skipPlayer(pp), 11000L);
    }
    ...
    public void nextStage(int ntturn, boolean used) {
        ...
        if (this.round >= 4) {
            ...
            org.extalia.server.Timer.EtcTimer.getInstance().schedule(() -> endGame(null, false), 4000L);
        } else {
            pyramidcheck.clear();
            for (int i = 0; i < 8; i++) {
                for (int i2 = 0; i2 < 8 - i; i2++) {
