...
public class MapleMiniGame extends AbstractPlayerStore {
  ...
  public void setPoints(int slot) {
    this.points[slot] = this.points[slot] + 1;
    checkWin();
  }
  ...
  public void checkExitAfterGame() {
    for (int i = 0; i < 2; i++) {
      if (this.exitAfter[i]) {
        this.exitAfter[i] = false;
        ((i == 0) ? getMCOwner() : this.chrs[i - 1].get()).getClient().getSession().writeAndFlush(PlayerShopPacket.shopErrorMessage(0, i));
        if (i == 0 && getShopType() != 1) {
          closeShop(false, isAvailable());
        } else {
          ((i == 0) ? getMCOwner() : this.chrs[i - 1].get()).setPlayerShop(null);
          removeVisitor(this.chrs[i - 1].get());
        } 
      } 
    } 
  }
