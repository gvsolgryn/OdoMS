...
import org.extalia.client.MapleQuestStatus;
import org.extalia.client.StructPotentialItem;
...
public class MapleItemInformationProvider {
    ...
    public String getPotentialName(Item item, int id) {
        String msg = "";
        int level = getReqLevel(item.getItemId()) / 10;
        if (level >= 20) {
            level -= 8;
        }
        msg += getPotentialInfo(id).get(level);
        return msg;
    }
    ...
    public final int getReqJob(int itemId) {
        if (getEquipStats(itemId) == null || !getEquipStats(itemId).containsKey("reqJob") || itemId == 1190999) {
            return 0;
        }
