...
import org.extalia.client.*;
import org.extalia.client.inventory.*;
import org.extalia.constants.GameConstants;
import org.extalia.constants.ServerConstants;
import org.extalia.handling.SendPacketOpcode;
import org.extalia.handling.channel.handler.AttackInfo;
import org.extalia.handling.channel.handler.PlayerInteractionHandler;
import org.extalia.handling.world.World;
import org.extalia.handling.world.guild.MapleGuild;
import org.extalia.handling.world.guild.MapleGuildAlliance;
import org.extalia.scripting.EventInstanceManager;
import org.extalia.server.*;
import org.extalia.server.enchant.EnchantFlag;
import org.extalia.server.enchant.StarForceStats;
import org.extalia.server.field.skill.*;
import org.extalia.server.life.MapleHaku;
import org.extalia.server.life.MapleMonster;
import org.extalia.server.life.MapleNPC;
import org.extalia.server.maps.ForceAtom;
import org.extalia.server.maps.*;
import org.extalia.server.movement.LifeMovementFragment;
import org.extalia.server.quest.MapleQuest;
import org.extalia.server.shops.MapleShop;
import org.extalia.tools.AttackPair;
import org.extalia.tools.HexTool;
import org.extalia.tools.Pair;
import org.extalia.tools.Triple;
...
public class CField {
    ...
    public static class NPCPacket {
        ...
        public static byte[] spawnNPC(MapleNPC life, boolean show) {
            ...
            mplew.writeInt(0);
            mplew.write(0);
            mplew.writeInt(0); // 1.2.371 ++
            mplew.writeInt(-1);
            mplew.writeLong(PacketHelper.getTime(-2L)); // 1.2.371 ++
            mplew.writeInt(0);
            mplew.writeInt(0);
            mplew.writeMapleAsciiString("");
            return mplew.getPacket();
        ...
        public static byte[] getNPCTalkStyle(int npc, String talk, List<Integer> args) {
            ...
            for (int i = 0; i < args.size(); i++) {
                mplew.writeInt(((Integer) args.get(i)).intValue());
            }
            return mplew.getPacket();
        }
...
/*            mplew.write(5);
    ...
    public static byte[] getCharInfo(MapleCharacter chr) {
        ...
        mplew.write(0);
        mplew.writeLong(PacketHelper.getTime(-2L));
        mplew.write(false); //  CWvsContext::SetWhiteFadeInOut
        mplew.write(false); //  pChatBlockReason
        mplew.write(0); //355
    ...
    public static byte[] spawnPlayerMapobject(MapleCharacter chr) {
        ...
        mplew.write((chr.getKeyValue(100, "medal") == 1 ? 1 : 0)); // 훈장표시
        mplew.write((chr.getKeyValue(100, "title") == 1 ? 1 : 0)); // 칭호표시
    ...
    public static byte[] getChatText(MapleCharacter chr, String text, boolean whiteBG, int show, Item item) {
        ...
        mplew.writeLong(0); // 364 new
        mplew.writeShort(0); // 364 new
        mplew.write(show);
        mplew.write(0);
        if (item != null) {
            mplew.write(1);
...
//        mplew.write(-1); //274 ++
    ...
    public static class AuctionPacket {
        ...
        public static byte[] AuctionMarketPrice(List<AuctionItem> items) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
            mplew.writeShort(SendPacketOpcode.AUCTION.getValue());
            //mplew.write(HexTool.getByteArrayFromHexString("29 00 00 00 E8 03 00 00 00 00 00 00 01 00 21 00 00 00 DD 03 00 00 00 00 00 00 03 00 00 00 00 00 00 00 00 F2 05 2A 01 00 00 00 00 00 00 00 00 00 00 00 00 F2 05 2A 01 00 00 00 00 F2 05 2A 01 00 00 00 00 00 00 20 5F A0 F2 41 00 28 26 45 2A B2 9D 01 00 28 26 45 2A B2 9D 01 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00
00 00 00 80 05 BB 46 E6 17 02 FF FF FF FF 01 3D 07 00 00 01 4A 00 4A 00 9A 00 4A 00 19 00 19 00 69 00 1C 00 00 00 09 8A BA 90 B0 07 00 00 00 00 01 00 00 00 00 14 00 96 9C 5B 75 86 75 56 7D 1B 56 20 56 00 00 0B 9F 0A 00 00 00 00 00 00 00 00 00 00 00 00 00 00 40 E0 FD 3B 37 4F 01 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 FF FF 00 80 05 BB 46 E6 17 02 00 40 E0 FD 3B 37 4F 01 00 80 05 BB 46 E6 17 02 01 04 00 00 00 00 00 00 03 00 00 00 00 00 00 00 61 4B 94 95 00 00 00 00 00 00 00 00 00 00 00 00 30 22 CE 0B 1C 00 00 00 61 4B 94 95 00 00 00 00 00 00 20 6C 89 B2 E2 41 00 28 26 45 2A B2 9D 01 00 28 26 45 2A B2 9D 01 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 E0 6F CC E2 D9 37 D9 01 01 00 00 00 00 00 00 00 00 F2 03 00 00 D9 0D 00 00 01 00 56 02 EA 0E 3D 00 00 00 80 05 BB 46 E6 17 02 FF FF FF FF 00 30 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00"));
            mplew.writeInt(41);
            mplew.writeInt(1000);
            mplew.writeInt(0);
    ...
    public static byte[] RuneAction(int type, int time) {
        ...
        if (type == 8 || type == 9) {
            ...
            mplew.writeShort(1);
            String[] test = {
                    "37 30 88 CF 28 00 00 00 35 50 F1 A0 3A A7 E4 BD 81 4C 10 11 E0 6E EC 4C DF 50 5D CE 93 C8 34 36 EC F6 2A 10 15 E9 EA 7E B8 47 0F 02 2C BA CE 0A",
                    "8F 9A B9 A6 28 00 00 00 A2 67 73 C9 B8 AE 07 D4 31 F2 BC 9E 32 A7 DD FD 4F AC 2F A5 E7 3C B2 B8 A0 0C 7F A3 E6 4E BE 3E 68 2E 4B 22 1D 8A B3 F3",
                    "79 8B F6 47 28 00 00 00 35 08 7E 37 2C 31 8D 34 41 6B 13 79 2E 97 C2 0F F9 E3 A7 83 03 B7 BF F0 11 4C D1 54 AA 18 1A 22 C2 6A 44 E6 38 7B F9 A8",
                    "81 BB B6 FE 28 00 00 00 6A B9 AD 91 7A 82 B8 8C C8 E9 4F 49 01 2C 67 7D FB 07 63 93 85 FB 91 20 F4 F2 94 34 39 7D 5B FA 36 59 A0 A9 B1 7B 45 F1",
                    "89 D7 E1 DC 28 00 00 00 3A 7B BF B3 4B 34 B0 AE DF 52 35 39 84 74 BB D1 A2 36 9F E0 96 10 EC 1D F7 05 63 03 42 65 A6 19 A9 74 C4 BC 6F 39 87 44",
                    "4F 6B B7 BB 28 00 00 00 C3 6A 16 D4 D5 53 23 C9 B7 CF F7 1A 9F 63 90 B5 54 F4 88 9A 34 62 90 72 9E 48 11 59 DE A9 14 2E 25 D2 22 99 54 89 94 DA",
                    "06 89 62 B1 28 00 00 00 56 22 D7 DE 5B 6B CA C3 18 4C 45 C7 AA 1F B3 B3 F9 B4 DD 43 B9 6B BA F4 E2 8E 92 4D F2 62 86 02 4B B8 4F EB 45 2C CB 39",
                    "E7 4D 5A 9F 28 00 00 00 20 F6 83 F0 2C BF 96 ED 0A C7 E1 42 16 08 9C 2A 33 90 2B A2 6A AE DD 08 51 D7 4B 00 1B 6E 1F D5 C0 A7 B7 59 13 BC CE 85",
                    "C2 0B 0B 44 28 00 00 00 89 51 9B 34 8C 9A 88 37 A2 32 6C 7F 11 FE 49 38 57 47 41 2D 3A 90 A3 65 39 73 B9 18 81 7E 8A FB FA 77 79 8E C2 D2 6F D0",
                    "6E 73 EB 28 28 00 00 00 12 EA 2D 58 10 A3 52 5B 6B 92 76 A6 6D 45 F8 1F 47 14 38 0E 47 CE A8 FA 03 6B 43 43 F9 5C 32 E1 32 E6 23 D6 F9 2A 18 3E",
                    "C2 0B 0B 44 28 00 00 00 89 51 9B 34 8C 9A 88 37 A2 32 6C 7F 11 FE 49 38 57 47 41 2D 3A 90 A3 65 39 73 B9 18 81 7E 8A FB FA 77 79 8E C2 D2 6F D0",
            };
    ...
    public static byte[] rangeAttack(int firstSkill, List<RangeAttack> skills) {
        ...
        for (RangeAttack skill : skills) {
            ...
            packet.writeInt(0);
            packet.writeInt(0);
            packet.writeInt(0); // 1149++
//            if (firstSkill != 400041040) {
//                packet.writeInt(0); // 1149++
//   }
        }
        return packet.getPacket();
    ...
    public static byte[] createAtom(MapleAtom atom) {
        ...
        mplew.writeInt(atom.getnForceAtomType()); // 이 값이 바뀐거임 스킬별로 안되는거 맞쳐주면 될거임
    ...
    public static byte[] spawnAdelProjectiles(MapleCharacter chr, List<AdelProjectile> tiles, boolean infinity) { // 이게 제꺼임
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        for (AdelProjectile tile : tiles) {
            mplew.writeInt(tile.getObjectId());
            mplew.writeInt(tile.getSkillId() == 5201017 ? 11 : tile.getSkillId() == 400021122 ? 8 : tile.getSkillId() == 4121020 ? 2 : tile.getSkillId() == 63101104 || tile.getSkillId() == 162111005 || tile.getSkillId() == 2121052 ? 1 : 0); //342
            mplew.writeInt(tile.getProjectileType());
            mplew.writeInt(i);
            ...
            mplew.writeInt(tile.getSkillId() == 63101104 || tile.getSkillId() == 400031063 || tile.getSkillId() == 4121020 ? tile.getStartX() : 0);
            mplew.writeInt(tile.getSkillId());
            mplew.writeInt(0);
            mplew.writeInt(tile.getSkillId() == 4121020 ? 0x1E : tile.getSkillId() == 162111002 ? 20 : tile.getSkillId() == 400051069 || tile.getSkillId() == 162101000 || tile.getSkillId() == 2121052 || tile.getSkillId() == 5201017 ? 1 : 0);
            mplew.writeInt(tile.getDuration());
            mplew.writeInt(tile.getStartX());
            mplew.writeInt(tile.getStartY());
            mplew.writeInt(tile.getSkillId() == 162101000 || tile.getSkillId() == 2121052 ? 0 : tile.getSkillId() == 400011047 ? 0x1 : 0);
    ...
    public static byte[] updateGuildScore(int guildScore) {
        ...
        mplew.writeInt(guildScore); // guildScore
        return mplew.getPacket();
    }
    ...
    public static byte[] PartyRankingInfo(List<String> info) {
        ...
        for (int i = 0; i < info.size(); i++) {
            ...
            packet.writeMapleAsciiString(player[2]); // 계승레벨
