...
import org.extalia.server.MapleChatEmoticon;
import org.extalia.server.MapleItemInformationProvider;
import org.extalia.server.MapleSavedEmoticon;
...
import org.extalia.server.shops.*;
...
public class PacketHelper {
    ...
    public static final void addCharStats(MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
        ...
        if (chr.getBaseColor() != -1) {
            ...
            int basehair = 0;
            hair = (chr.getHair() / 10 * 10 + chr.getBaseColor()) * 1000;
            hair += chr.getAddColor() * 100;
            hair += chr.getBaseProb();
            mplew.writeInt(hair);
        } else {
    ...
    public static final void addCharacterInfo(MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
        ...
        //mplew.writeShort(0); 1.2.371 삭제됨
        mplew.writeShort(0);
        ...
        //mplew.write(HexTool.getByteArrayFromHexString("0F 03 01 00 FF FF FF FF 01 00 FF FF FF FF 01 00 FF FF FF FF 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00"));
        //mplew.write(HexTool.getByteArrayFromHexString("0F 03 01 00 FF FF FF FF 01 00 FF FF FF FF 01 00 FF FF FF FF 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00"));
        //mplew.write(HexTool.getByteArrayFromHexString("06 03 01 00 FF FF FF FF 01 00 FF FF FF FF 01 00 FF FF FF FF 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00"));
    ...
    public static void encodeForRemote(MaplePacketLittleEndianWriter mplew, Map<SecondaryStat, Pair<Integer, Integer>> statups, MapleCharacter chr) {
        ...
        if (statups.containsKey(SecondaryStat.ReturnTeleport)) {
            ...
            mplew.writeInt(chr.getBuffSource(SecondaryStat.ReturnTeleport)); // 일단 두개잡음
        }
        if (statups.containsKey(SecondaryStat.CapDebuff)) {
            mplew.writeShort(chr.getBuffedSkill(SecondaryStat.CapDebuff));
            mplew.writeInt(chr.getBuffSource(SecondaryStat.CapDebuff)); // 얘도 잡음
        }
        ...
        if (statups.containsKey(SecondaryStat.CrystalGate)) { // 1143 기준 644 아무리봐도 CrystalGate는 아님;
