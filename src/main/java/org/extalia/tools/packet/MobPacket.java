...
import org.extalia.client.AvatarLook;
...
import org.extalia.server.field.boss.FieldSkill;
...
public class MobPacket {
    ...
    public static byte[] showBossHP(MapleMonster mob) {
       ...
       // System.out.println("지점 : " + (mob.getHp() + mob.getBarrier()));
        mplew.writeLong(mob.isEliteboss() ? mob.getElitehp() : (mob.getStats().getHp() * (long) mob.bonusHp()));
       // System.out.println("지점 : " + mob.getStats().getHp());
    ...
    public static byte[] spawnMonster(MapleMonster life, int spawnType, int link, int control) {
        ...
        if (life.getStats() != null
                && life.getStats().getName() != null
                ) {

            if  (life.getStats().getName().contains("자폭")) {
                flying = true;
            }
            switch (life.getStats().getName()) {
                case "어둠의 집행자":
                    flying = true;
                    break;
            }
        ...
        if (life.getEliteGrade() >= 0) {
            ...
            for (Pair<Integer, Integer> info : (Iterable<Pair<Integer, Integer>>) life.getEliteGradeInfo()) {
                mplew.writeInt(((Integer) info.left).intValue());
                mplew.writeInt(((Integer) info.right).intValue());
            }
        ...
        for (Triple<String, Integer, Integer> skeleton : (Iterable<Triple<String, Integer, Integer>>) life.getStats().getSkeleton()) {
...
//        }
    ...
    public static byte[] createEnergySphere(int oid, int skillLevel, List<MapleEnergySphere> mese) {
        ...
        if (mes.isOk()) {
            ...
            mplew.write(mes.isDelayed()); // mes.isDelayed()
