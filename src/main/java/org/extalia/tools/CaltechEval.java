...
public class CaltechEval {
  ...
  private Vector toTokens(String s) {
    ...
    while (i < s.length()) {
      ...
      if (c == 'e' && nd > 0 && i < s.length() - 2 && (cnext == '+' || cnext == '-' || 
        inOperand(cnext))) {
        operand = operand + c;
      ...
      operand = "";
      operator = operator + c;
      if (operator.equals("+") || operator.equals("-") || operator
  ...
  private double eval(String s) {
    ...
    return Double.parseDouble(tokens.elementAt(0));
  }
  ...
  public Vector reduceTokens(Vector<String> tokens) {
    ...
    while (tokens.indexOf("(") != -1) {
      ...
      for (int it = ib + 1; it < tokens.size(); it++) {
        ...
        if (brackets == 0) {
          ...
          int ip = ib;
          for (int j = 0; j < bracketTokens.size(); j++)
            tokens.setElementAt(bracketTokens.elementAt(j), ip++); 
    ...
    while (tokens.size() > 1) {
      ...
      for (it = 0; it < tokens.size(); it++) {
        String str = tokens.elementAt(it);
        int i = getOperator(str);
        if (i != -1)
          if (this.OperatorsPrecedence[i] >= maxprec) {
            maxprec = this.OperatorsPrecedence[i];
            ipos = it;
          }  
      } 
      if (ipos == -1)
      ...
      if (this.OperatorsLeftRight[iop]) {
        if (!st.equals(",")) {
          int ipt = it - 1;
          if (it > 0) {
            String stleft = tokens.elementAt(it - 1);
          ...
          String str = tokens.elementAt(it + 1);
          double d1 = Double.parseDouble(str);
          double d2 = doOp(st, leftValue, d1);
          str = "" + d2;
          tokens.setElementAt(str, ipt);
