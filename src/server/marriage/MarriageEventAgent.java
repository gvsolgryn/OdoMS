...
public class MarriageEventAgent {
    ...
    public void startEvent() {
        ...
        hall.broadcastMessage(MaplePacketCreator.getClock(1200)); //맞는지는 모르겠음
        setCanEnter(false);
        ...
        Runnable run = new Runnable() {
            ...
            public void run() {
                ...
                if (count == 0) {
                    ...
                    count++;
                    String str = "제가 보기에는 이 두 사람은 그 리본의 끝에 있는 서로를 찾아낸 것 같습니다.";
                    hall.broadcastMessage(MaplePacketCreator.startMapEffect(str, 5120025, true));
                    ...
                    count++;
                    String str = "신랑, 검은 머리가 예티의 털처럼 하얗게 변할때까지 신부를 사랑하시겠습니까?";
                    hall.broadcastMessage(MaplePacketCreator.startMapEffect(str, 5120025, true));
                } else if (count == 6 && time >= 60000) {
                    count++;
                    String str = "신부, 엘나스산의 만년설이 모두 녹아 니할사막처럼 될 때까지 신랑을 사랑하시겠습니까?";
                    hall.broadcastMessage(MaplePacketCreator.startMapEffect(str, 5120025, true));
                } else if (count == 7 && time >= 70000) {
                    count++;
                    String str = "오늘 모인 하객분들이 두 사람의 맹세에 대한 증인이 되어주십시오.";
                    hall.broadcastMessage(MaplePacketCreator.startMapEffect(str, 5120025, true));
                } else if (count == 8 && time >= 80000) {
                    count++;
                    String str = "모두의 축복 속에 두 젊은이가 부부가 되었음을 선포합니다.";
                    hall.broadcastMessage(MaplePacketCreator.startMapEffect(str, 5120025, true));
                } else if (count == 9 && time >= 90000) {
                    setEndedCeremony(true);
                    count++;
                    String str = "신랑은 신부에게 키스해도 좋습니다."; //TODO: 하트 이펙트 출력
                    hall.broadcastMessage(MaplePacketCreator.startMapEffect(str, 5120025, true));
                } else if (count == 10 && time >= 110000) {
                    count++;
