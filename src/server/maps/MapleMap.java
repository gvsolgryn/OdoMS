...
import server.life.DemianSword;
       ...
       /* if (chr.getDateKey("questB4", false) != null) {
        ...
        if (mobid == 8820008 || mobid == 8820108) {
            ...
            for (MapleCharacter mchr : getCharacters()) {
                ...
                mchr.dropMessage(5, "루시드가 분노했습니다!");
            }
            ...
            for (MapleCharacter mchr : getCharacters()) {
                ...
                mchr.dropMessage(5, "검은마법사가 분노했습니다!");
            }
        ...
        } else if (mobid == 8800002 || mobid == 8800102 || mobid == 8800022) { // 자쿰 일때 근데 이 조건문을 닫는게
            killAllMonsters(true);
        } // 이게 여기 있어야하는데
        ...
        || mobid == 8880503) {
            if (chr.getParty() != null) {
                for (MapleCharacter mchr : chr.getMap().getCharacters()) {
                    if (mchr != null) {
                        mchr.Message("클리어를 축하드립니다! `@보스` 명령어를 치고 나가시면 됩니다.");
                        if (mobid == 8880150) {
                ...
                "[서버보스] : " + chr.getParty().getLeader().getName() + "님의 파티가 " + monster.getStats().getName() + "을(를) 처치하였습니다."));
            }
        }
                ...
                if (st.getCustomData().equals("1")) {
                    ...
                    if (st.getMobKills(mobid) >= 5) {
                        chr.getClient().sendPacket(
                                MainPacketCreator.getQuestMessage("직업 클레스 수업 퀘스트를 완료 하였습니다. 허드레를 클릭 해 주세요."));
                    }
                }
            }
        ...
        if (!this.isEliteBossMap() && !monster.isEliteMonster() && !monster.isEliteBoss()
                && !monster.getStats().isBoss() && monster.getStats().getLevel() - 20 <= chr.getLevel()
                && chr.getLevel() <= monster.getStats().getLevel() + 20) {
        ...
        if (monster.getId() == 8920000) {
                spawnMobDrop(new Item(4310034, (byte) 0, (short) 160, (byte) 0), calcDropPos(new Point(0, monster.getPosition().x), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }
        if (monster.getId() == 8900000) {
                spawnMobDrop(new Item(4310034, (byte) 0, (short) 170, (byte) 0), calcDropPos(new Point(0, monster.getPosition().x), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }
        if (monster.getId() == 8880000) {
                spawnMobDrop(new Item(4310034, (byte) 0, (short) 180, (byte) 0), calcDropPos(new Point(0, monster.getPosition().x), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }
        if (monster.getId() == 8500002) {
                spawnMobDrop(new Item(4310034, (byte) 0, (short) 190, (byte) 0), calcDropPos(new Point(0, monster.getPosition().x), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }  
        if (monster.getId() == 9801029) {
        ...
        if (monster.getId() == 9101078) {
                spawnMobDrop(new Item(4032101, (byte) 0, (short) 60, (byte) 0), calcDropPos(new Point(0, monster.getPosition().x), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        } 
        ...
        if (monster.getId() == 9300890) {
                spawnMobDrop(new Item(4032101, (byte) 0, (short) 100, (byte) 0), calcDropPos(new Point(0, monster.getPosition().x), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }  
