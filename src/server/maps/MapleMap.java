...
public final class MapleMap {
    ...
    public int moonstak = 0, Monstermarble = 0;

    public int mooncake = 0;

    public int rpportal = 0, RPTicket = 0;
...
/*            if (MapleMap.this.elitetime > 0 && MapleMap.this.isElitebossmap()) {
    ...
    private void dropFromMonster(MapleCharacter chr, MapleMonster mob, boolean instanced) {
        ...
        if (chr.getParty() != null) {
            if (mob.isHellMode() || mob.isExtreme() ) {
                int size = chr.getParty().getMembers().size();
                int quantity = size == 1 ? 6 : size == 2 ? 5 : size == 3 ? 4 : size == 4 ? 3 : size == 5 ? 2 : size == 6 ? 1 : 0;
                switch (mob.getId()) {
                    case 8880405: // 진 힐라
                    case 8645066: // 듄켈
                    case 8644655: // 더스크
                    case 8880518: // 검은 마법사 보상 페이지
                    case 8880614: { // 세렌 보상 페이지
                        if (mob.isHellMode()) {
                            customs.add(new MonsterDropEntry(4036582, 9999999, quantity, quantity, 0, 0));
                        } else if (mob.isExtreme()){
                            customs.add(new MonsterDropEntry(4036060, 9999999, quantity, quantity, 0, 0));
                        }
        ...
        switch (mob.getId()) {
            ...
            case 9300453:
                ...
                break;
            case 9300173:
                customs.add(new MonsterDropEntry(4001161, 10000000, 1, 1, 0));
                break;
            case 9300175:
            ...
            case 9300005:
            case 9300006:
                customs.add(new MonsterDropEntry(4001022, 10000000, 1, 1, 0));
                break;
        }
        ...
        if (chr.getV("arcane_quest_7") != null && Integer.parseInt(chr.getV("arcane_quest_7")) >= 0 && Integer.parseInt(chr.getV("arcane_quest_7")) < 4) {
            ...
            if (itemId != 0) customs.add(new MonsterDropEntry(itemId, 5, 1, 1, 0));
        ...
        for (MonsterDropEntry de : finals) {
            ...
            if (chr.IsAutoHuntMap() == -1) { //일반 자동사냥 드롭률
               d1 = d1 / 5;
    ...
    public final void QuestMobKill (final MapleMonster monster, final MapleCharacter chr, final int qnum) {
        if(monster.getId() == chr.getKeyValue(qnum, "mobid")){
            if(chr.getKeyValue(qnum, "mobkill") < chr.getKeyValue(qnum, "mobcount")-1){
                chr.setKeyValue(qnum, "mobkill", ""+(chr.getKeyValue(qnum, "mobkill")+1));
            } else {
                chr.setKeyValue(qnum, "mobkill", ""+(chr.getKeyValue(qnum, "mobkill")+1));
                chr.setKeyValue(qnum, "info", "2");
            }
        }
    }
    ...
    public final void killMonster(final MapleMonster monster, final MapleCharacter chr, boolean withDrops, boolean second, byte animation, int lastSkill) {
        ...
        //QuestId
            ...
            for (int i = 0; i < questNum.length; i++) {
                if(chr.getKeyValue(questNum[i], "info") == 1){
                    QuestMobKill(monster, chr, questNum[i]);
                }
            }
        ...
        EventManager em2 = chr.getClient().getChannelServer().getEventSM().getEventManager("KerningPQ");
            ...
            if (monster.getId() == 9300912) {
                ...
                broadcastMessage(CField.achievementRatio(100));
            }
            ...
            if (monster.getId() == 9300010) {
                ...
                if (this.RPTicket == 4) {
                    ...
                    broadcastMessage(CField.environmentChange("Map/Effect.img/quest/party/clear", 4));
                    broadcastMessage(CField.achievementRatio(50));
                }
            }
            ...
            if (monster.getId() == 9300008 || monster.getId() == 9300014) {
                ...
                if (q == 0) {
                    broadcastMessage(CField.environmentChange("Map/Effect.img/quest/party/clear", 4));
                    broadcastMessage(CField.achievementRatio(25));
                    broadcastMessage(CField.startMapEffect("다크아이와 쉐도우 아이를 모두 퇴치하였습니다. 엘로그린 벌룬에게 말을 걸어 다음 스테이지로 이동해주세요!", 5120018, true));
                }
            }
        ...
        if (chr.getPlayer().getKeyValue(100711, "point") > 9999) { // 사냥 포인트
            ...
            if (chr.erdacount == 100) {
                chr.erdacount = 0;
                chr.getPlayer().setKeyValue(100711, "point", "" + (chr.getPlayer().getKeyValue(100711, "point") + 1));
            }
        }
        ...
        if (GameConstants.isKhali(chr.getJob()) ) {
            ...
            if ( chr.getBuffedValue(154111000) || monster.getStats().isBoss()) {
        ...
        if (this.mapid == 930000100 && getAllMonstersThreadsafe().size() == 0) {
            broadcastMessage(CField.environmentChange("Map/Effect.img/quest/party/clear", 4));
            broadcastMessage(CField.achievementRatio(15));
        }

        
        if (chr.getV("ClearPassKillMonsterAmount") != "1") {
            if (chr.getV("Pass_kill_Monster_amount") != null) {
                int trueLevel = 20;
                boolean MonsterLevelisHigh = (monster.getStats().getLevel() - trueLevel) > chr.getLevel();
                boolean CharacterLevelisLow = chr.getLevel() < (monster.getStats().getLevel() - trueLevel);
                if (!MonsterLevelisHigh || !CharacterLevelisLow) {
                    if (chr.getPassKillMonsterAmount() != 5000) {
                        chr.addPassKillMonsterAmount(1);
                        chr.getClient().getSession().writeAndFlush(CField.UIPacket.detailShowInfo("[끼룩 패스] 레벨범위 몬스터 : " + chr.getPassKillMonsterAmount() + " / 5000", false));
                    }
                }
            }
        }
        ...
        if (this.Monstermarble == 20) {
            ...
            broadcastMessage(CField.environmentChange("Map/Effect.img/quest/party/clear", 4));
            broadcastMessage(CField.startMapEffect("모든 몬스터 구슬을 획득하였습니다. 파티장이 엘린에게 말을 걸어 다음 스테이지로 이동하여 주세요.", 5120023, true));
            broadcastMessage(CField.achievementRatio(50));
        }

        if (monster.getId() == 9300182) {
            broadcastMessage(CField.environmentChange("Map/Effect.img/quest/party/clear", 4));
            broadcastMessage(CField.startMapEffect("포이즌 골렘을 퇴치하였습니다. 오른쪽의 포탈로 퇴장하실 수 있습니다.", 5120023, true));
            broadcastMessage(CField.achievementRatio(100));
        }

        if (this.mapid == 921160200 && getAllMonstersThreadsafe().size() == 0) {
            broadcastMessage(CField.environmentChange("Map/Effect.img/quest/party/clear", 4));
            broadcastMessage(CField.achievementRatio(20));
        } else if (this.mapid == 921160400 && getAllMonstersThreadsafe().size() == 0) {
            broadcastMessage(CField.environmentChange("Map/Effect.img/quest/party/clear", 4));
            broadcastMessage(CField.achievementRatio(50));
        }
        if (monster.getId() == 9300454) {
            broadcastMessage(CField.environmentChange("Map/Effect.img/quest/party/clear", 4));
            broadcastMessage(CField.achievementRatio(100));
            spawnNpc(9020006, new Point(-1639, -181));
        }
        //파티퀘스트
        if (partyquest == 1) {
        ...
        if (monster.isExtreme() || monster.isHellMode()) {
            ...
            int prob = 300; // 확률 1% = 10
            switch (monster.getId()) {
                case 8644655: // 더스크
                    itemArray = new int[]{1113132, 1162084, 1132309}; // 드롭 원하는 아이템 선생님이 아이템 코드만 넣으면 끝임다
                    break;
                case 8880405: // 진 힐라
                ...
                case 8880614: // 세렌
                    ...
                    break;
                default: {
                    //  itemArray = new int[]{ㅂ4031013};
                    break;
                }
            }
            if (itemArray.length > 0) {
                int itemid = 0;
                int randItem = Randomizer.rand(0, itemArray.length - 1);
                if (itemArray[randItem] <= 0) {
                    itemid = itemArray[0];
                } else {
                    itemid = itemArray[randItem];
                }

                Item toDrop;
                final MapleItemInformationProvider ii7 = MapleItemInformationProvider.getInstance();
                if (GameConstants.getInventoryType(itemid) == MapleInventoryType.EQUIP) {
                    toDrop = ii7.getEquipById(itemid);
                } else {
                    toDrop = new Item(itemid, (short) 0, (short) 1, 0);
                }

                if (toDrop != null) {
                    if (prob >= rand) {
                     //   spawnItemDrop(monster, chr, toDrop, new Point((monster.getTruePosition()).x, (monster.getTruePosition()).y), true, false);
                        spawnMobPublicDrop(toDrop, monster.getTruePosition(), monster, chr, (byte) 0, 0);
                    }
                }
...
//                        }
//                        if (quantity > 0) {
//                            toDrop = new Item(4319995, (short) 0, (short) quantity, 0);
//                            spawnItemDrop(monster, chr, toDrop, new Point((monster.getTruePosition()).x, (monster.getTruePosition()).y), false, false);
//                        }
//                        break;
//                    }
//                }
...
//                chr.checkLiveQuest(1, false);
            ...
            if (monster.getId() == 8880151 && chr.getClient().getChannelServer().getEventSM().getEventManager("Normal_Lucid").getName() == "Normal_Lucid") {
            ...
            if (monster.getId() == 8880803) {
                World.Broadcast.broadcastMessage(CField.getGameMessage(8, "[" + chr.getParty().getLeader().getName() + "]님의 파티가 감시자 칼로스를 격파하였습니다."));
            }
            if (monster.getId() == 9400080) {
                World.Broadcast.broadcastMessage(CField.getGameMessage(8, "[" + chr.getParty().getLeader().getName() + "]님의 파티가 텐구를 격파하였습니다."));
            }
            if (monster.getId() == 9460026) {
                World.Broadcast.broadcastMessage(CField.getGameMessage(8, "[" + chr.getParty().getLeader().getName() + "]님의 파티가 진격의 거인을 격파하였습니다."));
            }
            if (monster.getId() == 9601623) {
                World.Broadcast.broadcastMessage(CField.getGameMessage(8, "[" + chr.getParty().getLeader().getName() + "]님의 파티가 아케치 미츠히데를 격파하였습니다."));
            }
...
/*                        if (getEliteCount() >= 10) {
            ...
            FileoutputUtil.log("Log/Log_Kill.txt", "" + e);
        }
    ...
    public boolean isSpawnMonster(int id) {
        boolean a = true;
        List<MapleMonster> monsters = this.getAllMonster();
        for(MapleMonster monster : monsters) {
            if(monster.getId() == id) return a;
        }
        return !a;
    }
...
/*        spawnAndAddRangedMapObject(portal, c -> {
    ...
    public final void spawnSummon(final MapleSummon summon, final int duration, MapleCharacter chr) {
        summon.updateMap(this);
        summon.setStartTime(System.currentTimeMillis());
        spawnAndAddRangedMapObject(summon, new DelayedPacketCreation() {
            public void sendPackets(MapleClient c) {
        ...
        if ((duration > 0 && duration < Integer.MAX_VALUE && summon.getSkill() != 400051046 && summon.getSummonType() != 7) || (summon.getSkill() == 400021068 && summon.getMovementType() != SummonMovementType.SUMMON_JAGUAR)) {
            Timer.MapTimer.getInstance().schedule(new Runnable() {
                public void run() {
                    summon.removeSummon(map, false, false, chr);
                }
            }, (duration + 1000));
        }
    }
...
//        }*/
 ...
 //       }
        ...
        if (chr.getParty() != null) {
            ...
            chr.getClient().getSession().writeAndFlush(CWvsContext.PartyPacket.updateParty(chr.getClient().getChannel(), chr.getParty(), PartyOperation.SILENT_UPDATE, null, chr.getPlayer()));
        ...
        public void run() {
            if (this.mapitem != null && this.mapitem == MapleMap.this.getMapObject(this.mapitem.getObjectId(), this.mapitem.getType()) && !this.mapitem.isPickedUp()) {
                ...
                if (this.reactor.getDelay() > 0) {
                    Timer.MapTimer.getInstance().schedule(new Runnable() {
                        public void run() {
                            ActivateItemReactor.this.reactor.forceHitReactor((byte) 0, ActivateItemReactor.this.c.getPlayer().getId());
                        }
    ...
    public void respawn(boolean force, long now) {
        ...
        if (force) {
            ...
            int numShouldSpawn = (GameConstants.isForceRespawn(this.mapid) ? this.monsterSpawn.size() : this.maxRegularSpawn) - this.spawnedMonstersOnMap.get();
            if (numShouldSpawn > 0) {
                int spawned = 0;
