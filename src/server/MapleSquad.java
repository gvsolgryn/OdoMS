...
public class MapleSquad {
    ...
    public static enum MapleSquadType {
        ...
        bossbalrog(1), zak(1), chaoszak(1), horntail(1), chaosht(1), pinkbean(1), nmm_squad(1), vergamot(1), dunas(1), nibergen_squad(1), dunas2(1), core_blaze(1), aufheben(1), cwkpq(10), tokyo(1), vonleon(1), scartar(1), cygnus(1), showa(1), murim(1), tokyopq(1);
        ...
        while (type.queue.get(ch).size() > 0 && ChannelServer.getInstance(ch).getMapleSquad(type) == null) {
            ...
            if (theirCh > 0) {
                ...
                if (lead != null && lead.getMapId() == beginMapId && lead.getClient().getChannel() == ch) {
                    ...
                    if (lead != null) {
                        lead.dropMessage(6, "올바른 채널과 맵에 있지 않아 속한 원정대를 스킵했습니다.");
                    }
        ...
        if (join) {
            if (!containsMember(member) && !getAllNextPlayer().contains(member.getName())) {
                if (members.size() <= 30) {
                    ...
                    getLeader().dropMessage(5, member.getName() + " (" + job + ") (이)가 가입 했습니다. 건투를 빕니다!");
                    return 1;
                }
                return 2;
            }
            ...
            if (containsMember(member)) {
                ...
                getLeader().dropMessage(5, member.getName() + " (" + job + ") (이)가 탈퇴 했습니다.");
                return 1;
            }
            return -1;
        }
    ...
    public String getSquadMemberString(byte type) {
        switch (type) {
            case 0: {
                StringBuilder sb = new StringBuilder("원정대에 가입한 멤버 수 : ");
                sb.append("#b").append(members.size()).append(" #k ").append("참가자 목록 : \n\r ");
                int i = 0;
            ...
            case 1: {
                StringBuilder sb = new StringBuilder("원정대에 가입한 멤버 수 : ");
                sb.append("#b").append(members.size()).append(" #n ").append("참가자 목록 : \n\r ");
                int i = 0, selection = 0;
                for (Entry<String, String> chr : members.entrySet()) {
                    i++;
