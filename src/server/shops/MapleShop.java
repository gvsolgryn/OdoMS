...
public class MapleShop {
   ...
   if (quantity < 0) {
          WorldBroadcasting.broadcastMessage(MainPacketCreator.getGMText(7, c.getPlayer().getName() + " 님이 복사핵 사용으로 자동으로 밴 당하셨습니다."));
            c.getSession().close();
            c.getPlayer().ban("음수 복사핵 (판매)", false, false);
            return;
        }
        int x = 0, index = -1;
   ...
   if (quantity < 0) {
            WorldBroadcasting.broadcastMessage(MainPacketCreator.getGMText(21, c.getPlayer().getName()+"님이 복사핵 사용으로 자동으로 밴 당하셨습니다."));
            c.getSession().close();
            c.getPlayer().ban("음수 복사핵 (구매)", false, false);
            return;
        }
        ...
        if (quantity <= iQuant && iQuant > 0) {
            ...
            if (quantity < 0) {
                ban(c, c.getPlayer());
                WorldBroadcasting.broadcastMessage(MainPacketCreator.getGMText(21, "[알림] " + c.getPlayer().getName() + " 님이 복사핵 사용으로 자동으로 밴 당하셨습니다."));
            ...
            if (recvMesos >= Integer.MAX_VALUE) {
                c.getPlayer().dropMessage(1, "오류가 발생 하였습니다. 잠시 후 다시 시도해 주세요.");
                return;
            }
    ...
    public void ban(MapleClient c, MapleCharacter chr) {
        ...
        if (target != null) {
            ...
            if (MapleCharacter.ban(chr.getName(), sb.toString(), false)) {
                c.getPlayer().dropMessage(6, chr.getName() + " 오프라인 밴 성공.");
            } else {
                c.getPlayer().dropMessage(6, chr.getName() + " 를 밴 하는데 실패했습니다.");
            }
        }
    }
}
