...
public class EnforceSystem {
    ...
    public static void scrollstaticvoid(int job, int itemtype, int level) {
        ...
        if (itemtype == 121 || itemtype == 126 ||itemtype == 137 || itemtype == 138) {
        	a2 = true;
        } else if (itemtype > 120 && itemtype < 172) {//a = 무기
            a = true;
        ...
        try {
            ...
            for (int i = 0; i < scrollstring.length; i++) {
                    ...
                    switch (scrollstatic(scrollstring[i])) {
                            ...
                            case 9: // 무기(덱스)
                            case 33: // 무기(럭)
                                    packet.writeInt(scrollstring[i].startsWith("30%") ? 7 : 9); // 공격력
                                    packet.writeInt(scrollstring[i].startsWith("30%") ? (scrollstring[i].contains("HP") ? 150 : 3) : (scrollstring[i].contains("HP") ? 200 : 4)); // 주스탯
                                    break;
                            default:
