...
public class InventoryManipulator {
    ...
    public static boolean addbyItem(final MapleClient c, IItem item) {
        ...
        if (GameConstants.isArcaneSymbol(item.getItemId())) { //아케인심볼
    ...
    public static int addById(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, long period, boolean justice, String log) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (quantity == 1) {
                ...
                if (GameConstants.isArcaneSymbol(nEquip.getItemId())) { //아케인심볼
            ...
            if (type == MapleInventoryType.ETC && dst > 100 && dst % 100 != 0) {
                ...
                if (eSlot > 0) {
                    ...
                    if (dst % 100 > ee.getSkillStats().getStats("slotCount")
                        ...
                        c.getPlayer().dropMessage(1, "가방에 넣을 수 없는 아이템 입니다. " + ee.getSkillStats().getStats("type")
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
            ...
            case -1204: { // Bottom
                ...
                if (top != null && GameConstants.isOverall(top.getItemId())) { // 한벌옷을 낀채로 하의를 입을때
                    if (chr.getInventory(MapleInventoryType.EQUIP)
                            .isFull(bottom != null && GameConstants.isOverall(top.getItemId()) ? 1 : 0)) { // 낀 상의가 한벌옷이면 여유공간이 두칸은 필요.
                        c.getSession().writeAndFlush(MainPacketCreator.getInventoryFull());
                        c.getSession().writeAndFlush(MainPacketCreator.getShowInventoryFull());
                        return;
                    }
            ...
            if (android != null) {
                ...
                if (ItemFlag.ANDROID_ACTIVATED.check(flag)) {
                    if (android.getAndroid() != null) {
                        ...
                        System.err.println("[오류] " + chr.getName() + "의 " + android.getUniqueId()
                                + " 유니크 아이디의 안드로이드에서 NULL 포인터가 발생했습니다.");
                        return;
                    }
                } else {
        ...
        if (source.getItemId() == 1142145) {
            ...
            c.getPlayer().dropMessage(5, "프리미엄 PC방 훈장 착용으로 인해 몬스터 사냥시 보너스 경험치 50%를 추가로 획득하게 됩니다.");
        }
            ...
            if (heart != null) {
                ...
                if (ItemFlag.ANDROID_ACTIVATED.check(flag)) {
                    if (source.getAndroid() != null) {
                        ...
                        chr.dropMessage(1, "오류가 발생했습니다!");
                        System.err.println("[오류] " + chr.getName() + "의 " + source.getUniqueId()
                                + " 유니크 아이디의 안드로이드에서 NULL 포인터가 발생했습니다.");
                        return;
                    }
                } else {
