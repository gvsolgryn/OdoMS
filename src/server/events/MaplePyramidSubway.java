...
public class MaplePyramidSubway {
    ...
    public final void commenceTimerNextMap(final MapleCharacter c, final int stage) {
        ...
        if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
            for (MaplePartyCharacter mpc : c.getParty().getMembers()) {
                ...
                if (chr != null) {
                    chr.getClient().getSession().writeAndFlush(MainPacketCreator.getClock(time));
                    chr.getClient().getSession()
                            .writeAndFlush(MainPacketCreator.showMapEffect("killing/first/number/" + stage));
                    chr.getClient().getSession().writeAndFlush(MainPacketCreator.showMapEffect("killing/first/stage"));
                    chr.getClient().getSession().writeAndFlush(MainPacketCreator.showMapEffect("killing/first/start"));
                    fullUpdate(chr, stage);
                }
            }
        } else {
            c.getClient().getSession().writeAndFlush(MainPacketCreator.getClock(time));
            c.getClient().getSession().writeAndFlush(MainPacketCreator.showMapEffect("killing/first/number/" + stage));
            c.getClient().getSession().writeAndFlush(MainPacketCreator.showMapEffect("killing/first/stage"));
            c.getClient().getSession().writeAndFlush(MainPacketCreator.showMapEffect("killing/first/start"));
            fullUpdate(c, stage);
        }
        if (type != -1 && (stage == 4 || stage == 5)) { // yetis. temporary
            for (int i = 0; i < (stage == 4 ? 1 : 2); i++) {
                ...
                ourMap.spawnMonsterOnGroundBelow(MapleLifeProvider.getMonster(9700023), c.getPosition());
            }
        }
    ...
    public final void succeed(final MapleCharacter c) {
        ...
        c.getClient().getSession().writeAndFlush(MainPacketCreator.showMapEffect("killing/clear"));
        c.getClient().getSession().writeAndFlush(MainPacketCreator.sendPyramidResult(rank, exp));
        dispose(c);
    }
    ...
    public final void broadcastUpdate(final MapleCharacter c) {
        c.getClient().getSession().writeAndFlush(MainPacketCreator.sendPyramidUpdate(bar));
    }

    public final void broadcastEffect(final MapleCharacter c, final String effect) {
        c.getClient().getSession().writeAndFlush(MainPacketCreator.showMapEffect(effect));
    }

    public final void broadcastEnergy(final MapleCharacter c, final String type, final int amount) {
        // c.getClient().getSession().writeAndFlush(MainPacketCreator.sendPyramidEnergy(type,
    ...
    private static void changeMap(final MapleCharacter c, final MapleMap map, final int minLevel, final int maxLevel,
        ...
        if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
            for (MaplePartyCharacter mpc : c.getParty().getMembers()) {
                ...
                if (chr != null && chr.getId() != c.getId() && chr.getLevel() >= minLevel && chr.getLevel() <= maxLevel) {
                    if (clear == 1) {
                        chr.getClient().getSession().writeAndFlush(MainPacketCreator.showMapEffect("killing/clear"));
                    } else if (clear == 2) {
                        chr.getClient().getSession().writeAndFlush(MainPacketCreator.showMapEffect("killing/fail"));
                    }
        ...
        if (clear == 1) {
            ...
            c.getClient().getSession().writeAndFlush(MainPacketCreator.showMapEffect("killing/fail"));
        }
