...
//        }
    ...
    public void out(MapleCharacter chr) {
//        int map = chr.getSavedLocation(SavedLocationType.EVENT);
//        if (map <= -1) {
//            map = 104000000;
//        }
        final MapleMap mapp = chr.getClient().getChannelServer().getMapFactory().getMap(109050001);
        chr.changeMap(mapp, mapp.getPortal(0));
    }

    public void winOut(MapleCharacter chr) {
//        int map = chr.getSavedLocation(SavedLocationType.EVENT);
//        if (map <= -1) {
//            map = 104000000;
//        }
        final MapleMap mapp = chr.getClient().getChannelServer().getMapFactory().getMap(109050000);
        chr.changeMap(mapp, mapp.getPortal(0));
    }
            ...
            for (MapleEventType t : MapleEventType.values()) {
                ...
                if (e.isRunning) {
                    for (int i : e.type.mapids) {
                        if (cserv.getEvent() == i) {
                            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(0, "이벤트 참가 모집이 종료되었습니다."));
                            e.broadcast(MaplePacketCreator.serverNotice(0, "이벤트가 30초 후 시작됩니다!"));
            ...
            if (e.isRunning) {
                for (int i : e.type.mapids) {
                    if (chr.getMapId() == i) {
                        ...
                        chr.dropMessage(5, String.valueOf(t) + " 이벤트가 시작되었습니다.");
                    }
                }
            }
        }
    ...
    public static final String scheduleEvent(final MapleEventType event, final ChannelServer cserv) {
        ...
        World.Broadcast.broadcastMessage(MaplePacketCreator.yellowChat("GM 이벤트가 잠시 후 " + (i == 1 ? "1" : i == 2 ? "20세이상" : i - 1) + " 채널에서 시작됩니다! 해당 채널의 리스항구, 사우스페리, 오르비스, 루디브리엄에서 이벤트 NPC를 통해 이벤트 참가가 가능합니다!"));
        return "";
    }
}
