...
public class MapleDojoAgent {
    ...
    public static boolean warpNextMap(final MapleCharacter c, final boolean fromResting, final MapleMap currentmap) {
        try {
                ...
                if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
                    for (MaplePartyCharacter mem : c.getParty().getMembers()) {
                        ...
                        if (chr != null) {
                            ...
                            chr.getQuestNAdd(MapleQuest.getInstance(GameConstants.DOJO))
                                    .setCustomData(String.valueOf(dojo));
                            // chr.getClient().getSession().writeAndFlush(WvsContext.Mulung_Pts(point,
                            // dojo));
                        }
                    }
                } else {
                    ...
                    // c.getClient().getSession().writeAndFlush(WvsContext.Mulung_Pts(point, dojo));
                }
            ...
            if (currentmap.getId() == 925020001) {// 925030100 925020001
                ...
                final MapleMap mappzz = ch.getMapFactory().getMap(925020003);// rooftop
                c.changeMap(mappzz, mappzz.getPortal(1));
                c.getClient().getSession().writeAndFlush(UIPacket.openUI(72));// Dojo ui TODO add points?
            } else {
    ...
    private static void spawnMonster(final MapleMap map, final int stage) {
        final int mobid;
        /*
		 * mobid = 9300183 + stage; if (mobid < 9300184 || mobid > 9300215) { return; }
         */
        ...
        if (mobid != 0) {
            ...
            MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                    map.spawnMonsterWithEffect(MapleLifeProvider.getMonster(mobid), 15,
