...
public class MapScriptMethods {
        ...
        switch (onFirstUserEnter.fromString(scriptName)) {
            ...
            case dojang_Eff: {
                if (c.getPlayer().getMapId() == 925020100 || c.getPlayer().getMapId() == 925030100
                    ...
                    c.getPlayer().getMap().startMapEffect("제한시간은 15분, 늦지 않게 몬스터를 쓰러트리고 다음 층으로 올라가면 돼!", 5120024);
                }
            ...
            case kenta_mapEnter:
                ...
                break;
            case cygnus_Summon: {
                c.getPlayer().getMap().startMapEffect("이곳을 찾아 온 사람을 보는 것은 정말 오랜만이에요. 하지만 무사히 돌아간 분도 없었답니다.", 5120043);
                break;
            }
            case iceman_Boss: {
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            c.getSession().writeAndFlush(UIPacket.getDirectionInfo(1, 2100));
                            Thread.sleep(2800);
                            c.getSession().writeAndFlush(UIPacket.showInfo("키보드의 [←], [→] 키를 누르면 이동할 수 있습니다."));
                            c.getSession().writeAndFlush(UIPacket.getDirectionInfo(1, 3000));
                            Thread.sleep(3000);
                            c.getSession().writeAndFlush(UIPacket.showInfo("키보드의 [↑] 키를 누르면 다른 세계로 갈 수 있습니다."));
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            c.getSession().writeAndFlush(UIPacket.getDirectionInfo(1, 1800));
                            Thread.sleep(1800);
                            c.getSession().writeAndFlush(UIPacket.showInfo("Ctrl 키로 공격 할 수 있습니다."));
                            c.getSession().writeAndFlush(UIPacket.showInfo("쇠사슬을 부수세요."));
                            c.getSession().writeAndFlush(UIPacket.IntroEnableUI(0));
                            c.getPlayer().ea();
    ...
    private static void sendDojoClock(MapleClient c, int time) {
        c.getSession().writeAndFlush(MainPacketCreator.getClock(time));
    }
    ...
    private static void sendDojoStart(MapleClient c, int stage) {
        ...
        c.getSession().writeAndFlush(MainPacketCreator.showEnterEffect("dojang/start/stage"));
        c.getSession().writeAndFlush(MainPacketCreator.showEnterEffect("dojang/start/number/" + stage));
