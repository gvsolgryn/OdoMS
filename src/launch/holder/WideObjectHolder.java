...
public class WideObjectHolder {
    ...
    public void LoadAllGuild() {
        try {
            ...
            System.out.println("전체 길드 총" + guilds.size() + "개를 캐싱했습니다.");
        } catch (Exception e) {
        ...
        if (alliance != null) {
            ...
            for (int i = 0; i < alliance.getNoGuilds(); i++) {
                ...
                if (guild != null) {
                    if (g_ != null) {
                        if (expelled) {
                            guild.broadcast(MainPacketCreator.serverNotice(5, "[" + g_.getName() + "] 길드가 연합에서 추방당하였습니다."));
                        } else {
                            guild.broadcast(MainPacketCreator.serverNotice(5, "[" + g_.getName() + "] 길드가 연합에서 탈퇴하였습니다."));
                        }
            ...
            if (g_ != null) {
                ...
                if (expelled) {
                    g_.broadcast(MainPacketCreator.serverNotice(5, "연합에서 추방당하였습니다."));
                } else {
                    g_.broadcast(MainPacketCreator.serverNotice(5, "연합에서 탈퇴하였습니다."));
                }
            } else {
                alliance.broadcast(MainPacketCreator.serverNotice(5, "연합이 해체되었습니다."));
