            ...
            if (H == ServerConstants.AutoHotTimeSystemHour/* || H == ServerConstants.AutoHotTimeSystemHour - 12*/) {
                if (M == ServerConstants.AutoHotTimeSystemMinute) {
                    if (ServerConstants.AutoHotTimeSystemtemchacks) {
                        ...
                        WorldBroadcasting.broadcastMessage(MainPacketCreator.serverNotice(1, "핫타임 아이템을 지급 받으셨습니다."));
                        System.out.println("[핫타임] " + ItemInformation.getInstance().getName(itemcode) + " 및 " + ItemInformation.getInstance().getName(itemcode2) +" 및 " + ItemInformation.getInstance().getName(itemcode3) + "를(을) 지급했습니다.");
                    }
