...
import static constants.ServerConstants.chr;
...
import handler.channel.AswanHandler;
import handler.auction.AuctionHandler;
import handler.channel.BBSHandler;
import handler.channel.BuddyListHandler;
import handler.channel.ChatHandler;
import handler.channel.DemianHandler;
...
import handler.channel.MatrixHandler;
import handler.channel.MobHandler;
import handler.channel.MonsterCollectionHandler;
...
import handler.channel.SecurityHandler;
    ...
    public static final void handlePacket(final RecvPacketOpcode header, final ReadingMaple rh, final MapleClient c,
		 ...
		 * ENTER_ASWAN: AswanHandler.EnterAswan(c); break;
            ...
            case SLFCG_SECURITY2: {
                if (c.getPlayer().isGM()) {
                    System.out.println(c.getPlayer().getName() + " 운영자 핵사용 감지");
                    break;
                }
                ...
                if (target != null) {
                    if (target.ban(header == RecvPacketOpcode.SLFCG_SECURITY ? "Debugger Detected" : "Cheat Detected", true,
                            true)) {
                        System.out.println(c.getPlayer().getName() + " 핵사용 감지, 밴 완료");
                    } else {
                        System.out.println(c.getPlayer().getName() + " 핵사용 감지, 밴 실패");
                    }
