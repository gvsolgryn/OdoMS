...
public class CharLoginHandler {
    ...
    public static void CharlistRequest(ReadingMaple rh, MapleClient c) {
        ...
        System.out.println("[알림] " + c.getSessionIPAddress().toString() + " 에서 " + c.getAccountName() + " 계정으로 " + (channel == 0 ? 1 : channel == 1 ? "20세이상" : channel) + " 채널로 연결을 시도중입니다.");
   ...
   /* public static boolean SakuraLogin(String LoginID)
    ...
    public static void secondpasword(ReadingMaple r, MapleClient c) {
        if (c.isUsing2ndPassword()) {
            ...
            c.sendPacket(MainPacketCreator.serverNotice(1, "왼쪽 상단에 있는 2차 비밀번호 설정을 해 주세요."));
    ...
    public static void CreateChar(ReadingMaple lea, MapleClient c) { //변조
        String name;
        byte gender, skin;
        short subcategory;
