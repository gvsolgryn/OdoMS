...
import handler.admin.AdminToolClientHandler;
import handler.admin.AdminToolPacket;
import launch.AdminToolServer;
import packet.transfer.read.ByteStream;
import packet.transfer.read.ReadingMaple;

public class AdminToolServerHandler extends IoHandlerAdapter {

    @Override
    public void sessionOpened(IoSession session) {
        AdminToolServer.session.add(session);
        session.write(AdminToolPacket.Info());
    }

    @Override
    public void sessionClosed(IoSession session) {
        AdminToolServer.session.remove(session);
    }
    ...
    public void handlePacket(final IoSession session, final short header, final String data) {
        ...
        if (!ServerConstants.isAdminIp(address)) {
            return;
        }
        switch (header) {
            case 0: {
