...
public class PlayerHandler {
    ...
    public static void ExitBlockGame(ReadingMaple rh, MapleClient c) { //와글와글
    ...
    public static void HandleBlockGameRes(ReadingMaple rh, MapleClient c) { //와글와글
        byte type = rh.readByte();
        if (type == 0x03) {
            c.getSession().writeAndFlush(SLFCGPacket.BlockGameCommandPacket(3));
            c.getPlayer().setBlockCount(0);
    ...
    public static void UseChair(int itemId, MapleClient c, MapleCharacter chr, ReadingMaple rh) {
        ...
        if (chr.getMapId() == FishingHandler.FishingMap && itemId == FishingHandler.FishingChair && chr.Fishing()) {
            chr.dropMessage(6, "낚시를 끝낸 후 5초간은 낚시를 다시 시작할 수 없습니다.");
        ...
        for (int j = 0; j < v1; j++) {
            ...
            switch (skillid) {
                ...
                case 31221014:
                    ...
                    break;
                case 400041023:
                    ForceAtom atom = new ForceAtom(m1, 3, 0x20, 0x21, 0x16, 0x54, (short) 0); //  0x20, 0x21, 0x16 요거 3개중 하나인데
                ...
                case 400021045: {
                    if (c.getPlayer().FlameBall > 0) {
                        ...
                        c.getPlayer().getMap().broadcastMessage(MatrixPacket.플레임디스차지여우재생성(c.getPlayer(), monster));
                    }
                }
            }
        }
                ...
                if ((chr.getJob() >= 411 && chr.getJob() <= 412) || (chr.getJob() >= 421 && chr.getJob() <= 422) || (chr.getJob() >= 1411 && chr.getJob() <= 1412)) {
                    ...
                    for (Pair<Integer, MapleSummon> s : chr.getSummons().values()) {
                        if (s.getLeft() == v1) {
                            ...
                            attacker.damage(chr, refDmg, true); // 데미지 제한
                        }
                    }
                }
            }
    ...
    public static void SpecialSkill(ReadingMaple rh, final MapleClient c, final MapleCharacter chr) {
        ...
        switch (skillid) {
            ...
            case 3321003: {
                ...
                if (!objs.isEmpty()) {
                    for (MapleMapObject monss : c.getPlayer().getMap().getAllMonstersThreadsafe()) { // 범위 내 보스몬스터가 있을시 추가 공격
            ...
            case 400021044: {
                ...
                for (byte i = 1; i <= 8; i++) {
                    chr.getMap().broadcastMessage(MatrixPacket.플레임디스차지여우(c.getPlayer()));
                }
            ...
            case 33120056: //램피지 애즈 원
            case 33121052: //램피지 애즈 원
...
//                                System.out.println(pet);
    ...
    public static void closeRangeAttack(ReadingMaple rh, final MapleClient c, final MapleCharacter chr,
        ...
        if (attack.skill > 0) {
            if (chr.getMapId() == 123456788) {
                ...
                chr.dropMessage(1, "마을에서 스킬 사용은 금지되어있습니다.");
                return;
            }
        ...
        if (attack.skill != 0) {
            ...
            if (SkillFactory.getSkill(attack.skill).getEffect(skillLevel).getCooldown() > 0) {
                switch (attack.skill) {
                    ...
                    case 155001102:  //되살아나는 악몽 , 끝나지 않은 악몽   쿨타임
                    case 155110000:
                    case 155120000: {
    ...
    public static void SetFreeJob(final ReadingMaple rh, final MapleCharacter chr) {
        ...
        if (chr.haveItem(4310086, jobcoin, false, false)) {
            ...
            //    chr.getClient().getSession().writeAndFlush(MainPacketCreator.getNPCTalk(9010000, (byte) 0, "당신은 이제부터 새로운 직업을 갖게 되었습니다.", "00 00", (byte) 0));
            chr.getClient().getSession().writeAndFlush(MainPacketCreator.resetActions(chr));
            ...
            //    chr.getClient().getSession().writeAndFlush(MainPacketCreator.getNPCTalk(9010000, (byte) 0, "새로운 직업을 선택하기 위한 메소가 부족합니다.", "00 00", (byte) 0));
            chr.getClient().getSession().writeAndFlush(MainPacketCreator.resetActions(chr));
        }
    }
    ...
    public static void OnThrowGrenade(ReadingMaple rh, MapleClient c) {
        ...
        int nBySummonedID = rh.readInt(); // 아마 아닐거
    ...
    public static void link_skill(ReadingMaple rp, MapleClient c) {
        try {
            ...
            if (ps.executeUpdate() == 1) {
                c.getPlayer().dropMessage(1, "해당 플레이어에게 스킬을 전수 하였습니다.");
                c.getPlayer().ea();
            } else {
                c.getPlayer().dropMessage(1, "DB에 등록을 실패 하였습니다. 관리자에게 문의 해 주세요");
            }
            ps.close();
            con.close();
            con.close();
    ...
    public static void OnMemoInGameRequest(final ReadingMaple r, final MapleClient c) {
        ...
        for (ChannelServer cs : ChannelServer.getAllInstances()) {
            for (MapleCharacter ch : cs.getPlayerStorage().getAllCharacters()) {
                chr = ch;
                break;
            }
        }
    ...
    public static void HyperSkillRemove(final ReadingMaple rh, final MapleClient c) {
        ...
        if (c.getPlayer().getMeso() < meso) {
            c.getPlayer().dropMessage(1, "메소가 부족하여 하이퍼스킬 초기화를 하실 수 없습니다.");
            c.getSession().writeAndFlush(MainPacketCreator.resetActions(c.getPlayer()));
            return;
        }
    ...
    public static void HyperStatRemove(final ReadingMaple rh, final MapleClient c) {
        if (c.getPlayer().getMeso() < 10000000) {
            c.getPlayer().dropMessage(1, "메소가 부족하여 하이퍼스탯 초기화를 하실 수 없습니다.");
            c.getSession().writeAndFlush(MainPacketCreator.resetActions(c.getPlayer()));
            return;
        }
        final HashMap<ISkill, SkillEntry> hashMap = new HashMap<ISkill, SkillEntry>();
    ...
    public static void DarkSpear(ReadingMaple rm, MapleClient c) {
        ...
        int mainskillid = nSkillID; //에인션트 아스트라 쿨타임
    ...
    public static void SymbolExp(ReadingMaple slea, MapleClient c) { //아케인심볼
        try {
            int type = slea.readInt();
            if (type == 0) {
                int src = slea.readInt();//슬롯 넘버
                    ...
                    if (symbol.getItemId() == items.getItemId()) {
                        ...
                        if (source.getArcLevel() == _source.getArcLevel()) {
                            ...
                            if (symbol.getArcEXP() >= Math.pow(symbol.getArcLevel(), 2) + 11) {
                                ...
                                c.getSession().writeAndFlush(MainPacketCreator.serverNotice(1, "심볼 경험치가 최대치에 달했습니다.\n심볼의 레벨을 올려주십시오."));
                ...
                if (c.getPlayer().getMeso() < meso) {
                    c.getSession().writeAndFlush(MainPacketCreator.serverNotice(1, "[알림] 메소가 부족하여 심볼 레벌업을 하실 수 없습니다."));
                }
    ...
    public static void DailyGift(ReadingMaple rm, MapleClient c) {
        ...
        if (c.getPlayer().getDailyGift() == null) {
            c.getSession().writeAndFlush(MainPacketCreator.serverNotice(1, "알 수 없는 오류로 지급을 받으실 수 없습니다."));
            c.getSession().writeAndFlush(MainPacketCreator.resetActions(c.getPlayer()));
            return;
        }
        if (!c.getPlayer().getDailyGift().getDailyData().equals(cal.get(Calendar.YEAR) + (Month == 10 || Month == 11 || Month == 12 ? "" : "0") + Month + (cal.get(Calendar.DATE) < 10 ? "0" : "") + cal.get(Calendar.DATE))) {
            ...
            c.getSession().writeAndFlush(MainPacketCreator.serverNotice(1, "아이템 지급이 완료 되었습니다."));
        }
