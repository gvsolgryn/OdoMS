...
public class ItempotHandler {
    ...
    public static void putItempot(ReadingMaple rm, MapleClient c) {
        ...
        if (targetslot == 0) { // 아이템팟 사용후 메세지
            ...
            c.send(MainPacketCreator.resetActions(c.getPlayer())); // 플레이어를 움직일수 있게함
            return;
        }
        ...
        InventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, (byte) slot, (short) 1, false); // 아이템팟을 인벤토리에서
        // 삭제
        c.send(MainPacketCreator.resetActions(c.getPlayer())); // 플레이어를 움직일수 있게함
    }
