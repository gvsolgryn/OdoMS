    ...
    public static final void summonSkill(final ReadingMaple rh, final MapleClient ha, final MapleCharacter chr) {
        ...
        switch (skillid) {
            ...
            case 400041038: {
                    ...
                    if (objs.size() > 0)
                        chr.getMap().broadcastMessage(MainPacketCreator.absorbingMarkOfTheif(chr.getId(), monsters.get(0).getObjectId(), 4120019, monsters, pos1, chr.getKeyValue2("표창"), count));
                }
                break;
            default: {
