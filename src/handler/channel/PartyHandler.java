            ...
            if (party != null) {
                if (action == 0x28) { // Accept 1.2.220+ (+1)
                    if (party.getMembers().size() < 6) {
                        ...
                        c.getSession().writeAndFlush(MainPacketCreator.serverNotice(5, "해당 파티는 이미 파티원이 꽉 찼습니다."));
                    }
                    ...
                    if (cfrom != null) {
                        cfrom.getClient().getSession().writeAndFlush(
                                MainPacketCreator.serverNotice(5, c.getPlayer().getName() + " 님이 파티 초대를 거절하였습니다."));
                    }
                }
            } else {
        ...
        switch (operation) {
            ...
            case 4: // invite
                final MapleCharacter invited = c.getChannelServer().getPlayerStorage()
                        .getCharacterByName(rh.readMapleAsciiString());
                if (invited != null) {
                    if (invited.getParty() == null) {
                        if (party.getMembers().size() < 6) {
                            if (c.getPlayer().getMapId() == invited.getMapId()) {
                                ...
                                c.getPlayer().dropMessage(1, "같은 장소에 있지 않으면 초대할 수 없습니다.");
                            }
                        } else {
