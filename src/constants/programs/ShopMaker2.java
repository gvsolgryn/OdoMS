...
public class ShopMaker2 {
    ...
    private static void dropInfo(List<Integer> list, PrintWriter name) {
        ...
        for (Integer Int : list) {
            ...
            String shopname;
            if (et == null) {
                shopname = MapleWeaponType.getMapleWeaponTypeById(value - EquipType.무기.shopIdAlter()).name();
            } else {
                shopname = et.name();
            }
            String a = "    ";
    ...
    private enum MapleWeaponType {
        ...
        private static MapleWeaponType getWeaponType(int itemId) {
            ...
            MapleWeaponType[] type = {무기아님, 샤이닝로드, 소울슈터, 무기아님, 무기아님, 무기아님, 무기아님, 무기아님, 무기아님, 무기아님, 한손검, 한손도끼, 한손둔기, 단검,
                블레이드, 보조무기, 케인, 완드, 스태프, 무기아님, 두손검, 두손도끼, 두손둔기, 창, 폴암, 활, 석궁, 아대, 너클, 총, 무기아님, 무기아님, 듀얼보우건};
