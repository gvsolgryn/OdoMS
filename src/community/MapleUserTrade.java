...
public class MapleUserTrade {
    ...
    public final void chat(final String message) {
        if (ServerConstants.chatlimit >= 500) {
            ...
            ControlUnit.TradeChatList.clear();
            ControlUnit.trade.setModel(ControlUnit.TradeChatList);
        }
        ServerConstants.chatlimit++;
        ControlUnit.TradeChatList.addElement("[교환] : " + chr.getName() + " : " + message + "");
        ControlUnit.trade.setModel(ControlUnit.TradeChatList);
        ...
        if (partner.isLocked()) { // Both locked
            if (!local.check() || !partner.check()) { // Check for full inventories
                ...
                c.getClient().getSession()
                        .writeAndFlush(MainPacketCreator.serverNotice(5, "트레이드를 완료하는데 필요한 인벤토리 공간이 부족합니다."));
                partner.getChr().getClient().getSession()
                        .writeAndFlush(MainPacketCreator.serverNotice(5, "트레이드를 완료하는데 필요한 인벤토리 공간이 부족합니다."));
            } else {
    ...
    public static final void startTrade(final MapleCharacter c, boolean isTrade, byte type) {
        if (c.getLevel() < 60) {
            c.getClient().getSession().writeAndFlush(MainPacketCreator.serverNotice(5, "레벨 60이상부터 트레이드 이용이 가능합니다."));
            return;
        }
        if (c.getTrade() == null) {
            ...
            c.getClient().getSession().writeAndFlush(MainPacketCreator.serverNotice(5, "이미 트레이드 중입니다."));
        }
    }
    ...
    public static final void inviteTrade(final MapleCharacter c1, final MapleCharacter c2, boolean isTrade, byte type) {
        if (c2.getLevel() < 60 || c1.getLevel() < 60) {
            c1.getClient().getSession().writeAndFlush(MainPacketCreator.serverNotice(5, "레벨 60이상부터 트레이드 이용이 가능합니다."));
        ...
        if (c2.getTrade() == null) {
            ...
            c1.getClient().getSession().writeAndFlush(MainPacketCreator.serverNotice(5, "이미 누군가와 트레이드 중입니다."));
            cancelTrade(c1.getTrade());
        }
    }
    ...
    public static final void visitTrade(final MapleCharacter c1, final MapleCharacter c2, boolean isTrade, byte type) {
        if (c1.getTrade() != null && c1.getTrade().getPartner() == c2.getTrade() && c2.getTrade() != null
            ...
            c1.getClient().getSession().writeAndFlush(MainPacketCreator.serverNotice(5, "상대방이 이미 트레이드를 종료했습니다."));
        }
    }
        ...
        if (trade != null) {
            if (trade.getPartner() != null) {
                ...
                other.getClient().getSession()
                        .writeAndFlush(MainPacketCreator.serverNotice(5, c.getName() + " 님이 교환 요청을 거절하셨습니다."));
            }
            trade.cancel();
            c.setTrade(null);
        }
