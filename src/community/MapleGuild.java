...
public class MapleGuild {
    ...
    public final void saveGuildSkill(int guildid, int skillid, int skillLevel) {
        try {
            Connection con = MYSQL.getConnection();
            PreparedStatement ps = con.prepareStatement("DELETE FROM guildskills WHERE guildid=" + guildid + " AND skillid=" + skillid);
            ps.executeUpdate();
            ps.close();
            if (skillLevel != 0) {
                ps = con.prepareStatement("INSERT INTO guildskills(`guildid`, `skillid`, `level`, `timestamp`, `purchaser`) VALUES(?, ?, ?, ?, ?)");
    ...
    public void removeJoinRequester(int playerId, boolean joined) {
        ...
        if (joined) {
            p.dropMessage(1, "길드 가입을 성공하였습니다.");
        }
    ...
    public final boolean changeRank(final int cid, final int newRank) {
        for (final MapleGuildCharacter mgc : members) {
            if (cid == mgc.getId()) {
                ...
                broadcast(MainPacketCreator.serverNotice(5, "[길드 알림] " + mgc.getName() + "님의 직위가 [" + this.getRankTitle(newRank) + "](으)로 변경 되었습니다."));
                sendGuildInfo();
                return true;
            }
        }
